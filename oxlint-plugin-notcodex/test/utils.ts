...
export const createOxlintRuleHarness = (
  ...
  const run: RuleHarness["run"] = Effect.fnUntraced(function* (source: string) {
    ...
    const fixtureDir = yield* fs.makeTempDirectoryScoped({ prefix: "notcodex-oxlint-" });
    ...
    const pluginPath = path.join(repoRoot, "oxlint-plugin-notcodex", "index.ts");

    yield* fs.writeFileString(
      configPath,
      yield* encodeOxlintConfig({
        jsPlugins: [{ name: "notcodex", specifier: pluginPath }],
