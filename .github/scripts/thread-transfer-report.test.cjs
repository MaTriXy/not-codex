...
test("renders baseline, impact, ceiling, and ceiling changes", () => {
  ...
  const comment = renderComment({
    ...
    currentRun: {
      ...
      conclusion: "success",
      url: "https://github.com/MaTriXy/not-codex/actions/runs/2",
    },
    baselineRun: {
      sha: "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
      matchesBase: true,
      url: "https://github.com/MaTriXy/not-codex/actions/runs/1",
    },
  });
  ...
  assert.match(comment, /<!-- notcodex-thread-transfer-report -->/);
  assert.match(
    comment,
    /<!-- notcodex-thread-transfer-result-sha:bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb -->/,
  );
});
...
test("resolves a fallback PR with a redacted head repo and exact main baseline", async () => {
  ...
  await resolve({
    github,
    context: {
      repo: { owner: "MaTriXy", repo: "not-codex" },
      payload: {
        workflow_run: {
          id: 2,
          event: "pull_request",
          ...
          head_repository: { full_name: "MaTriXy/not-codex" },
          conclusion: "success",
          pull_requests: [],
        },
      },
      ...
      paginate: async (method) => {
        ...
        return [5350, 5351].map((number) => ({
          ...
          head: {
            sha: "head-sha",
            ref: "feature-branch",
            repo: { full_name: "MaTriXy/not-codex" },
          },
        }));
      },
      rest: {
      ...
      paginate: async () => [
        {
          ...
          body: `<!-- notcodex-thread-transfer-report -->\n<!-- notcodex-thread-transfer-result-sha:${sha} -->`,
        },
      ],
      rest: {
        issues: {
