...
const PullRequestRepository = TrimmedNonEmptyString.check(Schema.isMaxLength(1024));
const PullRequestPath = TrimmedNonEmptyString.check(Schema.isMaxLength(4096));
const PullRequestOpaqueId = TrimmedNonEmptyString.check(Schema.isMaxLength(1024));
const PullRequestWebUrl = TrimmedNonEmptyString.check(
...
export const PullRequestActor = Schema.Struct({
  login: TrimmedNonEmptyString.check(Schema.isMaxLength(512)),
  name: Schema.NullOr(Schema.String.check(Schema.isMaxLength(1024))),
  /** Null where a host does not report one, which is what the initials fall back to. */
  avatarUrl: Schema.NullOr(PullRequestWebUrl),
});
...
export const PullRequestCheck = Schema.Struct({
  ...
  url: Schema.NullOr(PullRequestWebUrl),
});
...
export const PullRequestComment = Schema.Struct({
  id: PullRequestOpaqueId,
  kind: PullRequestCommentKind,
  author: Schema.NullOr(PullRequestActor),
  body: Schema.String,
  createdAt: IsoDateTime,
  url: Schema.NullOr(PullRequestWebUrl),
  path: Schema.NullOr(PullRequestPath),
...
export const PullRequestListCursors = Schema.Record(
  TrimmedNonEmptyString.check(Schema.isMaxLength(1280)),
  ...
  Schema.makeFilter(
    (cursors) =>
      Object.keys(cursors).length <= 500 || "A pull request page may carry at most 500 cursors.",
  ),
);
...
export const PullRequestListInput = Schema.Struct({
   ...
   */
  host: Schema.optional(PullRequestHost),
...
export const PullRequestDiffInput = Schema.Struct({
   ...
   */
  commit: Schema.optional(TrimmedNonEmptyString.check(Schema.isMaxLength(512))),
});
...
export const PullRequestDiffFileContentsInput = Schema.Struct({
  ...
  oldPath: PullRequestPath,
  newPath: PullRequestPath,
});
...
export const PullRequestReviewCommentDraft = Schema.Struct({
   ...
   */
  oldPath: Schema.optional(PullRequestPath),
  line: PositiveInt,
  side: PullRequestDiffSide,
  body: CommentBody,
});
...
export const PullRequestSubmitReviewInput = Schema.Struct({
  ...
  Schema.makeFilter((review) => {
    const text = [review.body, ...review.comments.map((comment) => comment.body)].join("");
    return (
      new TextEncoder().encode(text).byteLength <= 1_048_576 ||
      "A pull request review may contain at most 1 MiB of UTF-8 text."
    );
  }),
);
...
export const PullRequestReviewerRequestInput = Schema.Struct({
  ...
  reviewers: Schema.Array(
    Schema.Struct({ id: PullRequestOpaqueId, kind: PullRequestReviewerKind }),
...
> = {
  ...
  bitbucket: {
    missing:
      "Bitbucket needs API credentials on the server. Set NOT_CODEX_BITBUCKET_EMAIL and NOT_CODEX_BITBUCKET_API_TOKEN, or NOT_CODEX_BITBUCKET_ACCESS_TOKEN.",
    unauthenticated:
      "Bitbucket rejected the configured credentials. Check NOT_CODEX_BITBUCKET_EMAIL and NOT_CODEX_BITBUCKET_API_TOKEN.",
  },
};
...
/** The environment is already spending its bounded source-control host budget. */
export class PullRequestBusyError extends Schema.TaggedErrorClass<PullRequestBusyError>()(
  "PullRequestBusyError",
  {
    host: PullRequestHost,
    retryAfterMs: Schema.Int.check(Schema.isBetween({ minimum: 100, maximum: 60_000 })),
  },
  { httpApiStatus: 429 },
) {
  [HttpServerRespondable.symbol]() {
    return HttpServerResponse.schemaJson(PullRequestBusyError)(this, { status: 429 });
  }

  override get message(): string {
    return `Pull request reads for ${this.host} are busy. Retry shortly.`;
  }
}
