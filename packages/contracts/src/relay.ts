...
}).annotate({ description: "Links an authenticated cloud user to a Not Codex environment." });
...
>()("@notcodex/contracts/relay/RelayClientPrincipal") {}
...
>()("@notcodex/contracts/relay/RelayEnvironmentPrincipal") {}

const RelayClientBearerAuthorization = HttpApiSecurity.http({ scheme: "bearer" }).pipe(
  HttpApiSecurity.annotate(
    OpenApi.Description,
    "Clerk session or OAuth bearer token for the signed-in Not Codex Connect user.",
  ),
);
...
export const RelayPublicClientId = Schema.Literals(["notcodex-mobile", "notcodex-web"]);
export type RelayPublicClientId = typeof RelayPublicClientId.Type;
export const RelayMobileClientId = "notcodex-mobile" as const;
export const RelayWebClientId = "notcodex-web" as const;
...
export const RelayApi = HttpApi.make("RelayApi")
  ...
  .annotate(OpenApi.Title, "Not Codex Relay API")
  .annotate(OpenApi.Version, "1.0.0")
  .annotate(
    OpenApi.Description,
    "Control-plane API for linking Not Codex environments, connecting authorized clients, and publishing agent activity.",
  );
