...
import { resolveGitWorktreePath, resolveWorktreeNotCodexHome } from "./devHome.ts";
...
const makeRepo = (
  ...
  Effect.acquireRelease(
    Effect.sync(() => {
      const root = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "notcodex-devhome-"));
      if (kind === "worktree") {
...
describe("resolveWorktreeNotCodexHome", () => {
  it.effect("answers with .notcodex before the dev runner creates it", () =>
    Effect.gen(function* () {
      const { root, nested } = yield* makeRepo("worktree");
      const home = yield* resolveWorktreeNotCodexHome(nested);
      assert.equal(home, NodePath.join(NodePath.resolve(root), ".notcodex"));
