...
describe("connectAuth", () => {
  it("round-trips state and challenge through the authorize URL fragment", () => {
    const url = buildConnectAuthorizeRequestUrl({
      hostedAppUrl: "https://app.notcodex.bpro.dev",
      state: "q7mK9xV2pL4nR8sT6wYzAQ",
      challenge: "E9Melhoa2OwvFrEMTJguCHaoeK1t8URWbuGJSstw-cM",
    });
    const parsed = new URL(url);

    expect(parsed.origin).toBe("https://app.notcodex.bpro.dev");
  ...
  it("rejects authorize requests missing state or challenge", () => {
    expect(
      readConnectAuthorizeRequest(new URL("https://app.notcodex.bpro.dev/connect")),
    ).toBeNull();
    expect(
      readConnectAuthorizeRequest(new URL("https://app.notcodex.bpro.dev/connect#state=abc")),
    ).toBeNull();
    expect(
      readConnectAuthorizeRequest(new URL("https://app.notcodex.bpro.dev/connect#challenge=abc")),
    ).toBeNull();
  });
  ...
  it("builds a PKCE authorize URL against the Clerk endpoint", () => {
    const url = new URL(
      buildConnectClerkAuthorizeUrl({
        authorizationEndpoint: "https://clerk.notcodex.bpro.dev/oauth/authorize",
        clientId: "oauthapp_123",
        redirectUri: connectCallbackUrl("https://app.notcodex.bpro.dev"),
    ...
    expect(url.origin).toBe("https://clerk.notcodex.bpro.dev");
