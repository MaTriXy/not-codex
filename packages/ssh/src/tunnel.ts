...
} from "@notcodex/shared/httpReadiness";
import * as NetService from "@notcodex/shared/Net";
import { extractJsonObject, fromLenientJson } from "@notcodex/shared/schemaJson";
import { satisfiesSemverRange } from "@notcodex/shared/semver";
...
export interface RemoteNotCodexRunnerOptions {
...
export interface SshEnvironmentManagerOptions {
  ...
  readonly resolveCliRunner?: Effect.Effect<RemoteNotCodexRunnerOptions>;
}
...
function sshRunnerLogFields(runner: RemoteNotCodexRunnerOptions | undefined) {
...
remote_node_satisfies_engine() {
  NOT_CODEX_NODE_ENGINE_RANGE=@@NOT_CODEX_NODE_ENGINE_RANGE@@
  if [ -z "$NOT_CODEX_NODE_ENGINE_RANGE" ]; then
    return 0
  fi
  node - "$NOT_CODEX_NODE_ENGINE_RANGE" <<'NODE'
@@NOT_CODEX_NODE_ENGINE_CHECK_SCRIPT@@
NODE
}
...
ensure_remote_node_path() {
  ...
  if ! command -v node >/dev/null 2>&1 && [ -d "$NVM_DIR/versions/node" ]; then
    for NOT_CODEX_NODE_BIN in "$NVM_DIR"/versions/node/*/bin; do
      if [ -x "$NOT_CODEX_NODE_BIN/node" ]; then
        PATH="$NOT_CODEX_NODE_BIN:$PATH"
        export PATH
      fi
    done
  fi
...
set -eu
@@NOT_CODEX_NODE_ENV_SCRIPT@@
ensure_remote_node_path || true
NOT_CODEX_NODE_SCRIPT_PATH=@@NOT_CODEX_NODE_SCRIPT_PATH@@
if [ -n "$NOT_CODEX_NODE_SCRIPT_PATH" ]; then
  ...
  exec node "$NOT_CODEX_NODE_SCRIPT_PATH" "$@"
fi
if command -v notcodex >/dev/null 2>&1; then
  exec notcodex "$@"
fi
if command -v npx >/dev/null 2>&1; then
  exec npx --yes @@NOT_CODEX_PACKAGE_SPEC@@ "$@"
fi
if command -v npm >/dev/null 2>&1; then
  exec npm exec --yes @@NOT_CODEX_PACKAGE_SPEC@@ -- "$@"
fi
printf 'Remote host is missing the notcodex CLI and could not install @@NOT_CODEX_PACKAGE_SPEC@@ because node/npm/npx are unavailable on PATH. Install Node or configure a supported version manager for non-interactive shells.\\n' >&2
exit 1
`;
...
@@NOT_CODEX_NODE_ENV_SCRIPT@@
STATE_KEY="$1"
STATE_DIR="$HOME/.notcodex/ssh-launch/$STATE_KEY"
DEFAULT_SERVER_HOME="$HOME/.notcodex"
DEFAULT_RUNTIME_FILE="$DEFAULT_SERVER_HOME/userdata/server-runtime.json"
PORT_FILE="$STATE_DIR/port"
PID_FILE="$STATE_DIR/pid"
MANAGED_FILE="$STATE_DIR/managed"
LOG_FILE="$STATE_DIR/server.log"
RUNNER_FILE="$STATE_DIR/run-notcodex.sh"
RUNNER_NEXT="$STATE_DIR/run-notcodex.next.$$"
mkdir -p "$STATE_DIR"
cleanup_runner_next() {
  rm -f "$RUNNER_NEXT"
}
...
pick_port() {
  node - "$PORT_FILE" "@@NOT_CODEX_DEFAULT_REMOTE_PORT@@" "@@NOT_CODEX_REMOTE_PORT_SCAN_WINDOW@@" <<'NODE'
@@NOT_CODEX_PICK_PORT_SCRIPT@@
NODE
}
wait_ready() {
  node - "$REMOTE_PORT" "$1" "@@NOT_CODEX_READY_PROBE_TIMEOUT_MS@@" <<'NODE'
@@NOT_CODEX_WAIT_READY_SCRIPT@@
NODE
}
wait_for_pid_exit() {
  PID_TO_WAIT="$1"
...
if [ -n "$DEFAULT_REMOTE_PORT" ]; then
  ...
  if wait_ready "@@NOT_CODEX_REUSE_READY_TIMEOUT_MS@@"; then
...
if [ "$REMOTE_MANAGED" = "external" ]; then
  if [ -z "$REMOTE_PORT" ] || ! wait_ready "@@NOT_CODEX_REUSE_READY_TIMEOUT_MS@@"; then
    REMOTE_PID=""
    REMOTE_PORT=""
    REMOTE_MANAGED=""
  fi
  ...
  if [ "$RUNNER_CHANGED" -eq 1 ]; then
    ...
    REMOTE_PORT=""
    REMOTE_MANAGED=""
  elif ! wait_ready "@@NOT_CODEX_REUSE_READY_TIMEOUT_MS@@"; then
    kill "$REMOTE_PID" 2>/dev/null || true
    wait_for_pid_exit "$REMOTE_PID"
    REMOTE_PID=""
    REMOTE_PORT=""
...
if [ -z "$REMOTE_PORT" ]; then
  ...
  nohup env NOT_CODEX_NO_BROWSER=1 "$RUNNER_FILE" serve --host 127.0.0.1 --port "$REMOTE_PORT" --base-dir "$DEFAULT_SERVER_HOME" >>"$LOG_FILE" 2>&1 < /dev/null &
  REMOTE_PID="$!"
  printf '%s\\n' "$REMOTE_PID" >"$PID_FILE"
  printf '%s\\n' "$REMOTE_PORT" >"$PORT_FILE"
  printf 'managed\\n' >"$MANAGED_FILE"
  if ! wait_ready "@@NOT_CODEX_READY_TIMEOUT_MS@@"; then
    printf 'Remote Not Codex server did not become ready on 127.0.0.1:%s.\\n' "$REMOTE_PORT" >&2
...
STATE_DIR="$HOME/.notcodex/ssh-launch/@@NOT_CODEX_STATE_KEY@@"
DEFAULT_SERVER_HOME="$HOME/.notcodex"
RUNNER_FILE="$STATE_DIR/run-notcodex.sh"
mkdir -p "$STATE_DIR"
cat >"$RUNNER_FILE" <<'SH'
...
export function buildRemoteNotCodexRunnerScript(input?: RemoteNotCodexRunnerOptions): string {
  const packageSpec = shellSingleQuote(input?.packageSpec?.trim() || "notcodex@latest");
  const nodeScriptPath = input?.nodeScriptPath?.trim() || "";
  return stripTrailingNewlines(
    applyScriptPlaceholders(REMOTE_RUNNER_SCRIPT, {
      NOT_CODEX_PACKAGE_SPEC: packageSpec,
      NOT_CODEX_NODE_SCRIPT_PATH: shellSingleQuote(nodeScriptPath),
      NOT_CODEX_NODE_ENV_SCRIPT: buildRemoteNodeEnvScript(input),
    }),
  );
}

export function buildRemoteNodeEnvScript(input?: RemoteNotCodexRunnerOptions): string {
  return stripTrailingNewlines(
    applyScriptPlaceholders(REMOTE_NODE_ENV_SCRIPT, {
      NOT_CODEX_NODE_ENGINE_RANGE: shellSingleQuote(input?.nodeEngineRange?.trim() || ""),
      NOT_CODEX_NODE_ENGINE_CHECK_SCRIPT: stripTrailingNewlines(buildRemoteNodeEngineCheckScript()),
    }),
  );
}

export function buildRemoteLaunchScript(input?: RemoteNotCodexRunnerOptions): string {
  return applyScriptPlaceholders(REMOTE_LAUNCH_SCRIPT, {
    NOT_CODEX_NODE_ENV_SCRIPT: buildRemoteNodeEnvScript(input),
    NOT_CODEX_RUNNER_SCRIPT: stripTrailingNewlines(buildRemoteNotCodexRunnerScript(input)),
    NOT_CODEX_PICK_PORT_SCRIPT: stripTrailingNewlines(REMOTE_PICK_PORT_SCRIPT),
    NOT_CODEX_WAIT_READY_SCRIPT: stripTrailingNewlines(REMOTE_WAIT_READY_SCRIPT),
    NOT_CODEX_DEFAULT_REMOTE_PORT: String(DEFAULT_REMOTE_PORT),
    NOT_CODEX_REMOTE_PORT_SCAN_WINDOW: String(REMOTE_PORT_SCAN_WINDOW),
    NOT_CODEX_READY_TIMEOUT_MS: String(REMOTE_READY_TIMEOUT_MS),
    NOT_CODEX_REUSE_READY_TIMEOUT_MS: String(REMOTE_REUSE_READY_TIMEOUT_MS),
    NOT_CODEX_READY_PROBE_TIMEOUT_MS: String(SSH_READY_PROBE_TIMEOUT_MS),
  });
}

export function buildRemotePairingScript(
  target: DesktopSshEnvironmentTarget,
  input?: RemoteNotCodexRunnerOptions,
): string {
  return applyScriptPlaceholders(REMOTE_PAIRING_SCRIPT, {
    NOT_CODEX_STATE_KEY: remoteStateKey(target),
    NOT_CODEX_RUNNER_SCRIPT: stripTrailingNewlines(buildRemoteNotCodexRunnerScript(input)),
  });
}
...
export const launchOrReuseRemoteServer = Effect.fn("ssh/tunnel.launchOrReuseRemoteServer")(
  function* (
    ...
    runner?: RemoteNotCodexRunnerOptions,
  ): Effect.fn.Return<
...
const makeSshEnvironmentManager = Effect.fn("ssh/tunnel.SshEnvironmentManager.make")(function* (
  ...
  const createTunnelEntry = Effect.fn("ssh/tunnel.ensureTunnelEntry.create")(function* (input: {
    readonly key: string;
    readonly resolvedTarget: DesktopSshEnvironmentTarget;
    readonly runner?: RemoteNotCodexRunnerOptions;
...
>()("@notcodex/ssh/tunnel/SshEnvironmentManager") {
