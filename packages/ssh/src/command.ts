...
import type { DesktopSshEnvironmentTarget, DesktopUpdateChannel } from "@notcodex/contracts";
...
const PUBLISHABLE_NOT_CODEX_VERSION_PATTERN = /^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/u;
...
export function resolveRemoteNotCodexCliPackageSpec(input: {
  readonly appVersion: string;
  readonly updateChannel: DesktopUpdateChannel;
  readonly isDevelopment?: boolean;
}): string {
  const appVersion = input.appVersion.trim();
  if (!input.isDevelopment && PUBLISHABLE_NOT_CODEX_VERSION_PATTERN.test(appVersion)) {
    return `notcodex@${appVersion}`;
  }

  if (input.isDevelopment) {
    return "notcodex@nightly";
  }

  return input.updateChannel === "nightly" ? "notcodex@nightly" : "notcodex@latest";
}
