...
import {
  ...
  resolveRemoteNotCodexCliPackageSpec,
  runSshCommand,
} from "./command.ts";
...
describe("ssh command", () => {
  it.effect("parses resolved ssh config output into a target", () =>
    Effect.sync(() => {
      assert.deepEqual(
        parseSshResolveOutput(
          "devbox",
          ["hostname devbox.example.com", "user collaborator", "port 2222", ""].join("\n"),
        ),
        {
          alias: "devbox",
  ...
  it.effect("resolves the remote notcodex package spec from the desktop release channel", () =>
    Effect.sync(() => {
      assert.equal(
        resolveRemoteNotCodexCliPackageSpec({
          appVersion: "0.0.17",
      ...
      assert.equal(
        ...
        "notcodex@0.0.17-nightly.20260415.44",
      );
      assert.equal(
        resolveRemoteNotCodexCliPackageSpec({
