...
describe("ssh auth", () => {
  it.effect("detects ssh auth failures from common permission denied messages", () =>
    Effect.sync(() => {
      assert.equal(
        isSshAuthFailure(
          new Error(
            "collaborator@100.65.180.100: Permission denied (publickey,password,keyboard-interactive).",
          ),
        ),
        true,
      );
  ...
  it.effect("creates askpass env for cached password prompts", () =>
    Effect.gen(function* () {
      ...
      const directory = yield* fs.makeTempDirectoryScoped({ prefix: "notcodex-ssh-askpass-test-" });
      ...
      assert.equal(env.NOT_CODEX_SSH_AUTH_SECRET, "super-secret");
      assert.equal(env.DISPLAY, "notcodex");
  ...
  it.effect("builds a windows askpass launcher pair", () =>
    Effect.gen(function* () {
      ...
      assert.equal(descriptor.launcherPath, "C:\\temp\\notcodex-ssh-askpass\\ssh-askpass.cmd");
      assert.deepEqual(
