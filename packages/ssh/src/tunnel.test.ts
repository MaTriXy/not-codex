...
import {
  ...
  buildRemoteNotCodexRunnerScript,
...
describe("ssh tunnel scripts", () => {
  it("builds the remote notcodex runner with npx and npm fallbacks", () => {
    const script = buildRemoteNotCodexRunnerScript({ nodeEngineRange: TEST_NODE_ENGINE_RANGE });

    assert.include(script, "NOT_CODEX_NODE_SCRIPT_PATH=''");
    assert.include(script, 'exec notcodex "$@"');
    assert.include(script, "exec npx --yes 'notcodex@latest' \"$@\"");
    assert.include(script, "exec npm exec --yes 'notcodex@latest' -- \"$@\"");
    assert.include(script, "could not install 'notcodex@latest'");
    assert.include(script, 'prepend_path_if_dir "$HOME/.local/bin"');
    assert.include(script, `NOT_CODEX_NODE_ENGINE_RANGE='${TEST_NODE_ENGINE_RANGE}'`);
    ...
    assert.include(script, 'for NOT_CODEX_NODE_BIN in "$NVM_DIR"/versions/node/*/bin');
    assert.notInclude(script, "ensure $NVM_DIR/nvm.sh is available");
  });

  it("does not hard-code a remote node engine range", () => {
    const script = buildRemoteNotCodexRunnerScript();

    assert.include(script, "NOT_CODEX_NODE_ENGINE_RANGE=''");
    assert.notInclude(script, TEST_NODE_ENGINE_RANGE);
  });

  it("shell-quotes package specs in the remote notcodex runner", () => {
    const script = buildRemoteNotCodexRunnerScript({
      packageSpec: "notcodex@nightly; touch /tmp/notcodex-owned",
    });

    assert.include(script, "exec npx --yes 'notcodex@nightly; touch /tmp/notcodex-owned' \"$@\"");
    assert.include(
      script,
      "exec npm exec --yes 'notcodex@nightly; touch /tmp/notcodex-owned' -- \"$@\"",
    );
    assert.notInclude(script, "exec npx --yes notcodex@nightly; touch /tmp/notcodex-owned");
  });

  it("builds the remote notcodex runner with a node script override", () => {
    const script = buildRemoteNotCodexRunnerScript({
      nodeScriptPath: "/Users/collaborator/Development/example-repo/apps/server/dist/bin.mjs",
    });

    assert.include(
      script,
      "NOT_CODEX_NODE_SCRIPT_PATH='/Users/collaborator/Development/example-repo/apps/server/dist/bin.mjs'",
    );
    assert.include(script, 'exec node "$NOT_CODEX_NODE_SCRIPT_PATH" "$@"');
  });

  it("uses the remote notcodex runner for launch and pairing scripts", () => {
    const target = {
      alias: "devbox",
    ...
    assert.include(
      buildRemoteLaunchScript({ nodeEngineRange: TEST_NODE_ENGINE_RANGE }),
      `NOT_CODEX_NODE_ENGINE_RANGE='${TEST_NODE_ENGINE_RANGE}'`,
    );
    assert.include(
      buildRemoteLaunchScript({ nodeEngineRange: TEST_NODE_ENGINE_RANGE }),
    ...
    assert.include(buildRemoteLaunchScript(), "Remote Not Codex server did not become ready");
    assert.include(
      buildRemoteLaunchScript({ packageSpec: "notcodex@nightly" }),
      "notcodex@nightly",
    );
    assert.include(
    ...
    assert.include(
      buildRemotePairingScript(target, { packageSpec: "notcodex@nightly" }),
      "notcodex@nightly",
    );
    assert.include(
