import { AuthStandardClientScopes, EnvironmentId } from "@notcodex/contracts";
...
function pairingHttpLayer(
  ...
  const fetchFn = ((input, init = {}) => {
    ...
    if (url.endsWith("/.well-known/notcodex/environment")) {
  ...
  it.effect("prepares a persisted bearer registration from pairing details", () =>
    Effect.gen(function* () {
      ...
      expect(calls.map((call) => call.url)).toEqual([
        "https://remote.example.test/.well-known/notcodex/environment",
      ...
      expect(tokenParams.get("client_label")).toBe("Not Codex Test");
    }),
  );
