  ...
  yield* Effect.forkScoped(
    subscribeDynamic(
      ...
      Effect.fn("EnvironmentShellState.makeSubscribeInput")(function* (session) {
        ...
        // Foreground resubscriptions on the same live session resume from the
        // in-memory cursor. A replacement session reloads the authoritative
        // HTTP snapshot so a valid cursor cannot preserve incomplete cache.
        ...
        return { afterSequence: current.snapshot.value.snapshotSequence };
      }),
      {
