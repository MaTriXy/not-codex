import { EnvironmentId, type EnvironmentId as EnvironmentIdType } from "@notcodex/contracts";
          ...
          case "connected":
            return Option.isSome(session)
              ? runInEnvironment(target.environmentId, options.execute(target.input)).pipe(
                  // Session replacement can race an already-running request.
                  // Keep the query pending until the connection dependency
                  // advances instead of exposing that transient gap as data.
                  Effect.catchIf(Schema.is(EnvironmentRpcUnavailableError), () => Effect.never),
                )
              : Effect.never;
          case "connecting":
          case "backoff":
