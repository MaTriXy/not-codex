...
This guide is for people who want to use more than one Claude setup in Not Codex.
...
In Not Codex Settings, your Claude provider can stay like this:
...
Not Codex looks for Claude skills in the Claude config directory's `skills` folder, then
...
Not Codex only offers Claude providers that use the same Claude home for an existing thread. A
...
multiple files under its home directory, so Not Codex keeps separate Claude homes isolated instead of
...
Add or edit a Claude provider in Not Codex Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. Not Codex stores the value as a server secret and does not
...
Not Codex does not need a special Claude Code Router provider. Treat the router as a Claude
environment.
...
2. Add or configure a Claude provider in Not Codex.
