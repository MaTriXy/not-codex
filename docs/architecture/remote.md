...
This document describes the target architecture for first-class remote environments in Not Codex.
...
- Keep the Not Codex server as the execution boundary.
...
Not Codex already has a clean runtime boundary: the client talks to a Not Codex server over HTTP/WebSocket, and the server owns orchestration, providers, terminals, git, and filesystem operations.
...
└───────────────┬──────────────────────────────┘
                │
                │ connects to one Not Codex server
                │
┌───────────────▼──────────────────────────────┐
│ Execution environment = one Not Codex server       │
│                                              │
...
The important decision is that remoteness is expressed at the environment connection layer, not by splitting the Not Codex runtime itself.

## Domain model

### ExecutionEnvironment

An `ExecutionEnvironment` is one running Not Codex server instance.
...
How does the client speak WebSocket to a Not Codex server?
...
- best fit for hosted or self-managed remote Not Codex deployments
...
For Not Codex, tunnels are best modeled as another `AccessEndpoint`, not as a different kind of environment.
...
Tailscale-backed access sits here architecturally even though the current implementation is endpoint discovery rather than a Not Codex-managed tunnel. It contributes private-network endpoints and lets the existing HTTP/WebSocket client path do the actual connection.
...
- reach a machine
- probe it
- launch or reuse a remote Not Codex server
...
How does a Not Codex server come to exist on the target machine?
...
The user or operator already runs Not Codex on the target machine, and the client connects through a direct or tunneled WebSocket endpoint.
...
What should be different in Not Codex:
...
The recommended Not Codex flow is:

1. Desktop connects over SSH.
2. Desktop probes the remote machine and verifies Not Codex availability.
3. Desktop launches or reuses a remote Not Codex server.
...
This is the inverse of remote launch: a local Not Codex server is already running, and the client publishes it through a tunnel.
...
- A manually hosted Not Codex server might be reached through direct `wss`.
...
Not Codex already supports a WebSocket auth token on the server. That should become a first-class part of environment access rather than remaining an incidental query parameter convention.
...
Not Codex should not copy that part.

Not Codex already has the right runtime boundary:

- one Not Codex server per environment
- ordinary HTTP/WebSocket between client and environment

So Not Codex should borrow Zed's launch discipline, not its transport protocol.
