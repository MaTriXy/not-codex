...
Not Codex runs a **local Node.js environment server** that coordinates configured coding-agent
providers, durable orchestration, repository workflows, and connected web, desktop, and mobile clients.
...
- **Provider runtime**: Provider adapters start and supervise the configured CLI/runtime, then translate
  its native sessions and events into the canonical Not Codex orchestration model. `ProviderService` is
  the provider-neutral boundary; provider-native shapes remain inside adapters.
...
- **Automations**: Durable definitions, schedules, leases, and run events coordinate ordinary orchestration threads. The executor uses the same provider, VCS, approval, checkpoint, and projection boundaries as interactive work. See [Automations architecture](./automations.md).
...
sequenceDiagram
    ...
    participant Runtime as selected provider runtime
    ...
    Provider->>Runtime: Provider-specific protocol
    Runtime-->>Ingest: Canonical provider runtime events
...
3. [`ProviderService`][8] resolves the configured provider instance and starts or resumes a session
   through its adapter.
