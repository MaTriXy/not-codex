...
Not Codex connects directly to your Git hosting provider so you can create pull requests, review code, and manage repositories without leaving your editor. Work stays in flow—no more jumping between browser tabs and terminal windows.

## Supported Providers

Not Codex works with the platforms your team already uses:
...
- Not Codex can suggest titles and descriptions based on your commits
...
- Command-click (Control-click on Windows and Linux) a pull request number in the sidebar to open it in your browser instead of in Not Codex
...
1. Install the GitHub CLI on the machine running Not Codex:
   ```bash
   brew install gh
   ```
2. Sign in:
   ...
   gh auth login
   ```
3. Open **Settings → Source Control** in Not Codex and verify GitHub shows as authenticated
...
2. Add these environment variables to the environment running Not Codex:
   ```bash
   export NOT_CODEX_BITBUCKET_EMAIL="you@example.com"
   export NOT_CODEX_BITBUCKET_API_TOKEN="your-token"
   ```
3. Restart Not Codex and verify the connection in **Source Control settings**
...
**Git is required** – Not Codex uses Git for all local operations. Ensure `git` is installed on your server.

**Server-side setup** – Authentication happens on the machine running Not Codex (the server), not your local browser. If you're using a hosted or team instance, your administrator may have already configured providers.
