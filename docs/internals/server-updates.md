...
> For maintainers. Using Not Codex? See [docs/user](../user/).
...
The launcher is the only runtime writer of `service-state.json`. `notcodex service install` and
`notcodex service update` may replace the launcher and state while the unit is stopped. Server children
...
1. The active server installs `notcodex@<target>` into a unique staging directory.
...
```sh
npx notcodex@<version> service update
```
