# Running Not Codex in the Background

On Linux and macOS, Not Codex can run as a background service for your user, so it is ready without
keeping a terminal open.

## Manage the Service

Install it with the latest Not Codex release:

```sh
npx notcodex@latest service install
```

Check whether it is installed:

```sh
npx notcodex@latest service status
```
...
```sh
npx notcodex@latest service uninstall
```

Updating restarts Not Codex briefly. Let active agent work and terminal commands finish first.
If a remote update is already in progress, wait for it to finish before retrying a local update.

The service runs a small stable launcher. Exact Not Codex versions are installed separately, so a
...
**Linux** uses a systemd user unit at `~/.config/systemd/user/notcodex.service`. The service starts
when the machine boots and keeps running after you log out (lingering is enabled during install).

**macOS** uses a launch agent at `~/Library/LaunchAgents/com.notcodex.notcodex.service.plist`. It
...
## Using It with Not Codex Connect

Not Codex Connect may offer to install the service during setup so the host stays reachable in the
background. This is only an onboarding shortcut: the service and Not Codex Connect are managed separately.

Signing out of Not Codex Connect does not remove the service. Use `notcodex service uninstall` when you no longer
want Not Codex to start in the background.
