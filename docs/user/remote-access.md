...
Use this when you want to connect to a Not Codex server from another device such as a phone, tablet, or separate desktop app.
...
- HTTPS/WSS-compatible endpoints can work from a separately deployed hosted app, but are not made the default
  automatically.
...
If the copied link points directly at `http://192.168.x.y:3773`, open it from a client that can reach that LAN address. A hosted pairing link is generated only when the deployment explicitly configures `VITE_HOSTED_APP_URL`; the reserved public hostname is not live yet.
...
with the same server-side behavior as `notcodex serve --tailscale-serve`, then the server asks Tailscale
...
For a hosted HTTPS app, prefer an HTTPS Tailnet or other HTTPS endpoint. A plain `http://100.x.y.z:3773` endpoint can still work from a desktop client or another browser page served over HTTP, but it will not work from a hosted HTTPS app because of browser mixed-content rules.
...
Build the source, then run the server with `vp run start -- serve`.

```bash
vp run start -- serve --host "$(tailscale ip -4)"
```

The `serve` command starts the server without opening a browser and prints:
...
Use `vp run start -- serve --help` for the full flag reference. It supports the same general startup options as the normal server command, including an optional `cwd` argument.

For hosted web pairing over Tailscale HTTPS, opt in to Tailscale Serve:

```bash
vp run start -- serve --tailscale-serve
```
...
Use this when you want the desktop app to start or reuse Not Codex on another machine over SSH.
...
5. Confirm the launch. The desktop app probes the host, starts or reuses a remote Not Codex server, opens a local port forward, and saves the environment.

After setup, the renderer connects to a local forwarded HTTP/WebSocket endpoint. The remote host still owns the actual Not Codex server, projects, files, git state, terminals, and provider sessions.
...
The desktop SSH launcher connects with a non-interactive `sh` session, writes a small launcher script under `~/.notcodex/ssh-launch/<host-key>/`, starts or reuses a remote Not Codex server, and forwards the remote loopback port back to your desktop.

The remote host must have a compatible Node.js runtime. Not Codex uses the server package's `engines.node` requirement:

```text
^22.16 || ^23.11 || >=24.10
```

During SSH launch, Not Codex first checks whether `node` is already available on `PATH`. If it is missing, the launcher tries common non-interactive shell locations and version-manager shims/activation hooks:
...
If launch fails with `node: command not found`, a port-scan failure, or a message that the remote Node version does not satisfy the required range, SSH into the host and check the same non-interactive shell path Not Codex uses:
...
1. `notcodex serve` issues a one-time owner pairing token.
...
When an operator explicitly deploys and configures a hosted web app, it can save a remote backend in browser storage from a URL like:
...
Hosted pairing does not proxy traffic through Not Codex. The browser still connects directly to the backend URL in the pairing link.
...
Use `notcodex auth --help` and the nested subcommand help pages for the full reference.

### Deregister a Not Codex Connect Environment

Open your account menu and choose **Not Codex Connect** to see every environment registered to your
account. On mobile, open **Settings** → **Not Codex Connect**. Choose **Deregister** to revoke an
environment's Not Codex Connect access, remove any managed tunnel, and free its host space.
...
- Use `notcodex auth` to revoke credentials or sessions you no longer trust.
