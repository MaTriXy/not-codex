# Not Codex Connect Clerk Setup

Not Codex Connect uses one Clerk application for web, desktop, and mobile authentication. The relay accepts
Clerk JWTs only when they are generated from the `notcodex-relay` template with the shared
`notcodex-relay` audience.

## Application Keys

Not Codex Connect is disabled in a fresh clone. To enable it for source builds, add a repository-root `.env`
or `.env.local` file:

```dotenv
NOT_CODEX_CLERK_PUBLISHABLE_KEY=<publishable key>
NOT_CODEX_CLERK_JWT_TEMPLATE=<JWT template name>
NOT_CODEX_CLERK_CLI_OAUTH_CLIENT_ID=<public OAuth application client ID>
NOT_CODEX_RELAY_URL=https://relay.example.com
```
...
should set `NOT_CODEX_CLERK_PUBLISHABLE_KEY`, `NOT_CODEX_CLERK_JWT_TEMPLATE`,
`NOT_CODEX_CLERK_CLI_OAUTH_CLIENT_ID`, and `NOT_CODEX_RELAY_URL` before building. EAS preview and
...
absent, the `notcodex connect` CLI command group is omitted. The bundled server still accepts runtime
...
deployment defaults.
`vp run --filter notcodex-relay deploy` invokes Alchemy from the relay directory, so Alchemy loads
...
The `notcodex connect` commands authorize a headless environment with a separate Clerk OAuth application.
...
```sh
notcodex connect login
notcodex connect link
notcodex connect status
notcodex connect unlink
notcodex connect logout
notcodex serve
```

`notcodex connect login` opens the Clerk authorization flow and stores the CLI credential without enabling
cloud exposure. `notcodex connect link` installs the pinned managed `cloudflared` binary when needed,
authorizes when needed, and records durable intent to expose the environment. It works without a
running Not Codex server. The next `notcodex serve` or `notcodex start` reconciles the relay link and launches the
managed tunnel. `notcodex connect unlink` records disabled intent immediately, stops a reachable running
connector, and attempts to revoke the relay-side environment record. It retains the stored CLI
authorization so `notcodex connect link` can re-enable exposure without another browser flow. `notcodex connect
...
forward that port before running `notcodex connect login` or `notcodex connect link`:
...
| ------- | ----------------------------- |
| Name    | `notcodex-relay`              |
| Claims  | `{ "aud": "notcodex-relay" }` |

Set `NOT_CODEX_CLERK_JWT_TEMPLATE=notcodex-relay` in the repository-root `.env`, and set
`CLERK_JWT_AUDIENCE=notcodex-relay` in `infra/relay/.env`. Define `CLERK_JWT_TEMPLATE` and
`CLERK_JWT_AUDIENCE` in the production relay deployment environment as well. The stable `aud` value
is shared by production and non-production relay stages. The client-facing `NOT_CODEX_RELAY_URL` still
...
Local desktop development uses `notcodex-dev://app`, while packaged builds use `notcodex://app`. Add the
matching origin to each Clerk instance's Backend API `allowed_origins` array as well. The development
Clerk instance should only need `notcodex-dev://app`; the production Clerk instance should only need
`notcodex://app`. `@clerk/electron` owns the native request adapter, encrypted Clerk token persistence,
...
curl -X PATCH https://api.clerk.com/v1/instance \
  ...
  -d '{"allowed_origins":["notcodex://app"]}'
```
...
The production macOS bundle ID is `com.notcodex.notcodex`. To enable native passkeys:

1. Create an explicit macOS App ID for `com.notcodex.notcodex` in the Apple Developer portal and enable
   **Associated Domains**.
...
4. Confirm Clerk serves `https://<frontend-api>/.well-known/apple-app-site-association` and that
   `webcredentials.apps` contains `<TEAM_ID>.com.notcodex.notcodex`.
...
```dotenv
NOT_CODEX_APPLE_TEAM_ID=ABC1234567
NOT_CODEX_MACOS_PROVISIONING_PROFILE=/absolute/path/to/notcodex.provisionprofile
# Optional: comma-separated override when Clerk's RP ID differs from the Frontend API hostname.
NOT_CODEX_CLERK_PASSKEY_RP_DOMAINS=example.clerk.accounts.dev,clerk.example.com
```

When `NOT_CODEX_CLERK_PASSKEY_RP_DOMAINS` is absent, the build derives the RP domain from
`NOT_CODEX_CLERK_PUBLISHABLE_KEY`. Signed macOS builds fail early if the Team ID, provisioning profile,
...
installed app executable with `VITE_DEV_SERVER_URL` and `NOT_CODEX_PORT` set. Rebuild the signed app
...
NOT_CODEX_PORT=13773 \
  "/Applications/Not Codex (Alpha).app/Contents/MacOS/Not Codex (Alpha)"
```
...
```sh
codesign --verify --deep --strict "/Applications/Not Codex (Alpha).app"
codesign -d --entitlements :- "/Applications/Not Codex (Alpha).app"
```
...
Approved signed-in users manage Not Codex Connect under **Connections**. The web and desktop sidebars do
not expose a dedicated account or waitlist control. Signed-out users reach Clerk's waitlist and
sign-in flow contextually from the Not Codex Connect controls on the Connections page.

On mobile, signed-out users open **Settings > Not Codex Account** to reach `/settings/waitlist` within the
