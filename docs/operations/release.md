...
- Reads the shared production Not Codex Connect relay URL and Clerk client configuration before packaging clients.
...
- Publishes the CLI package (`apps/server`, npm package `notcodex`) with OIDC trusted publishing from the same workflow file:
...
```sh
vp run --filter notcodex-relay deploy -- --stage "$USER" --env-file .env.local
```
...
through `/__notcodex/channel` on the router domain so the user's channel cookie is
...
- Repository slug source:
  - `NOT_CODEX_DESKTOP_UPDATE_REPOSITORY` (format `owner/repo`), if set.
  - otherwise `GITHUB_REPOSITORY` from GitHub Actions.
- Temporary private-repo auth workaround:
  - set `NOT_CODEX_DESKTOP_UPDATE_GITHUB_TOKEN` (or `GH_TOKEN`) in the desktop app runtime environment.
...
1. Confirm npm org/user owns package `notcodex` (or rename package first if needed).
...
2. Create an explicit App ID for `com.notcodex.notcodex` and enable Associated Domains.
...
- macOS build unsigned when expected signed:
  ...
  - Confirm the provisioning profile belongs to `APPLE_TEAM_ID.com.notcodex.notcodex` and includes
    Associated Domains.
