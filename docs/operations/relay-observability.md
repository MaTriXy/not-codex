...
- `notcodex-relay-traces-prod`, an OpenTelemetry trace dataset for Worker requests
- `notcodex-relay-otel-ingest-prod`, a dataset-scoped ingest token bound to the Worker
- `notcodex-relay-recent-spans-prod`, a view of recent request and endpoint spans

Alchemy stages append their sanitized stage name to isolate resources, for example
`notcodex-relay-traces-dev-collaborator` for a personal stage.
