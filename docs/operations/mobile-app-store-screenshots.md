...
The screenshot harness runs the real mobile application against three disposable local Not Codex
...
the software keyboard, and supplies deterministic Not Codex Connect discovery rows to the real
...
1. Creates three temporary Not Codex base directories and starts a local server for each on an available
   port.
2. Creates Not Codex, React, and Linux Git repositories with recognizable favicons, feature branches,
   and a deterministic Not Codex review diff.
...
ARM64 by default for local Apple Silicon development; CI sets `NOT_CODEX_SHOWCASE_ANDROID_ABI=x86_64` so the
...
Run Metro separately:
    ...
    pnpm --filter @notcodex/mobile showcase
...
The Pending rows use the production offline outbox and point at the real Not Codex and React fixture
