...
Not Codex has one server-side observability model:
...
Completed spans are written as NDJSON records to `serverTracePath` (by default, `~/.notcodex/userdata/logs/server.trace.ndjson`).
...
```bash
export NOT_CODEX_OTLP_TRACES_URL=http://localhost:4318/v1/traces
export NOT_CODEX_OTLP_METRICS_URL=http://localhost:4318/v1/metrics
export NOT_CODEX_OTLP_SERVICE_NAME=notcodex-local
```

Optional:

```bash
export NOT_CODEX_TRACE_MIN_LEVEL=Info
export NOT_CODEX_TRACE_TIMING_ENABLED=true
```
...
Launch the actual app executable from the same shell so the desktop app and embedded backend inherit `NOT_CODEX_OTLP_*`.

macOS app bundle example:

```bash
NOT_CODEX_OTLP_TRACES_URL=http://localhost:4318/v1/traces \
NOT_CODEX_OTLP_METRICS_URL=http://localhost:4318/v1/metrics \
NOT_CODEX_OTLP_SERVICE_NAME=notcodex-desktop \
"/Applications/Not Codex.app/Contents/MacOS/Not Codex"
```
...
```bash
tail -f "$NOT_CODEX_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -c 'select(.exit._tag != "Success") | {
  ...
  exit,
  attributes
}' "$NOT_CODEX_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -r 'select(.traceId == "TRACE_ID_HERE") | [
  ...
  (.parentSpanId // "-"),
  .durationMs
] | @tsv' "$NOT_CODEX_HOME/userdata/logs/server.trace.ndjson"
```
...
- service name such as `notcodex-local`, `notcodex-dev`, or `notcodex-desktop`
...
- `notcodex_rpc_request_duration`
- `notcodex_orchestration_command_duration`
- `notcodex_orchestration_command_ack_duration`
- `notcodex_provider_turn_duration`
- `notcodex_git_command_duration`
- `notcodex_db_query_duration`

Counters tell you volume and failure rate:

- `notcodex_rpc_requests_total`
- `notcodex_orchestration_commands_total`
- `notcodex_provider_turns_total`
- `notcodex_git_commands_total`
- `notcodex_db_queries_total`
...
`notcodex_orchestration_command_ack_duration` measures:
...
1. Check `notcodex_orchestration_command_ack_duration` by `commandType`.
...
- `NOT_CODEX_OTLP_TRACES_URL` was not set
...
- `NOT_CODEX_TRACE_FILE`: override trace file path
- `NOT_CODEX_TRACE_MAX_BYTES`: per-file rotation size, default `10485760`
- `NOT_CODEX_TRACE_MAX_FILES`: rotated file count, default `10`
- `NOT_CODEX_TRACE_BATCH_WINDOW_MS`: flush window, default `200`
- `NOT_CODEX_TRACE_MIN_LEVEL`: minimum trace level, default `Info`
- `NOT_CODEX_TRACE_TIMING_ENABLED`: enable timing metadata, default `true`

OTLP export:

- `NOT_CODEX_OTLP_TRACES_URL`: OTLP trace endpoint
- `NOT_CODEX_OTLP_METRICS_URL`: OTLP metric endpoint
- `NOT_CODEX_OTLP_EXPORT_INTERVAL_MS`: export interval, default `10000`
- `NOT_CODEX_OTLP_SERVICE_NAME`: service name, default `notcodex-server`
