...
#import "NotCodexMarkdownTextShadowNode.h"
#import "NotCodexMarkdownTextComponentDescriptor.h"
...
static void NotCodexMarkdownTextApplyParagraphStyles(
    NSMutableAttributedString *attributedString,
    const std::vector<NotCodexMarkdownTextParagraphStyleRange> &styleRanges)
{
...
static void NotCodexMarkdownTextApplyAttachments(
    NSMutableAttributedString *attributedString,
    const std::vector<NotCodexMarkdownTextAttachmentRange> &attachmentRanges,
    NSDictionary<NSString *, UIImage *> *images)
{
  for (const auto &attachmentRange : attachmentRanges) {
    ...
    const CGFloat attachmentSize = NotCodexMarkdownTextAttachmentSize(attachmentRange);
    attachment.bounds = CGRectMake(
        0,
        NotCodexMarkdownTextAttachmentBaselineOffset(attachmentRange),
        attachmentSize,
        attachmentSize);
...
@protocol NotCodexMarkdownOutsideTapTarget <NSObject>
- (void)clearSelectionForOutsideTapWithHitView:(UIView *)hitView;
@end

@interface NotCodexMarkdownOutsideTapCoordinator : NSObject <UIGestureRecognizerDelegate>

- (instancetype)initWithWindow:(UIWindow *)window;
- (void)addTarget:(id<NotCodexMarkdownOutsideTapTarget>)target;
- (void)removeTarget:(id<NotCodexMarkdownOutsideTapTarget>)target;

@end

static const void *NotCodexMarkdownOutsideTapCoordinatorKey =
    &NotCodexMarkdownOutsideTapCoordinatorKey;

@implementation NotCodexMarkdownOutsideTapCoordinator {
  __weak UIWindow *_window;
  UITapGestureRecognizer *_recognizer;
  NSHashTable<id<NotCodexMarkdownOutsideTapTarget>> *_targets;
}
...
- (void)removeTarget:(id<NotCodexMarkdownOutsideTapTarget>)target
{
  ...
  if (objc_getAssociatedObject(window, NotCodexMarkdownOutsideTapCoordinatorKey) == self) {
...
- (void)handleTap:(UITapGestureRecognizer *)sender
{
  ...
  for (id<NotCodexMarkdownOutsideTapTarget> target in _targets.allObjects) {
...
static NotCodexMarkdownOutsideTapCoordinator *
NotCodexMarkdownOutsideTapCoordinatorForWindow(UIWindow *window)
{
  NotCodexMarkdownOutsideTapCoordinator *coordinator =
      objc_getAssociatedObject(window, NotCodexMarkdownOutsideTapCoordinatorKey);
  if (coordinator == nil) {
    coordinator = [[NotCodexMarkdownOutsideTapCoordinator alloc] initWithWindow:window];
    objc_setAssociatedObject(
        window,
        NotCodexMarkdownOutsideTapCoordinatorKey,
        coordinator,
...
@interface NotCodexMarkdownText () <RCTNotCodexMarkdownTextViewProtocol, UIGestureRecognizerDelegate, UITextViewDelegate>

@end

@interface NotCodexMarkdownText () <NotCodexMarkdownOutsideTapTarget>
@end

@implementation NotCodexMarkdownText {
  UIView * _view;
  UITextView * _textView;
  NotCodexMarkdownTextShadowNode::ConcreteState::Shared _state;
...
{
  return concreteComponentDescriptorProvider<NotCodexMarkdownTextComponentDescriptor>();
}

- (instancetype)initWithFrame:(CGRect)frame
{
  if (self = [super initWithFrame:frame]) {
    static const auto defaultProps = std::make_shared<const NotCodexMarkdownTextProps>();
    _props = defaultProps;
...
- (void)didMoveToWindow
{
  ...
  if (_outsideTapWindow != nil) {
    NotCodexMarkdownOutsideTapCoordinator *coordinator =
        objc_getAssociatedObject(_outsideTapWindow, NotCodexMarkdownOutsideTapCoordinatorKey);
    [coordinator removeTarget:self];
  }
  _outsideTapWindow = self.window;
  if (_outsideTapWindow != nil) {
    [NotCodexMarkdownOutsideTapCoordinatorForWindow(_outsideTapWindow) addTarget:self];
  }
}
...
- (void)drawRect:(CGRect)rect
{
  ...
  const auto &props = *std::static_pointer_cast<NotCodexMarkdownTextProps const>(_props);
  ...
  NotCodexMarkdownTextApplyParagraphStyles(
      convertedAttrString,
      _state->getData().paragraphStyleRanges);
  NotCodexMarkdownTextApplyAttachments(
      convertedAttrString,
  ...
  if (_eventEmitter != nullptr) {
    std::dynamic_pointer_cast<const facebook::react::NotCodexMarkdownTextEventEmitter>(_eventEmitter)
    ->onTextLayout(facebook::react::NotCodexMarkdownTextEventEmitter::OnTextLayout{static_cast<int>(self.tag), lines});
  };
}

- (void)loadAttachmentImages:(const std::vector<NotCodexMarkdownTextAttachmentRange> &)attachmentRanges
{
  for (const auto &attachmentRange : attachmentRanges) {
    NSString *imageUri = [NSString stringWithUTF8String:attachmentRange.imageUri.c_str()];
    if ([imageUri hasPrefix:@"sf:"]) {
...
{
  const auto &oldViewProps = *std::static_pointer_cast<NotCodexMarkdownTextProps const>(_props);
  const auto &newViewProps = *std::static_pointer_cast<NotCodexMarkdownTextProps const>(props);
  ...
  if (oldViewProps.ellipsizeMode != newViewProps.ellipsizeMode) {
    if (newViewProps.ellipsizeMode == NotCodexMarkdownTextEllipsizeMode::Head) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingHead;
    } else if (newViewProps.ellipsizeMode == NotCodexMarkdownTextEllipsizeMode::Middle) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingMiddle;
    } else if (newViewProps.ellipsizeMode == NotCodexMarkdownTextEllipsizeMode::Tail) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingTail;
    } else if (newViewProps.ellipsizeMode == NotCodexMarkdownTextEllipsizeMode::Clip) {
...
{
  _state = std::static_pointer_cast<const NotCodexMarkdownTextShadowNode::ConcreteState>(state);
  [self setNeedsDisplay];
}
...
- (NotCodexMarkdownTextRun*)getTouchChild:(CGPoint)location
{
  ...
  for (UIView* child in self.subviews) {
    if (![child isKindOfClass:[NotCodexMarkdownTextRun class]]) {
      continue;
    }

    NotCodexMarkdownTextRun* textChild = (NotCodexMarkdownTextRun*)child;
...
- (void)textViewDidChangeSelection:(UITextView *)textView
{
  ...
  std::dynamic_pointer_cast<const facebook::react::NotCodexMarkdownTextEventEmitter>(_eventEmitter)
    ->onSelectionChange(facebook::react::NotCodexMarkdownTextEventEmitter::OnSelectionChange{
...
Class<RCTComponentViewProtocol> NotCodexMarkdownTextCls(void)
{
  return NotCodexMarkdownText.class;
}
