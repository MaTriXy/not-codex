...
export function NativeMarkdownSelectableText(props: {
  ...
  // NotCodexMarkdownText only rebuilds its attributed string during native layout. A
      ...
      {keyedRuns.map(({ key, run, text }) => {
        ...
        return (
          <MarkdownTextPrimitive
            ...
            nativeID={
              run.fileIcon
                ? `notcodex-file:${Image.resolveAssetSource(markdownFileIconSource(run.fileIcon)).uri}`
                : run.skillName
                  ? "notcodex-skill:sf:cube"
                  : undefined
            }
