...
public final class NotCodexKeyboardCommandsModule: Module {
  public func definition() -> ModuleDefinition {
    Name("NotCodexKeyboardCommands")

    View(NotCodexKeyboardCommandsView.self) {
      Prop("enabledCommands") { (view: NotCodexKeyboardCommandsView, commands: [String]) in
...
public final class NotCodexKeyboardCommandsView: ExpoView {
  ...
  @objc private func reclaimFirstResponderIfAvailable() {
    DispatchQueue.main.async { [weak self] in
      guard let self, self.window?.notCodexFirstResponder == nil else { return }
...
private extension UIView {
  var notCodexFirstResponder: UIResponder? {
    if isFirstResponder { return self }
    for subview in subviews {
      if let responder = subview.notCodexFirstResponder { return responder }
    }
    return nil
  }
}
