...
// Ships the branded Not Codex mark to the Live Activity / widget extension.
//
...
const IMAGE_SET = "NotCodexMark.imageset";
const IMAGE_NAME = "NotCodexMark.png";

const CATALOG_CONTENTS = JSON.stringify({ info: { author: "expo", version: 1 } }, null, 2) + "\n";
const IMAGE_SET_CONTENTS =
  JSON.stringify(
    {
      images: [{ idiom: "universal", filename: IMAGE_NAME, scale: "1x" }],
...
function withAssetFiles(config) {
  return withDangerousMod(config, [
    "ios",
    (cfg) => {
      const source = path.join(cfg.modRequest.projectRoot, "assets", "widget", IMAGE_NAME);
