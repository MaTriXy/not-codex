...
export function CloudEnvironmentRows(props: {
  ...
  /**
   * Hide the "Not Codex Connect" section title + refresh button for hosts that
    ...
    <View collapsable={false} className={cn("gap-3", showHeader && "mt-5")}>
      ...
      {controller.relayDiscovery.error && !controller.relayDiscovery.isRefreshing ? (
        <View collapsable={false} className="gap-3 rounded-[24px] bg-card p-5">
          ...
          >
            <Text className="text-xs font-notcodex-bold text-foreground">Try again</Text>
          </Pressable>
        </View>
      ) : null}
    </View>
    ...
    >
      <View className="min-w-0 flex-1 gap-0.5">
        <View className="min-w-0 flex-row items-center gap-2">
          ...
          <Text
            className="min-w-0 flex-shrink text-base font-notcodex-bold leading-snug text-foreground"
            numberOfLines={1}
          >
            {props.label}
          </Text>
...
function CopyTraceIdButton(props: { readonly traceId: string }) {
  ...
  return (
    ...
    >
      ...
      <Text className="text-xs font-notcodex-bold text-foreground">Copy trace ID</Text>
    </Pressable>
  );
}
