...
describe("buildHomeThreadGroups", () => {
  ...
  it("matches web repository, repository-path, and separate grouping modes", () => {
    ...
    const projects = [
      makeProject({
        ...
        title: "Web",
        workspaceRoot: "/workspaces/notcodex/apps/web",
        repositoryIdentity,
      }),
      makeProject({
        environmentId,
        ...
        title: "Mobile",
        workspaceRoot: "/workspaces/notcodex/apps/mobile",
        repositoryIdentity,
      }),
    ];
    const threads = projects.map((project) =>
  ...
  it("does not apply the recency window while searching", () => {
    ...
    const group = buildGroups([project], threads, { searchQuery: "Not Codex" })[0];
  ...
  it("targets quick new threads at the group member with the newest thread", () => {
    ...
    const repositoryIdentity = {
      ...
      locator: {
        source: "git-remote" as const,
        remoteName: "origin",
        remoteUrl: "git@github.com:MaTriXy/not-codex.git",
      },
    };
