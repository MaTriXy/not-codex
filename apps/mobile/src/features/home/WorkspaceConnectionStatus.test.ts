...
describe("workspace connection status", () => {
  ...
  it("names the environment while reconnecting", () => {
      ...
      connectingEnvironments: [
        {
          ...
          environmentLabel: "Collaborator’s Mac mini",
          displayUrl: "",
    ...
    expect(shouldShowWorkspaceConnectionStatus(state)).toBe(true);
    expect(workspaceConnectionStatusLabel(state)).toBe("Reconnecting to Collaborator’s Mac mini");
  });

  it("surfaces connection errors before the generic disconnected fallback", () => {
    const state = workspaceState({
      connectionError: "Could not reach Collaborator’s Mac mini",
      hasLoadedShellSnapshot: false,
      hasReadyEnvironment: false,
    });

    expect(shouldShowWorkspaceConnectionStatus(state)).toBe(true);
    expect(workspaceConnectionStatusLabel(state)).toBe("Could not reach Collaborator’s Mac mini");
  });
});
