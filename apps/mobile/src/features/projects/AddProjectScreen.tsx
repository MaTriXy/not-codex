...
} from "@notcodex/client-runtime/operations/projects";
import {
...
import { CommandId, type EnvironmentId, ProjectId } from "@notcodex/contracts";
...
function SectionTitle(props: { readonly children: string }) {
  return (
    <Text className="px-1 text-2xs font-notcodex-bold tracking-[0.7px] uppercase text-foreground-muted">
      {props.children}
    </Text>
  );
}
    ...
    >
      <View className="flex-row items-center gap-3">
        ...
        <View className="flex-1 gap-0.5">
          <Text className="text-base leading-snug font-notcodex-bold">{props.title}</Text>
          {props.subtitle ? (
  ...
  return (
    ...
    >
      {props.loading ? (
        ...
        <Text className="text-base font-notcodex-bold text-primary-foreground">{props.label}</Text>
      )}
    </Pressable>
  );
}
...
function EmptyEnvironmentState() {
  ...
  return (
    <View className="items-center gap-3 rounded-2xl bg-card px-5 py-8">
      <Text className="text-center text-lg font-notcodex-bold">No environments connected</Text>
    ...
    <AddProjectShell>
      ...
      {repositoryTitle ? (
        <View className="rounded-[24px] bg-card px-4 py-3">
          <Text className="text-base font-notcodex-bold">{repositoryTitle}</Text>
