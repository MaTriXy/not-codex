import type { ProjectEntry } from "@notcodex/contracts";
...
const FileTreeRow = memo(function FileTreeRow(props: {
  ...
  return (
    ...
    >
      ...
      <Text
        className={cn(
          ...
          props.selected
            ...
            : "font-notcodex-medium text-foreground-secondary",
        )}
        numberOfLines={1}
      >
        {node.name}
      ...
      {node.kind === "directory" ? (
        <Text className="text-2xs font-notcodex-medium text-foreground-tertiary">
  ...
  if (props.error && props.entries.length === 0) {
    return (
      <View className="flex-1 bg-sheet px-4 py-5">
        <Text className="text-sm font-notcodex-bold text-foreground">Files unavailable</Text>
      ...
      ListEmptyComponent={
        <View className="px-4 py-5">
          {props.isPending ? (
            ...
            <>
              <Text className="text-sm font-notcodex-bold text-foreground">No files found</Text>
