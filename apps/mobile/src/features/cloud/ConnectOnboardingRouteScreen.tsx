...
import { reportAtomCommandResult, settlePromise } from "@notcodex/client-runtime/state/runtime";
...
/**
 * Post-sign-in onboarding sheet for Not Codex Connect. Mobile never publishes
 * environments itself — it consumes ones published elsewhere — so this simply
 * surfaces the account's Not Codex Connect environments right after sign-in so every
...
function ConfiguredConnectOnboardingRouteScreen() {
  ...
  return (
    <View collapsable={false} className="flex-1 bg-sheet">
      {Platform.OS === "android" ? (
        <AndroidSheetHeader
          title="Set up Not Codex Connect"
        ...
        {isSignedIn ? (
          ...
          <View collapsable={false} className="rounded-[24px] bg-card p-5">
            <Text className="text-sm leading-normal text-foreground-muted">
              Sign in to your Not Codex account to set up Not Codex Connect.
            </Text>
          </View>
        )}
