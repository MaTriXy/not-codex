...
import type { EnvironmentId, MessageId, ThreadId, TurnId } from "@notcodex/contracts";
import {
  CHAT_LIST_ANCHOR_OFFSET,
  resolveChatListAnchoredEndSpace,
} from "@notcodex/shared/chatList";
...
import { markdownFileIconSource } from "@notcodex/mobile-markdown-text/file-icons";
import { resolveMarkdownLinkPresentation } from "@notcodex/mobile-markdown-text/links";
import {
...
function renderFeedEntry(
  ...
  if (entry.type === "turn-fold") {
    return (
      ...
      >
        <Text className="font-notcodex-medium text-sm tabular-nums text-foreground-muted">
          {entry.label}
        </Text>
        <SymbolView
      ...
      return (
        ...
        >
          ...
          <View className="mt-1 flex-row items-center justify-end gap-1 pr-0.5">
            <Text className="font-notcodex-medium text-xs tabular-nums text-neutral-600 dark:text-neutral-400">
              {timestampLabel}
            </Text>
            {message.text.trim().length > 0 ? (
              <CopyTextButton
  ...
  return (
    ...
    >
      <View className="max-w-[320px] items-center gap-2">
        <Text className="text-center font-notcodex-bold text-lg text-foreground">
          {props.title}
        </Text>
