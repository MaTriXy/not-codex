import type {
  ...
  RuntimeMode,
  ServerConfig as NotCodexServerConfig,
...
} from "@notcodex/shared/composerTrigger";
...
import {
  ...
  scoreQueryMatch,
} from "@notcodex/shared/searchRanking";
...
export interface ThreadComposerProps {
  ...
  readonly serverConfig: NotCodexServerConfig | null;
  ...
  return (
    <View className="items-center pb-2">
      ...
      >
        ...
        <Text
          className="max-w-[260px] text-sm font-notcodex-bold leading-snug text-foreground"
          numberOfLines={1}
        >
          {props.status.label}
        </Text>
...
export const ThreadComposer = memo(function ThreadComposer(props: ThreadComposerProps) {
  ...
  const handleSend = useCallback(async () => {
    ...
    armAgentAwarenessLiveActivityForLocalWork({
      ...
      projectTitle: props.environmentLabel ?? "Not Codex",
    });
    try {
      await onSendMessage();
    } finally {
          ...
          {!isExpanded && props.draftAttachments.length > 0 ? (
            <View className="flex-row gap-1 pl-1">
              {props.draftAttachments.slice(0, 3).map((image) => {
                ...
                return (
                  <Pressable key={image.id} onPress={() => onPressImage(previewUri)}>
                    <Image
              ...
              {props.draftAttachments.length > 3 ? (
                <View className="size-[30px] items-center justify-center rounded-lg bg-subtle-strong">
                  <Text className="text-foreground-muted text-2xs font-notcodex-bold">
                    +{props.draftAttachments.length - 3}
                  </Text>
                </View>
              ) : null}
