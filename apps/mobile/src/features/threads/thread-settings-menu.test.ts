...
import { ProviderInstanceId, type ProviderOptionDescriptor } from "@notcodex/contracts";
...
describe("buildThreadSettingsMenu", () => {
  ...
  it("summarizes the current choice on each submenu row", () => {
    ...
    expect(menu.actions.find((action) => action.title === "Runtime")?.subtitle).toBe(
      "Approve actions",
    );
  });
  ...
  it("resolves select-option and runtime events with checkmarked current values", () => {
    ...
    expect(runtimeItems.find((action) => action.title === "Approve actions")?.state).toBe("on");
    expect(
