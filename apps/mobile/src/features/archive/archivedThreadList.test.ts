import type { ArchivedSnapshotEntry } from "@notcodex/client-runtime/state/threads";
import type { OrchestrationProjectShell, OrchestrationThreadShell } from "@notcodex/contracts";
import { EnvironmentId, ProjectId, ProviderInstanceId, ThreadId } from "@notcodex/contracts";
...
describe("buildArchivedThreadGroups", () => {
  it("groups archived threads by project and sorts newest first", () => {
    const project = makeProject({ id: ProjectId.make("project-1"), title: "Not Codex" });
    ...
    const result = buildArchivedThreadGroups({
      ...
      environmentLabels: { [environmentId]: "Collaborator's MacBook Pro" },
      environmentId: null,
      searchQuery: "",
      sortOrder: "newest",
    });
  ...
  it("filters by environment and matches project, thread, and branch text", () => {
    ...
    const firstProject = makeProject({ id: ProjectId.make("project-1"), title: "Not Codex" });
