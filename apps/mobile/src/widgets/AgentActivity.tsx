...
export function AgentActivity(
  ...
  const deepLink =
    deepLinkRow && deepLinkRow.deepLink.startsWith("/") && !deepLinkRow.deepLink.startsWith("//")
      ? `notcodex://${deepLinkRow.deepLink.slice(1)}`
      : null;
  ...
  // The branded Not Codex mark. `assetName` resolves the template image set bundled in
  ...
  // foreground style, which the template image inherits. The frame matches the
  // original mark's 568:657 aspect ratio so it never distorts.
  const renderLogo = (height: number, color: string) => (
    <HStack modifiers={[frame({ width: height * (568 / 657), height }), foregroundStyle(color)]}>
      <Image assetName="NotCodexMark" modifiers={[resizable()]} />
    </HStack>
  );
