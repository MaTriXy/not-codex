...
import { EnvironmentRegistry } from "@notcodex/client-runtime/connection";
import { request, runStream } from "@notcodex/client-runtime/rpc";
import { makeEnvironmentHttpApiClient } from "@notcodex/client-runtime/rpc";
import { ManagedRelay } from "@notcodex/client-runtime/relay";
  ...
  return Effect.gen(function* () {
    ...
    if (status.status === "unsupported") {
      return yield* new CloudEnvironmentLinkError({
        message: `Not Codex cannot install the relay client automatically on ${status.platform}-${status.arch}.`,
      });
    }
      ...
      return yield* new CloudEnvironmentLinkError({
        message:
          installedStatus.status === "unsupported"
            ? `Not Codex cannot install the relay client automatically on ${installedStatus.platform}-${installedStatus.arch}.`
...
> {
  return Effect.gen(function* () {
    ...
    if (!configuredRelayUrl) {
      return yield* new CloudEnvironmentLinkError({
        message: "NOT_CODEX_RELAY_URL is not configured.",
      });
    }
...
// independently of Not Codex Connect while clients reach the environment out of band.
