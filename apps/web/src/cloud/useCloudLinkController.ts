...
/**
 * Drives the primary environment's Not Codex Connect link. Not Codex Connect (managed
...
export function useCloudLinkController() {
  ...
  const reportUpdateFailure = (cause: unknown) => {
    const message =
      cause instanceof Error ? cause.message : "Could not update Not Codex Connect access.";
    const traceId = findErrorTraceId(cause);
    console.error("[not-codex-connect] Could not update Not Codex Connect", {
      message,
      traceId,
      cause,
    });
    ...
    toastManager.add({
      type: "error",
      title: "Could not update Not Codex Connect",
      description: message,
      data: traceId
        ? {
  ...
  const reconcileCloudState = async (desired: CloudLinkDesiredState): Promise<boolean> => {
    ...
    if (!wantsLink) {
      ...
      // leave the user unable to turn Not Codex Connect off.
      ...
      if (!clerkToken) {
        reportUpdateFailure(new Error("Sign in to Not Codex Connect before enabling this."));
        return false;
      }
