...
describe("Open VSX themes", () => {
  ...
  it("rejects forged import metadata before making a network request", async () => {
    const fetchMock = vi.fn();
    vi.stubGlobal("fetch", fetchMock);
    ...
    await expect(
      importOpenVsxThemeExtension({
        ...extension,
        manifestUrl: "http://127.0.0.1/private-theme.json",
      }),
    ).rejects.toThrow("untrusted package metadata");
    await expect(
      importOpenVsxThemeExtension({
        ...extension,
        collectionId: "open-vsx:another.theme",
      }),
    ).rejects.toThrow("untrusted package metadata");
