import type { ConfirmDialogOptions, ConfirmDialogVariant } from "@notcodex/contracts";
...
function resolvePendingConfirmations(confirmed: boolean): void {
  ...
  for (const confirmation of queuedConfirmations) confirmation.resolve(confirmed);
...
/** Returns undefined when no themed host is mounted so callers can fail closed. */
