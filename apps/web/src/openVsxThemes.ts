...
export async function importOpenVsxThemeExtension(
  ...
  const manifestUrl = trustedOpenVsxUrl(extension.manifestUrl);
  const sha256Url = trustedOpenVsxUrl(extension.sha256Url);
  const vsixUrl = trustedOpenVsxUrl(extension.vsixUrl);
  if (
    !manifestUrl ||
    !sha256Url ||
    !vsixUrl ||
    !SUPPORTED_LICENSES.has(extension.license) ||
    extension.collectionId !== openVsxCollectionId(extension.id)
  ) {
    throw new Error("That Open VSX theme has untrusted package metadata.");
  }

  const manifestResponse = await fetch(manifestUrl, signal ? { signal } : {});
  ...
  const packageBytes = await fetchPackage(vsixUrl, signal);
  signal?.throwIfAborted();
  const checksumResponse = await fetch(sha256Url, signal ? { signal } : {});
