...
/** Preserve bare Windows drive links through react-markdown URL sanitization. */
export function normalizeWindowsMarkdownFileHref(value: string): string {
  return WINDOWS_DRIVE_PATH_PATTERN.test(value) ? `file:///${value.replaceAll("\\", "/")}` : value;
}
