...
describe("Pierre file icons", () => {
  ...
  it("extends Pierre with Not Codex-specific exact filename icons", () => {
    assert.equal(
      resolvePierreIconForEntry("package.json", "file")?.name,
      "notcodex-file-icon-package-json",
    );
    assert.equal(
      resolvePierreIconForEntry("config/tsconfig.json", "file")?.name,
      "notcodex-file-icon-tsconfig",
    );
    assert.equal(resolvePierreIconForEntry("AGENTS.md", "file")?.name, "notcodex-file-icon-agents");
    assert.equal(resolvePierreIconForEntry("CLAUDE.md", "file")?.name, "notcodex-file-icon-claude");
    assert.equal(resolvePierreIconForEntry("README.md", "file")?.name, "notcodex-file-icon-readme");
    assert.equal(
      resolvePierreIconForEntry("pnpm-lock.yaml", "file")?.name,
      "notcodex-file-icon-pnpm",
    );
    assert.equal(
  ...
  it("ships every custom icon referenced by the extended resolver", () => {
    const customIconNames = new Set(Object.values(NOT_CODEX_PIERRE_ICONS.byFileName));
    for (const iconName of customIconNames) {
      assert.include(NOT_CODEX_PIERRE_ICONS.spriteSheet, `id="${iconName}"`);
    }
  });
