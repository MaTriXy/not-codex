...
// thread holds a live, fully hydrated detail subscription, so this limit is a
// direct renderer-heap and server-load multiplier. Cold opens still render
// immediately from the cached snapshot.
...
export function shouldNavigateAfterProjectRemoval(input: {
  ...
  projectThreads: readonly { environmentId: string; id: string }[];
  projectDraftId: string | null;
}): boolean {
  const { projectDraftId, projectThreads, routeTarget } = input;
  if (routeTarget?.kind === "draft") return projectDraftId === routeTarget.draftId;
  if (routeTarget?.kind !== "server") return false;
