...
import { encodeConnectAuthCode, readConnectAuthorizeRequest } from "@notcodex/shared/connectAuth";
...
const invalidLinkMessage = {
  ...
  description:
    "The link is missing its authorization request. Re-run `notcodex connect` in your terminal and open the freshly printed URL.",
} as const;
  ...
  return (
    <AuthSurfaceShell>
      <ConnectCliAuthMessage
        ...
        description={
          isSignedIn
            ? "Redirecting to authorize Not Codex Connect for your CLI…"
            : "Sign in to continue authorizing Not Codex Connect for your CLI."
        }
      />
      {isLoaded && !isSignedIn ? (
        <div className="mt-6">
...
export function ConnectCliCallbackSurface() {
  ...
  const [stateMatches] = useState(() => result !== null && hasConnectCliAuthState(result.state));
  const { user } = useUser();
  const { copyToClipboard, isCopied } = useCopyToClipboard({ target: "authentication code" });

  useEffect(() => {
    if (result && stateMatches) {
      forgetConnectCliAuthState(result.state);
    }
  }, [result, stateMatches]);
    ...
    return (
      <AuthSurfaceShell>
        <ConnectCliAuthMessage
          ...
          description="No authorization code was returned. Re-run `notcodex connect` in your terminal and try again."
        />
      </AuthSurfaceShell>
    );
  }
    ...
    return (
      <AuthSurfaceShell>
        <ConnectCliAuthMessage
          ...
          description="This authorization response does not match a connect request started in this browser. Re-run `notcodex connect` in your terminal and open the freshly printed URL in this browser."
        />
      </AuthSurfaceShell>
    );
  }
  ...
  return (
    <AuthSurfaceShell>
      ...
      <p className="mt-6 text-xs leading-relaxed text-muted-foreground">
        ...
        their machine to your Not Codex Connect account while it is valid.
      </p>
    </AuthSurfaceShell>
  );
}
