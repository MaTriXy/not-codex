...
import {
  ...
  effectiveSnoozed,
  threadWokeAt,
} from "@notcodex/client-runtime/state/thread-settled";
import type { EnvironmentThreadShell } from "@notcodex/client-runtime/state/models";
import {
  scopeProjectRef,
  scopeThreadRef,
  scopedThreadKey,
} from "@notcodex/client-runtime/environment";
import type { ScopedThreadRef } from "@notcodex/contracts";
...
const SETTLED_SHELF_EXPANDED_KEY = "notcodex:sidebar-v2:settled-expanded";
const SNOOZED_SHELF_EXPANDED_KEY = "notcodex:sidebar-v2:snoozed-expanded";
...
const SidebarDraftBlock = memo(function SidebarDraftBlock(props: {
  ...
  projectFaviconPathByKey: ReadonlyMap<string, string | null>;
  ...
  const drafts = useMemo(() => {
    ...
    for (const [draftKey, session] of Object.entries(draftThreadsByThreadKey)) {
      ...
      if (!composer || !composerDraftHasUserContent(composer)) continue;
  ...
  const handleDiscard = useCallback(
    (draftId: DraftId) => clearDraftThread(draftId),
    [clearDraftThread],
  );
...
const SidebarV2Row = memo(function SidebarV2Row(props: {
  ...
  // an explicit act, so the pill clears only when the user re-engages or
  // explicitly dismisses it. Finished pull requests need no wake-up call.
...
export default function SidebarV2() {
  ...
  // instead rewrites every key in the section). Keep the optimistic order
  // until every planned key is reflected in canonical state so a multi-write
  // materialization cannot visibly reshuffle between writes.
  const pinnedDndSensors = useSensors(
    useSensor(PointerSensor, { activationConstraint: { distance: 6 } }),
  );
  const [optimisticPinnedOrder, setOptimisticPinnedOrder] = useState<{
    readonly order: readonly string[];
    /** pinOrderKey per thread as of the drop. */
    readonly keysAtDrop: ReadonlyMap<string, string | null>;
    /** Keys this drop is expected to write; all must land before release. */
  ...
  useEffect(() => {
    ...
    // Release on membership changes, a foreign reorder, or once all writes
    // from this drop have landed. Releasing on the first assignment exposes
    // half-materialized order and makes the pinned block jump repeatedly.
