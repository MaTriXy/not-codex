import { findErrorTraceId } from "@notcodex/client-runtime/errors";
import {
  isAtomCommandInterrupted,
  squashAtomCommandFailure,
} from "@notcodex/client-runtime/state/runtime";
import type { EnvironmentId } from "@notcodex/contracts";
import type { RelayClientEnvironmentRecord } from "@notcodex/contracts/relay";
...
export function NotCodexConnectEnvironmentRow(props: {
          ...
          <div className="pt-3">
            ...
            >
              ...
              <p className="mt-4 max-w-xl text-[0.8125rem] leading-[1.125rem] text-muted-foreground">
                Not Codex Connect access will be revoked, any managed tunnel will be removed, and a
...
export function NotCodexConnectUserProfilePage() {
  ...
  const handleDeregister = async (environment: RelayClientEnvironmentRecord) => {
    ...
    if (result._tag === "Success") {
      ...
      toastManager.add({
        ...
        description: "Not Codex Connect access was revoked and a host space is now available.",
      });
      return;
    }
    ...
    console.error("[notcodex-connect] Could not deregister environment", {
      environmentId: environment.environmentId,
      message,
      traceId,
      cause,
          ...
          <Empty className="min-h-64 gap-4 border-t px-6 py-10 md:p-10">
            ...
            <EmptyHeader>
              <EmptyTitle className="text-[1.0625rem] leading-6">
                No Not Codex Connect environments
              </EmptyTitle>
              <EmptyDescription className="text-[0.8125rem] leading-[1.125rem]">
                Link an environment from its local Settings to make it available through Not Codex
                Connect.
              </EmptyDescription>
            </EmptyHeader>
          </Empty>
