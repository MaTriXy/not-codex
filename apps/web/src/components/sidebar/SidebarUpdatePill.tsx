            ...
            <section>
              ...
              <ul className="mt-2 space-y-1.5 pl-4 text-xs leading-5 text-popover-foreground/90">
                {releaseNote.items.map((item) => (
                  <li className="list-disc break-words" key={item}>
                    {item}
                  </li>
                ))}
              </ul>
        ...
        <TooltipTrigger
          render={
            <button
              ...
              className={cn(
                "inline-flex size-8 items-center justify-center rounded-lg outline-hidden ring-ring transition-colors enabled:cursor-pointer focus-visible:ring-2 disabled:cursor-not-allowed disabled:opacity-60",
                isUpdateState
                  ? "bg-update-surface text-update-foreground enabled:hover:bg-update/22"
                  : "text-muted-foreground/70 enabled:hover:bg-accent enabled:hover:text-foreground",
              )}
        ...
        <TooltipPopup
          ...
          className={
            isUpdateState && state?.channel === "nightly" && state.releaseNotes.length > 0
              ? // Keep release notes open and scrollable when the cursor moves into them.
