...
import { parseScopedThreadKey } from "@notcodex/client-runtime/environment";
import type { AgentPanelModel } from "@notcodex/client-runtime/state/subagentRuntime";
import {
  emptyAgentPanelModel,
  formatSubagentTokenCount,
} from "@notcodex/client-runtime/state/subagentRuntime";
import { resolveChatListAnchoredEndSpace } from "@notcodex/shared/chatList";
...
export const MessagesTimeline = memo(function MessagesTimeline({
  ...
  const activityState = useMemo<TimelineRowActivityState>(
    ...
    [activeTurnInProgress, isRevertingCheckpoint, isWorking, workingStepLabel],
  );
      ...
      <TimelineRowActivityCtx value={activityState}>
        <div ref={setTimelineViewportElement} className="relative h-full min-h-0">
          <LegendList<MessagesTimelineRow>
            ...
            maintainScrollAtEnd={
              anchoredEndSpace || disclosureToggleSettling || !liveFollowEnabled
                ? false
...
const TurnPlanTimelineRow = memo(function TurnPlanTimelineRow({
  ...
  const label =
    steps.find(({ status }) => status === "inProgress")?.step ??
    steps.find(({ status }) => status === "pending")?.step ??
    steps.at(-1)?.step ??
    "Plan";
