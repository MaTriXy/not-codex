...
import { CHAT_LIST_ANCHOR_OFFSET } from "@notcodex/shared/chatList";
import { projectScriptCwd, projectScriptRuntimeEnv } from "@notcodex/shared/projectScripts";
import { truncate } from "@notcodex/shared/String";
import { nextTerminalId, resolveTerminalSessionLabel } from "@notcodex/shared/terminalLabels";
...
import { buildTemporaryWorktreeBranchName } from "@notcodex/shared/git";
...
} from "@notcodex/client-runtime/state/subagentRuntime";
import { DiffWorkerPoolProvider } from "./DiffWorkerPoolProvider";
import { BranchToolbar } from "./BranchToolbar";
import { resolveShortcutCommand, shortcutLabelForCommand } from "../keybindings";
import PlanSidebar from "./PlanSidebar";
import ThreadTerminalDrawer from "./ThreadTerminalDrawer";
import { AlarmClockIcon, ChevronDownIcon, Minimize2Icon, WifiOffIcon } from "lucide-react";
...
import {
  ...
  threadHasOlderTurns,
} from "@notcodex/client-runtime/state/threads";
...
function useLocalDispatchState(input: {
  ...
  const beginLocalDispatch = useCallback(
    (options?: { preparingWorktree?: boolean; dispatchedMessageId?: ChatMessage["id"] }) => {
      const preparingWorktree = Boolean(options?.preparingWorktree);
      setLocalDispatch((current) => {
        const active = serverAcknowledgedLocalDispatch ? null : current;
        if (active) {
          const dispatchedMessageId = options?.dispatchedMessageId ?? active.dispatchedMessageId;
          return active.preparingWorktree === preparingWorktree &&
            active.dispatchedMessageId === dispatchedMessageId
            ? active
            : { ...active, preparingWorktree, dispatchedMessageId };
        }
...
function ChatViewContent(props: ChatViewProps) {
  ...
  const visibleThreadError = shouldShowThreadErrorBanner(
    ...
    ? threadError
    : null;
  // The dismissal set is session-scoped and intentionally external to React;
  // bump a local tick so a persisted session error hides immediately.
  ...
  const serverUpdateEnvironmentId =
    activeThread?.environmentId ?? primaryEnvironment?.environmentId ?? null;
  ...
  const composerBannerItems = useMemo<ComposerBannerStackItem[]>(() => {
    ...
    if (activeEnvironmentUnavailableState) {
      ...
      if (!isReconnecting || reconnectWarningGraceElapsed)
        items.push({
    ...
    if (showVersionMismatchBanner && serverUpdateEnvironmentId !== null) {
    ...
    const priority = (item: ComposerBannerStackItem) => {
      ...
      if (item.id.startsWith("server-version:") && serverUpdateState.status !== "idle") return 1;
      if (item.id.startsWith("server-version:")) return 2;
      return 3;
    };
  ...
  const [resumeCompactionPermanentlyDismissed, setResumeCompactionPermanentlyDismissed] =
    useLocalStorage(
      `notcodex:resume-compaction-dismissed:${environmentId}:${activeProviderInstanceId ?? "claudeAgent"}`,
      false,
      Schema.Boolean,
    );
  ...
  const visibleComposerBannerItems = useMemo(
    () =>
      resumeCompactionBannerItem === null
        ? composerBannerItems
        : [...composerBannerItems, resumeCompactionBannerItem],
    [composerBannerItems, resumeCompactionBannerItem],
  );
  ...
  // the sent message can stay near the top. Not Codex only owns streaming adjustments
  ...
  const onSend = async (e?: { preventDefault: () => void }) => {
    ...
    beginLocalDispatch({
      ...
      dispatchedMessageId: messageIdForSend,
    });
  ...
  const onSubmitPlanFollowUp = useCallback(
    async ({
      ...
      sendInFlightRef.current = true;
      beginLocalDispatch({ preparingWorktree: false, dispatchedMessageId: messageIdForSend });
      setThreadError(threadIdForSend, null);
  ...
  const onImplementPlanInNewThread = useCallback(async () => {
    ...
    beginLocalDispatch({
      preparingWorktree: false,
      dispatchedMessageId: implementationMessageId,
    });
    const finish = () => {
      sendInFlightRef.current = false;
      resetLocalDispatch();
  ...
  const rightPanelContent = activeThreadRef ? (
    activeRightPanelSurface?.kind === "preview" ? (
      ...
      <PullRequestsUnavailableState
        ...
        error="Update this environment's Not Codex server to browse pull requests."
      />
          ...
          <div className="relative flex min-h-0 min-w-0 flex-1 flex-col">
            ...
            <div className="relative flex min-h-0 flex-1 flex-col">
              ...
              <MessagesTimeline
                ...
                key={activeThread.id}
                isWorking={isWorking}
                workingStepLabel={activeThreadShell?.planProgress?.step ?? null}
              ...
              {showScrollToBottom && (
                ...
                >
                  <button
                    ...
                    className="chat-composer-glass pointer-events-auto flex items-center gap-1.5 rounded-full border border-border/60 px-3 py-1 text-muted-foreground text-xs shadow-sm transition-colors hover:cursor-pointer hover:border-border hover:text-foreground dark:shadow-none"
                  >
                  ...
                  {isDraftHeroState ? (
                    <div className="absolute inset-x-0 bottom-full z-0">
                      ...
                      <ComposerBannerStack
                        ...
                        items={visibleComposerBannerItems}
                      />
                    </div>
                  ) : (
                    <ComposerBannerStack
