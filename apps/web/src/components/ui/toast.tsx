...
import { type ScopedThreadRef, type ThreadId } from "@notcodex/contracts";
import {
  CheckIcon,
  ChevronDownIcon,
  ChevronUpIcon,
      ...
      >
        {visibleToastLayout.items.map(({ toast, visibleIndex, offsetY }) => {
          ...
          return (
            <Toast.Root
              className={cn(
                "absolute z-[calc(9999-var(--toast-index))] w-full overflow-visible select-none rounded-lg text-popover-foreground shadow-xl shadow-black/25 [transition:transform_.5s_cubic-bezier(.22,1,.36,1),opacity_.5s,height_.15s] dark:shadow-none",
          ...
          .map((toast) => {
            ...
            return (
              ...
              >
                <Toast.Root
                  className={cn(
                    "relative overflow-visible text-balance text-popover-foreground text-xs shadow-xl shadow-black/25 transition-[scale,opacity] data-ending-style:scale-98 data-starting-style:scale-98 data-ending-style:opacity-0 data-starting-style:opacity-0 dark:shadow-none",
