import type { EnvironmentId, ProjectId, PullRequestListEntry } from "@notcodex/contracts";
...
function entry(
  ...
  return {
    ...
    url: `https://github.com/matrixy/not-codex/pull/${overrides.number}`,
...
describe("the list snapshot across a reload", () => {
  ...
  const data = {
    ...
    truncated: true,
    nextCursors: { "matrixy/not-codex": "cursor-1" },
  } as never;
...
describe("merging the environments' own listings", () => {
  ...
  it("keeps each environment's continuation to itself", () => {
    const merged = mergePullRequestLists([
      [ENV_1, answer({ nextCursors: { "github.com matrixy/not-codex": "cursor-1" } })],
      [ENV_2, answer()],
    ]);
    expect(merged?.nextCursors).toEqual({
      [ENV_1]: { "github.com matrixy/not-codex": "cursor-1" },
    });
  });
...
describe("colon-namespaced labels typed as a search", () => {
  ...
  it("leaves a pasted link alone rather than naming a label after its scheme", () => {
    const parsed = parsePullRequestQuery("https://github.com/matrixy/not-codex/pull/1");
    expect(parsed.filters.labels).toBeUndefined();
    expect(parsed.text).toBe("https://github.com/matrixy/not-codex/pull/1");
  });
