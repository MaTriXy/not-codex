...
import { PullRequestListEntry, PullRequestListResult } from "@notcodex/contracts";
import type { PullRequestInvolvement, PullRequestListState } from "@notcodex/contracts";
...
const snapshotStorageKey = (environmentId: string) => `notcodex.pullRequests.list:${environmentId}`;
...
export function readPullRequestListSnapshot(
  ...
  try {
    ...
    if (decoded._tag === "None" || nowMs - decoded.value.savedAt > SNAPSHOT_TTL_MS) {
      return null;
    }
    const { savedAt: _savedAt, ...snapshot } = decoded.value;
    return snapshot;
  } catch {
    return null;
  }
