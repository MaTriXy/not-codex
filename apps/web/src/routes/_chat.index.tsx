...
function IndexDraftLanding() {
  ...
  return shellSummary.firstError ? (
    <ShellBootstrapError message={shellSummary.firstError} />
  ) : (
    <NoProjectsHero />
  );
}

function ShellBootstrapError({ message }: { readonly message: string }) {
  return (
    <SidebarInset className="h-dvh min-h-0 overflow-hidden overscroll-y-none bg-background text-foreground">
      <Empty className="flex-1">
        <EmptyHeader className="max-w-md">
          <EmptyTitle className="text-foreground text-xl">
            Couldn’t load environment data
          </EmptyTitle>
          <EmptyDescription className="mt-2 text-sm text-muted-foreground/78">
            {message} Check the environment connection, then try again.
          </EmptyDescription>
          ...
          <div className="w-full max-w-xl rounded-3xl border border-border/55 bg-card/20 px-8 py-12 shadow-sm/5">
            <EmptyHeader className="max-w-none">
              ...
              <EmptyDescription className="mt-2 text-sm leading-relaxed text-muted-foreground/78">
                {cloudEnabled
                  ? "Sign in to Not Codex Connect to connect a linked environment through its managed tunnel, or add a reachable backend manually."
