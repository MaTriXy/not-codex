...
} from "@notcodex/client-runtime/platform";
import {
...
import { bootstrapRemoteBearerSession } from "@notcodex/client-runtime/authorization";
import { fetchRemoteEnvironmentDescriptor } from "@notcodex/client-runtime/environment";
import {
  managedRelayAccountChanges,
  managedRelaySessionAtom,
} from "@notcodex/client-runtime/relay";
import { EnvironmentRpcRequestObserver } from "@notcodex/client-runtime/rpc";
import {
...
function clientMetadata() {
  ...
  return {
    label: desktop ? "Not Codex Desktop" : "Not Codex Web",
    ...
    const cloudSession = CloudSession.of({
      clerkToken: Effect.gen(function* () {
        ...
        if (session === null) {
          return yield* new ConnectionBlockedError({
            reason: "authentication",
            detail: "Sign in to Not Codex Connect to connect this environment.",
          });
        }
        ...
        if (token === null) {
          return yield* new ConnectionBlockedError({
            reason: "authentication",
            detail: "The Not Codex Connect session is unavailable.",
          });
        }
        return token;
      }),
