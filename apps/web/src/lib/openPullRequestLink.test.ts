...
import { ProjectId } from "@notcodex/contracts";
...
describe("matchesLinkedPullRequestUrl", () => {
  ...
  it("matches the same pull request without looking up its project", () => {
    expect(
      matchesLinkedPullRequestUrl(
        linkedPullRequest,
        "https://github.com/matrixy/NOT-CODEX/pull/42/files",
      ),
    ).toBe(true);
  });
  ...
  it("rejects a different pull request or host", () => {
    expect(
      matchesLinkedPullRequestUrl(
        linkedPullRequest,
        "https://github.com/MaTriXy/not-codex/pull/43",
      ),
    ).toBe(false);
    expect(
      matchesLinkedPullRequestUrl(
        linkedPullRequest,
        "https://github.example.com/MaTriXy/not-codex/pull/42",
      ),
    ).toBe(false);
  });
});
...
describe("openPullRequestLink", () => {
  it("opens the requested pull request URL", async () => {
    ...
    const targetUrl = "https://github.com/matrixy/not-codex/pull/123";
  ...
  it("reports bridge failures with a safe target origin", async () => {
    ...
    const targetUrl = "https://github.com/matrixy/not-codex/pull/123?token=secret";
...
describe("parseChangeRequestUrl", () => {
  it("reads a GitHub pull request", () => {
    expect(parseChangeRequestUrl("https://github.com/matrixy/not-codex/pull/123")).toEqual({
      host: "github.com",
      repository: "matrixy/not-codex",
      number: 123,
    });
  ...
  it("reads a GitLab merge request, nested groups and all", () => {
    expect(
      parseChangeRequestUrl("https://gitlab.com/t3tools/platform/not-codex/-/merge_requests/42"),
    ).toEqual({
      host: "gitlab.com",
      repository: "t3tools/platform/not-codex",
      number: 42,
    });
  });
  ...
  it("reads both Azure DevOps URL forms, keeping `_git` in the repository path", () => {
    expect(
      parseChangeRequestUrl("https://dev.azure.com/acme/platform/_git/not-codex/pullrequest/17"),
    ).toEqual({
      host: "dev.azure.com",
      repository: "acme/platform/_git/not-codex",
      number: 17,
    });
    expect(
      parseChangeRequestUrl("https://acme.visualstudio.com/platform/_git/not-codex/pullrequest/17"),
    ).toEqual({
      host: "acme.visualstudio.com",
      repository: "platform/_git/not-codex",
      number: 17,
    });
  });
  ...
  it("survives trailing segments, a trailing slash and a query string", () => {
    expect(
      parseChangeRequestUrl("https://github.com/matrixy/not-codex/pull/123/files?w=1"),
    ).toEqual({
      host: "github.com",
      repository: "matrixy/not-codex",
      number: 123,
  ...
  it("claims nothing it cannot be sure of, so the link goes to the browser", () => {
    for (const link of [
      "https://github.com/matrixy/not-codex/issues/123",
      "https://github.com/matrixy/not-codex/commit/0a1b2c3",
      "https://github.com/matrixy/not-codex",
      "https://github.com/matrixy/not-codex/pull/abc",
      "https://gitlab.com/matrixy/not-codex/-/snippets/12",
      "https://gitlab.com/matrixy/not-codex/-/issues/12",
      ...
      "javascript:alert(1)//github.com/matrixy/not-codex/pull/1",
      "not a url",
    ]) {
...
describe("findProjectForChangeRequest", () => {
  ...
  it("matches a nested GitLab group by the whole path below the host", () => {
    ...
    // two-segment owner/name form would look for `matrixy/not-codex` and find nothing.
    const projects = [
      project({
        canonicalKey: "gitlab.com/t3tools/platform/not-codex",
        provider: "gitlab",
        displayName: "t3tools/platform/not-codex",
        owner: "t3tools",
        name: "not-codex",
      }),
    ];
  ...
  it("keeps two hosts apart, so an Enterprise link does not open the public one", () => {
    const projects = [
      project({
        canonicalKey: "github.com/matrixy/not-codex",
        provider: "github",
        owner: "MaTriXy",
        name: "not-codex",
      }),
