(() => {
  ...
  const CUSTOM_THEMES_STORAGE_KEY = "notcodex:themes:v1";
  const THEME_FOLLOW_SYSTEM_STORAGE_KEY = "notcodex:theme-follow-system";
  const THEME_APPEARANCE_MODE_STORAGE_KEY = "notcodex:theme-appearance-mode";
  const THEME_HALVES_STORAGE_KEY = "notcodex:theme-halves:v1";
  const SPLASH_COLORS = {
    light: {
      background: "#ffffff",
      foreground: "#262626",
  ...
  // Not Codex palette is visible before React has mounted.
  ...
  const isStoredCustomTheme = (value) => {
    if (
      !isRecord(value) ||
      !isThemeId(value.id) ||
      RESERVED_THEME_IDS.has(value.id) ||
  ...
  try {
    ...
    // Older builds stored the dark legacy palette as its own theme id
