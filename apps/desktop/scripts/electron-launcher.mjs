// This file mostly exists because we want dev mode to say "Not Codex (Dev)" instead of "electron"
...
const devBundleIdSuffix = NodePath.basename(repoRoot)
  .toLowerCase()
  .replaceAll(/[^a-z0-9]+/g, "");
export const APP_DISPLAY_NAME = isDevelopment ? "Not Codex (Dev)" : "Not Codex (Alpha)";
export const APP_BUNDLE_ID = isDevelopment
  ? `com.notcodex.notcodex.dev.${devBundleIdSuffix || "local"}`
  : "com.notcodex.notcodex";
const APP_PROTOCOL_SCHEMES = isDevelopment ? ["notcodex-dev"] : ["notcodex"];
...
// oxlint-disable-next-line notcodex/no-global-process-runtime -- Standalone launcher script has no Effect runtime.
...
export function makeDevelopmentLauncherScript({
  ...
  const envEntries = [
    ...
    ["NOT_CODEX_PORT", environment.NOT_CODEX_PORT],
    ["NOT_CODEX_HOME", environment.NOT_CODEX_HOME],
    ["NOT_CODEX_COMMIT_HASH", environment.NOT_CODEX_COMMIT_HASH],
    ["NOT_CODEX_OTLP_TRACES_URL", environment.NOT_CODEX_OTLP_TRACES_URL],
    ["NOT_CODEX_OTLP_EXPORT_INTERVAL_MS", environment.NOT_CODEX_OTLP_EXPORT_INTERVAL_MS],
    ["NOT_CODEX_DESKTOP_APP_USER_MODEL_ID", APP_BUNDLE_ID],
  ...
  return [
    ...
    `exec ${shellSingleQuote(electronBinaryPath)} --notcodex-dev-root=${shellSingleQuote(desktopRoot)} ${shellSingleQuote(mainEntryPath)} "$@"`,
    "",
  ].join("\n");
}
...
function buildMacLauncher(electronBinaryPath) {
  ...
  if (isDevelopment) {
    ...
    // Electron.app even though this bundle carries the Not Codex identity. The
    // conventional executable name also keeps Electron in development mode.
    writeDevelopmentLauncherScript(launcherBinaryPath, runtimeElectronBinaryPath);
  }
