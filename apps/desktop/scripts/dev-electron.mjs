...
const remoteDebuggingPort = process.env.NOT_CODEX_DESKTOP_REMOTE_DEBUGGING_PORT?.trim();
// oxlint-disable-next-line notcodex/no-global-process-runtime -- Standalone dev script has no Effect runtime.
...
if (devProtocolClient) {
  childEnv.NOT_CODEX_DESKTOP_APP_USER_MODEL_ID = devProtocolClient.appBundleId;
  childEnv.NOT_CODEX_DESKTOP_PROTOCOL_REGISTRATION_MANAGED = "1";
}
...
function cleanupStaleDevApps() {
  ...
  NodeChildProcess.spawnSync("pkill", ["-f", "--", `--notcodex-dev-root=${desktopDir}`], {
    stdio: "ignore",
  });
}
...
function startApp() {
  ...
  const launchArgs = devProtocolClient
    ? electronArgs
    : [...electronArgs, `--notcodex-dev-root=${desktopDir}`, "dist-electron/main.cjs"];
