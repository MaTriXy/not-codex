...
import { ConnectionCatalogDocument } from "@notcodex/client-runtime/platform";
import { EnvironmentId, type PersistedSavedEnvironmentRecord } from "@notcodex/contracts";
...
function makeLayer(
  ...
  const environmentLayer = DesktopEnvironment.layer({
    ...
    Layer.provide(
      Layer.mergeAll(NodeServices.layer, DesktopConfig.layerTest({ NOT_CODEX_HOME: baseDir })),
    ),
  );
...
const withStore = <A, E, R>(
  ...
  Effect.gen(function* () {
    ...
    const baseDir = yield* fileSystem.makeTempDirectoryScoped({
      prefix: "notcodex-desktop-connection-catalog-test-",
    });
