...
describe("DesktopClerk", () => {
  ...
  it("derives the Clerk Frontend API hostname used by the desktop CSP", () => {
    const publishableKey = `pk_test_${btoa("clerk.notcodex.example$")}`;

    assert.equal(
      DesktopClerk.resolveDesktopClerkFrontendApiHostname(publishableKey),
      "clerk.notcodex.example",
    );
        ...
        [
          {
            ...
            passkeys: true,
            renderer: { scheme: "notcodex-dev", host: "app" },
          },
        ],
      ]);
  ...
  it.effect("preserves bridge initialization failures", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.equal(error.stateDir, "/tmp/notcodex-state");
      assert.equal(error.isDevelopment, true);
      assert.strictEqual(error.cause, cause);
      assert.equal(
        error.message,
        'Failed to initialize the desktop Clerk bridge for state directory "/tmp/notcodex-state" (development: true).',
      );
    });
  });
  ...
  it.effect("preserves bridge cleanup failures", () => {
    ...
    return Effect.gen(function* () {
      ...
      if (exit._tag === "Failure") {
        ...
        assert.equal(
          error.message,
          'Failed to clean up the desktop Clerk bridge for state directory "/tmp/notcodex-state" (development: false).',
        );
      }
    });
  });
  ...
  it.each([
    ...
    assert.equal(
      DesktopClerk.createDesktopClerkBridge("/tmp/notcodex-state", isDevelopment),
      bridge,
    );
    assert.deepEqual(storageMock.mock.calls, [[{ path: "/tmp/notcodex-state" }]]);
    assert.deepEqual(createClerkBridgeMock.mock.calls, [
      [
        {
          storage: storageAdapter,
