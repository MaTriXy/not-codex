...
import { buildRemoteNodeEnvScript } from "@notcodex/ssh/tunnel";
...
>()("@notcodex/desktop/wsl/DesktopWslEnvironment") {}
...
export const formatNodePtyProbeFailureReason = (exitCode: number): string | null =>
  exitCode === NODE_PTY_PREBUILD_MISSING_EXIT_CODE
    ? "WSL support is missing from this Not Codex build: the packaged Linux node-pty binary was not included. Rebuild the Windows artifact with `--wsl-prebuild <path-to-linux-pty.node>` or install a build that includes WSL support."
    : null;
...
const marker = path.join(prebuildDir, "notcodex-wsl-node-pty.json");
...
const NODE_PTY_BUILD_SCRIPT = (linuxServerDir: string) =>
  [
    ...
    `printf '{"arch":"%s","modules":"%s","nodePtyVersion":"%s"}\\n' "$arch" "$modules" "$node_pty_version" > "$prebuild_dir/notcodex-wsl-node-pty.json"`,
...
export const formatNodeEngineMismatchReason = (
  nodeVersion: string | null,
  nodeEngineRange: string | null | undefined,
): string | null => {
  const range = nodeEngineRange?.trim() || null;
  if (nodeVersion === null || range === null || satisfiesSemverRange(nodeVersion, range)) {
    return null;
  }
  return `WSL Node.js ${nodeVersion} does not satisfy the server's required engine range (${range}). Install a compatible version, and restart the desktop app.`;
};
...
const ensureNodePtyImpl = (
  ...
  Effect.gen(function* () {
    ...
    if (probe.exitCode === 0) {
      ...
      const versionMismatchReason = formatNodeEngineMismatchReason(
        rawVersion,
        options.nodeEngineRange,
      );
      if (versionMismatchReason !== null) {
        return {
          ok: false,
