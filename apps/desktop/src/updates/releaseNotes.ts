import type { DesktopUpdateReleaseNote } from "@notcodex/contracts";
...
function extractReleaseNoteItems(note: string | null | undefined): ReadonlyArray<string> {
  ...
  for (const rawLine of stripMarkup(note).split("\n")) {
    ...
    const displayItem = truncateReleaseNoteItem(item);
    if (seenItems.has(displayItem)) continue;
    seenItems.add(displayItem);
