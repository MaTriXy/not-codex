...
import type { DesktopUpdateState } from "@notcodex/contracts";
...
function makeHarness(options: UpdatesHarnessOptions = {}) {
  ...
  const environmentLayer = DesktopEnvironment.layer({
    ...
    homeDirectory: `/tmp/notcodex-desktop-updates-home-${process.pid}`,
    platform: "darwin",
    processArch: "x64",
    appVersion: "1.2.3",
    appPath: "/repo",
    ...
    Layer.provide(
      Layer.mergeAll(
        ...
        DesktopConfig.layerTest({
          NOT_CODEX_HOME: `/tmp/notcodex-desktop-updates-test-${process.pid}`,
          NOT_CODEX_DESKTOP_MOCK_UPDATES: "true",
          NOT_CODEX_DESKTOP_MOCK_UPDATE_SERVER_PORT: "4141",
          ...options.env,
        }),
      ),
    ),
