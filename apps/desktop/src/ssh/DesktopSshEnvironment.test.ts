...
import { SshPasswordPromptError } from "@notcodex/ssh/errors";
...
function makeTempHomeDir() {
  return Effect.gen(function* () {
    ...
    return yield* fs.makeTempDirectoryScoped({ prefix: "notcodex-ssh-env-test-" });
  });
}
...
describe("sshEnvironment", () => {
  it("keeps prompt presentation diagnostics distinct from the legacy wrapper message", () => {
    ...
    assert.equal(
      ...
      "Not Codex window is not available for SSH authentication.",
    );
  });
