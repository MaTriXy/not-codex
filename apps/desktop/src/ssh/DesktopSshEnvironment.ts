...
import * as SshAuth from "@notcodex/ssh/auth";
import { discoverSshHosts } from "@notcodex/ssh/config";
import {
  SshCommandError,
  ...
  SshReadinessError,
} from "@notcodex/ssh/errors";
import * as SshTunnel from "@notcodex/ssh/tunnel";
...
>()("@notcodex/desktop/ssh/DesktopSshEnvironment") {}

export interface DesktopSshEnvironmentLayerOptions {
  readonly resolveCliPackageSpec?: () => string;
  readonly resolveCliRunner?: Effect.Effect<SshTunnel.RemoteNotCodexRunnerOptions>;
}
...
export function toSshPasswordPromptError(
  ...
  switch (cause._tag) {
    ...
    case "DesktopSshPromptPresentationError":
      message = "Not Codex window is not available for SSH authentication.";
      break;
