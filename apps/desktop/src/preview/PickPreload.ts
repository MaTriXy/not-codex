...
const OVERLAY_ATTRIBUTE = "data-notcodex-annotation-ui";
const Z_INDEX_OVERLAY = 2147483646;
const PRIMARY = "var(--notcodex-primary)";
const PRIMARY_FILL = "color-mix(in srgb, var(--notcodex-primary) 10%, transparent)";
...
const applyAnnotationTheme = (
  ...
  const variables = {
    "--notcodex-radius": theme.radius,
    "--notcodex-background": theme.background,
    "--notcodex-foreground": theme.foreground,
    "--notcodex-popover": theme.popover,
    "--notcodex-popover-foreground": theme.popoverForeground,
    "--notcodex-primary": theme.primary,
    "--notcodex-primary-foreground": theme.primaryForeground,
    "--notcodex-muted": theme.muted,
    "--notcodex-muted-foreground": theme.mutedForeground,
    "--notcodex-accent": theme.accent,
    "--notcodex-accent-foreground": theme.accentForeground,
    "--notcodex-border": theme.border,
    "--notcodex-input": theme.input,
    "--notcodex-ring": theme.ring,
    "--notcodex-font-sans": theme.fontSans,
    "--notcodex-font-mono": theme.fontMono,
  };
...
function startAnnotation(): void {
  ...
  cursorStyle.textContent = `html[data-notcodex-annotation-tool] body, html[data-notcodex-annotation-tool] body * { cursor: crosshair !important; } [${OVERLAY_ATTRIBUTE}], [${OVERLAY_ATTRIBUTE}] * { cursor: default !important; } [${OVERLAY_ATTRIBUTE}] input[type=number]::-webkit-inner-spin-button, [${OVERLAY_ATTRIBUTE}] input[type=number]::-webkit-outer-spin-button { appearance:none; margin:0; }`;
  ...
  const refreshToolButtons = (): void => {
    ...
    document.documentElement.setAttribute("data-notcodex-annotation-tool", tool);
  };
      ...
      if (isUsableRect(rect)) {
        ...
        if (found === 0) {
          ...
          const regionBox = createBox(
            PRIMARY,
            "color-mix(in srgb, var(--notcodex-primary) 6%, transparent)",
          );
  ...
  const teardown = (notifyMain: boolean): void => {
    ...
    document.documentElement.removeAttribute("data-notcodex-annotation-tool");
    cursorStyle.remove();
    host.remove();
    activeSession = null;
