...
>()("@notcodex/desktop/backend/DesktopBackendConfiguration") {}
...
const DESKTOP_BACKEND_ENV_NAMES = [
  "NOT_CODEX_PORT",
  "NOT_CODEX_MODE",
  "NOT_CODEX_NO_BROWSER",
  "NOT_CODEX_HOST",
  "NOT_CODEX_DESKTOP_WS_URL",
  "NOT_CODEX_DESKTOP_LAN_ACCESS",
  "NOT_CODEX_DESKTOP_LAN_HOST",
  "NOT_CODEX_DESKTOP_HTTPS_ENDPOINTS",
  "NOT_CODEX_TAILSCALE_SERVE",
  "NOT_CODEX_TAILSCALE_SERVE_PORT",
] as const;
...
const resolvePrimaryStartConfig = Effect.fn("desktop.backendConfiguration.resolvePrimary")(
  ...
  > {
    ...
    const bootstrap = {
      ...
      notCodexHome: environment.baseDir,
    ...
    return {
      ...
      // Primary wants process.env (PATH, dev-runner's NOT_CODEX_HOME, etc.).
      extendEnv: true,
      bootstrap,
...
> {
  ...
  const bootstrap = {
    ...
    noBrowser: true,
    port: input.port,
    // Omit notCodexHome so the Linux backend uses its own home dir instead of
  ...
  // Build an explicit copy of process.env minus NOT_CODEX_HOME (dev-runner
  // exports the Windows-side base dir for the primary; if it leaks into
  // the WSL backend the Linux side ends up sharing C:\Users\...\.notcodex via
  // /mnt/c, which means both backends read/write the same database and
  // their env-ids collide).
  const parentEnvWithoutNotCodexHome: Record<string, string | undefined> = {};
  for (const [key, value] of Object.entries(process.env)) {
    if (key === "NOT_CODEX_HOME") continue;
    parentEnvWithoutNotCodexHome[key] = value;
  }
  const wslEnv = mergeWslEnv(parentEnvWithoutNotCodexHome.WSLENV, forwardedEnvNames);
  ...
  const baseConfig = {
    ...
    cwd: environment.backendCwd,
    env: {
      ...parentEnvWithoutNotCodexHome,
      ...backendChildEnvPatch(),
      ...forwardedEnv,
      ...(wslEnv !== undefined ? { WSLENV: wslEnv } : {}),
    },
    // env is already a complete process.env minus NOT_CODEX_HOME; pass it
