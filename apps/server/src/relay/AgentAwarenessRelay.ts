...
import { projectThreadAwareness } from "@notcodex/shared/agentAwareness";
...
>()("notcodex/relay/AgentAwarenessRelay") {}
...
const makePublishProof = Effect.fn("makePublishProof")(function* (input: {
  ...
  const payload = {
    iss: `notcodex-env:${input.environmentId}`,
      ...
      switch (startupState) {
        case "waiting-for-link":
          yield* Effect.logInfo(
            "agent activity publishing standby; waiting for Not Codex Connect link reconciliation",
          );
          break;
        case "disabled":
          yield* Effect.logInfo(
            "agent activity publishing disabled by Not Codex Connect configuration",
          );
          break;
        case "enabled":
