...
const makeServerConfigLayer = (
  ...
  Layer.effect(
    ...
    Layer.provide(
      ServerConfig.layerTest(process.cwd(), { prefix: "notcodex-auth-bootstrap-test-" }),
    ),
  );
...
it.layer(NodeServices.layer)("PairingGrantStore.layer", (it) => {
  ...
  it.effect("issues one-time bootstrap tokens that can only be consumed once", () =>
    Effect.gen(function* () {
      ...
      expect(first.label).toBe("Collaborator iPhone");
      expect(issued.label).toBe("Collaborator iPhone");
