...
function parseOpenCodeResume(raw: unknown): { readonly sessionId: string } | undefined {
  ...
  if (record.schemaVersion !== OPENCODE_RESUME_VERSION) return undefined;
...
/** A missing session is the only resume failure that may safely start fresh. */
...
/** Compare cwd spellings without splitting sessions on symlinks or dot segments. */
  ...
  return Effect.gen(function* () {
    ...
    const startSession: OpenCodeAdapterShape["startSession"] = Effect.fn("startSession")(
      function* (input) {
          ...
          const startedExit = yield* Effect.exit(
            Effect.gen(function* () {
              ...
              const resolved = yield* Effect.gen(function* () {
                ...
                return { openCodeSession: createdResponse.data, created: true };
              });
              return {
                sessionScope,
                server,
        ...
        if (raceWinner) {
          // A resumed remote session may be shared by the winner, so only
          // abort remote state created by this losing call.
          if (started.created) {
            yield* runOpenCodeSdk("session.abort", () =>
