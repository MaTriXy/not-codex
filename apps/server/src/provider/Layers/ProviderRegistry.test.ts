...
import { deepMerge } from "@notcodex/shared/Struct";
import { createModelCapabilities } from "@notcodex/shared/model";
import { applyServerSettingsPatch } from "@notcodex/shared/serverSettings";
...
process.env.NOT_CODEX_CURSOR_ENABLED = "1";
  ...
  (it) => {
    ...
    describe("ProviderRegistryLive", () => {
            ...
            ProviderRegistryLive.pipe(
              ...
              Layer.provideMerge(
                ServerConfig.layerTest(process.cwd(), {
                  prefix: "notcodex-provider-registry-background-refresh-",
                }),
              ),
        ...
        Effect.gen(function* () {
          ...
          const runtimeServices = yield* Layer.build(
            ProviderRegistryLive.pipe(
              ...
              Layer.provideMerge(
                ServerConfig.layerTest(process.cwd(), {
                  prefix: "notcodex-provider-registry-merged-persist-",
                }),
              ),
              Layer.provideMerge(NodeServices.layer),
            ),
        ...
        Effect.gen(function* () {
          ...
          const runtimeServices = yield* Layer.build(
            ProviderRegistryLive.pipe(
              ...
              Layer.provideMerge(
                ServerConfig.layerTest(process.cwd(), {
                  prefix: "notcodex-provider-registry-refresh-failure-",
                }),
              ),
              Layer.provideMerge(NodeServices.layer),
            ),
        ...
        Effect.gen(function* () {
          ...
          const runtimeServices = yield* Layer.build(
            ProviderRegistryLive.pipe(
              ...
              Layer.provideMerge(
                ServerConfig.layerTest(process.cwd(), {
                  prefix: "notcodex-provider-registry-sync-failure-",
                }),
              ),
              Layer.provideMerge(NodeServices.layer),
            ),
      ...
      it.effect("propagates real Codex probe failures to the aggregator at boot", () =>
        Effect.gen(function* () {
          const missingBinary = `notcodex_codex_missing_`;
          const serverSettings = yield* makeMutableServerSettingsService(
            decodeServerSettings(
              deepMerge(encodedDefaultServerSettings, {
                ...
                providerInstances: {
                  // Matches the shape the user had in `.notcodex/dev/settings.json`
          ...
          const providerRegistryLayer = ProviderRegistryLive.pipe(
            ...
            Layer.provideMerge(
              ServerConfig.layerTest(process.cwd(), {
                prefix: "notcodex-provider-registry-",
              }),
            ),
      ...
      it.effect("re-probes settings changes when a materialized settings read fails", () =>
        Effect.gen(function* () {
          ...
          const providerRegistryLayer = ProviderRegistryLive.pipe(
            ...
            Layer.updateService(ChildProcessSpawner.ChildProcessSpawner, (spawner) =>
              ChildProcessSpawner.make((command) => {
                ...
                return Effect.sync(() => spawnedCommands.push(executable)).pipe(
                  Effect.andThen(
                    executable === firstMissing
                      ? Deferred.succeed(initialProbe, undefined)
                      : Effect.void,
                  ),
          ...
          yield* Effect.gen(function* () {
            const registry = yield* ProviderRegistry.ProviderRegistry;
            // Subscribe before waiting for the boot-time probe, so each
            // assertion waits for the actual state transition rather than
            // polling a scheduler or test clock.
            ...
            yield* Effect.forever(
              PubSub.take(changes).pipe(
                Effect.flatMap((providers) => {
                  ...
                  return spawnedCommands.includes(secondMissing)
                    ? Deferred.succeed(reprobed, providers)
                    : Effect.void;
                }),
              ),
            ...
            // `firstMissing`, so the real spawner yields ENOENT.
            yield* Deferred.await(initialProbe);
            ...
            const refreshed = yield* Deferred.await(reprobed);
          ...
          Effect.gen(function* () {
            ...
            yield* Effect.gen(function* () {
              ...
              assert.strictEqual(
                ...
                "Cursor is disabled in Not Codex settings.",
              );
      ...
      it.effect("skips codex probes entirely when the provider is disabled", () =>
        Effect.gen(function* () {
          ...
          assert.strictEqual(status.installed, false);
          assert.strictEqual(status.message, "Codex is disabled in Not Codex settings.");
        }),
      );
    });
    ...
    describe("checkClaudeProviderStatus", () => {
      ...
      it.effect("runs Claude status probes with the configured CLAUDE_CONFIG_DIR", () => {
        const claudeConfigDir = "/tmp/notcodex-claude-home";
