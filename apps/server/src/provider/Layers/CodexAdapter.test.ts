...
class CodexAdapter extends Context.Service<CodexAdapter, CodexAdapterShape>()(
  "notcodex/provider/Layers/CodexAdapter.test/CodexAdapter",
) {}
...
lifecycleLayer("CodexAdapterLive lifecycle", (it) => {
  ...
  it.effect("maps Codex web-search queries into lifecycle details", () =>
    Effect.gen(function* () {
      const { adapter, runtime } = yield* startLifecycleRuntime();
      const firstEventFiber = yield* Stream.runHead(adapter.streamEvents).pipe(Effect.forkChild);
      ...
      yield* runtime.emit({
        ...
        payload: {
          ...
          item: {
            type: "webSearch",
            id: "search_1",
            query: "Not Codex upstream fixes",
            action: { type: "search", queries: ["Not Codex upstream fixes"] },
          },
        },
      ...
      NodeAssert.equal(firstEvent.value.payload.detail, "Not Codex upstream fixes");
    }),
  );
  ...
  it.effect("labels MCP lifecycle entries with server and tool names", () =>
    Effect.gen(function* () {
      ...
      NodeAssert.equal(firstEvent.value.payload.title, "notcodex · preview_status");
...
it.effect("flushes managed native logs when the adapter layer shuts down", () =>
  Effect.gen(function* () {
    const tempDir = NodeFS.mkdtempSync(
      NodePath.join(NodeOS.tmpdir(), "notcodexx-adapter-native-log-"),
    );
