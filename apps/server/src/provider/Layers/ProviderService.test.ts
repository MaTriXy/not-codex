...
const serverConfigTestLayer = ServerConfig.layerTest(process.cwd(), {
  prefix: "notcodex-provider-service-test-",
}).pipe(Layer.provide(NodeServices.layer));
const defaultProviderServiceConfigLayer = Layer.mergeAll(
...
const encodePromptStringLiteral = Schema.encodeUnknownSync(Schema.UnknownFromJsonString);
  ...
  const layer = it.layer(
    Layer.mergeAll(
      makeProviderServiceLive().pipe(
        ...
        Layer.provide(defaultProviderServiceConfigLayer),
...
it.effect("ProviderServiceLive keeps persisted resumable sessions on startup", () =>
  Effect.gen(function* () {
    const tempDir = NodeFS.mkdtempSync(
      NodePath.join(NodeOS.tmpdir(), "notcodex-provider-service-"),
    );
...
it.effect(
  ...
  () =>
    Effect.gen(function* () {
      const tempDir = NodeFS.mkdtempSync(
        NodePath.join(NodeOS.tmpdir(), "notcodex-provider-service-restart-"),
      );
      const dbPath = NodePath.join(tempDir, "orchestration.sqlite");
      const persistenceLayer = makeSqlitePersistenceLive(dbPath);
      const runtimeRepositoryLayer = ProviderSessionRuntime.layer.pipe(
...
routing.layer("ProviderServiceLive routing", (it) => {
  it.effect("appends validated attachment paths to provider turn text", () =>
    Effect.gen(function* () {
      const provider = yield* ProviderService.ProviderService;
      ...
      const pathLine = `[Attached image ${encodePromptStringLiteral(attachment.name)} is saved at path ${encodePromptStringLiteral(attachmentPath)}]`;
      ...
      const attachmentOnlyInput = routing.codex.sendTurn.mock
        .calls[0]?.[0] as ProviderSendTurnInput;
      assert.equal(attachmentOnlyInput.input, pathLine);
  ...
  it.effect("reuses persisted resume cursor when startSession is called after a restart", () =>
    Effect.gen(function* () {
      const tempDir = NodeFS.mkdtempSync(
        NodePath.join(NodeOS.tmpdir(), "notcodex-provider-service-start-"),
      );
      const dbPath = NodePath.join(tempDir, "orchestration.sqlite");
      const persistenceLayer = makeSqlitePersistenceLive(dbPath);
      const runtimeRepositoryLayer = ProviderSessionRuntime.layer.pipe(
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        const tempDir = NodeFS.mkdtempSync(
          NodePath.join(NodeOS.tmpdir(), "notcodex-provider-service-cwd-"),
        );
        const dbPath = NodePath.join(tempDir, "orchestration.sqlite");
        const persistenceLayer = makeSqlitePersistenceLive(dbPath);
        const runtimeRepositoryLayer = ProviderSessionRuntime.layer.pipe(
...
fanout.layer("ProviderServiceLive fanout", (it) => {
  ...
  it.effect("records provider metrics with the routed provider label", () =>
    Effect.gen(function* () {
      ...
      assert.equal(
        hasMetricSnapshot(snapshots, "notcodex_provider_turns_total", {
          provider: ProviderDriverKind.make("claudeAgent"),
          operation: "interrupt",
          outcome: "success",
        }),
      ...
      assert.equal(
        hasMetricSnapshot(snapshots, "notcodex_provider_sessions_total", {
          provider: ProviderDriverKind.make("claudeAgent"),
          operation: "stop",
          outcome: "success",
        }),
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        assert.equal(
          hasMetricSnapshot(snapshots, "notcodex_provider_turn_duration", {
            provider: ProviderDriverKind.make("claudeAgent"),
            operation: "send",
          }),
          true,
