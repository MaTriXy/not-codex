...
import type { CursorSettings } from "@notcodex/contracts";
...
const cursorAcpDiscoveryFailedMessage = [
  ...
  "Cursor CLI setup may be incomplete; install or enable the Cursor CLI, restart Not Codex, and try again.",
  "See https://cursor.com/docs/cli/installation.",
  "Check server logs for ACP details.",
].join(" ");
const missingCursorBinaryPath = "/definitely/not/installed/notcodex-cursor-agent";
const cursorCliCommandMissingMessage = [
  `Cursor CLI command \`${missingCursorBinaryPath}\` was not found.`,
  `Install or enable the Cursor CLI, make sure \`${missingCursorBinaryPath}\` is on PATH, then restart Not Codex.`,
  "See https://cursor.com/docs/cli/installation.",
].join(" ");
