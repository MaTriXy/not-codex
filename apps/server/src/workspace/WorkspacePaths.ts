...
>()("notcodex/workspace/WorkspacePaths") {}
...
export function resolveWorkspaceRootInput(input: string, path: Path.Path): string {
  return path.resolve(expandHomePath(input.trim(), path));
}
