...
>()("notcodex/persistence/ProviderSessionRuntime/ProviderSessionRuntimeRepository") {}
...
export const make = Effect.gen(function* () {
  ...
  const list: ProviderSessionRuntimeRepository["Service"]["list"] = () =>
    listRuntimeRows(undefined).pipe(
      ...
      Effect.flatMap((rows) =>
        // Skip rows that no longer decode (for example, rows written by an
        // older build) so one stale session cannot disable list consumers.
