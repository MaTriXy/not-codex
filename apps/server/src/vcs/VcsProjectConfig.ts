...
import { VcsDriverKind, type VcsDriverKind as VcsDriverKindType } from "@notcodex/contracts";
import { fromLenientJson } from "@notcodex/shared/schemaJson";
...
export const make = Effect.gen(function* () {
  ...
  const findConfigPath = Effect.fn("VcsProjectConfig.findConfigPath")(function* (cwd: string) {
    let current = cwd;
    while (true) {
      const candidate = path.join(current, ".notcodex", "vcs.json");
