...
const ServerConfigLayer = ServerConfig.layerTest(process.cwd(), {
  prefix: "notcodex-git-vcs-driver-test-",
});
    ...
    it.effect("makes background upstream status fetches non-interactive", () =>
      Effect.gen(function* () {
        ...
        const envKeys = [
          ...
          "SSH_ASKPASS",
          "SSH_ASKPASS_REQUIRE",
          "NOT_CODEX_TEST_SSH_ASKPASS_LOG",
        ] as const;
        ...
        yield* fileSystem.writeFileString(
          ...
          [
            "#!/bin/sh",
            'printf "GCM_INTERACTIVE=%s\\n" "${GCM_INTERACTIVE:-}" > "$NOT_CODEX_TEST_SSH_ASKPASS_LOG"',
            'printf "GIT_ASKPASS=%s\\n" "${GIT_ASKPASS:-}" >> "$NOT_CODEX_TEST_SSH_ASKPASS_LOG"',
            'printf "GIT_TERMINAL_PROMPT=%s\\n" "${GIT_TERMINAL_PROMPT:-}" >> "$NOT_CODEX_TEST_SSH_ASKPASS_LOG"',
            'printf "SSH_ASKPASS=%s\\n" "${SSH_ASKPASS:-}" >> "$NOT_CODEX_TEST_SSH_ASKPASS_LOG"',
            'printf "SSH_ASKPASS_REQUIRE=%s\\n" "${SSH_ASKPASS_REQUIRE:-}" >> "$NOT_CODEX_TEST_SSH_ASKPASS_LOG"',
            "exit 1",
            "",
          ].join("\n"),
        );
        ...
        yield* Effect.gen(function* () {
          ...
          process.env.NOT_CODEX_TEST_SSH_ASKPASS_LOG = sshLogPath;
    ...
    it.effect("creates a worktree from the latest fetched remote commit", () =>
      Effect.gen(function* () {
        ...
        yield* driver.createWorktree({
          ...
          newRefName: "notcodex/fetched-origin",
        ...
        assert.equal(
          yield* driver.readConfigValue(
            worktreePath,
            "branch.notcodex/fetched-origin.gh-merge-base",
          ),
          initialBranch,
        );
        assert.equal(
          yield* driver.readConfigValue(worktreePath, "branch.notcodex/fetched-origin.remote"),
          null,
        );
