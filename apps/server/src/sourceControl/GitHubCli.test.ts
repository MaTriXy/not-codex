...
import { VcsProcessExitError, VcsProcessSpawnError } from "@notcodex/contracts";
          ...
          processOutput(
            ...
            JSON.stringify([
              ...
              },
              {
                number: 43,
                title: "  Valid PR  ",
                url: " https://github.com/collaborator/example-repo/pull/43 ",
                baseRefName: " main ",
  ...
  it.effect("creates draft pull requests when requested", () =>
    Effect.gen(function* () {
      ...
      yield* gh.createPullRequest({
        cwd: "/repo",
        baseBranch: "main",
        headSelector: "feature/automation",
  ...
  it.effect("creates repositories and parses clone URLs from create output", () =>
    Effect.gen(function* () {
      mockRun.mockReturnValueOnce(
        Effect.succeed(
          processOutput(
            "✓ Created repository octocat/example-repo on github.com\nhttps://github.com/octocat/example-repo\n",
          ),
        ),
      );
      ...
      expect(mockRun).toHaveBeenNthCalledWith(1, {
        operation: "GitHubCli.execute",
        command: "gh",
        args: ["repo", "create", "octocat/example-repo", "--private"],
        cwd: "/repo",
        timeoutMs: 30_000,
      });
    }).pipe(Effect.provide(layer)),
