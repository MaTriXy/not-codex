...
import { VcsRepositoryDetectionError } from "@notcodex/contracts";
...
it.effect("routes GitHub remotes to the GitHub provider", () =>
  Effect.gen(function* () {
    const registry = yield* makeRegistry({
      remotes: [{ name: "origin", url: "git@github.com:MaTriXy/not-codex.git" }],
    });
...
it.effect("refines the caller-selected remote instead of another configured remote", () =>
  Effect.gen(function* () {
    const registry = yield* makeRegistry({
...
it.effect("routes Bitbucket remotes to the Bitbucket provider", () =>
  Effect.gen(function* () {
    const registry = yield* makeRegistry({
      remotes: [{ name: "origin", url: "git@bitbucket.org:MaTriXy/not-codex.git" }],
    });
