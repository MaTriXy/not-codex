...
import { sanitizeBranchFragment } from "@notcodex/shared/git";
...
const BitbucketApiEnvConfig = Config.all({
  baseUrl: Config.string("NOT_CODEX_BITBUCKET_API_BASE_URL").pipe(
    Config.withDefault(DEFAULT_API_BASE_URL),
  ),
  accessToken: Config.string("NOT_CODEX_BITBUCKET_ACCESS_TOKEN").pipe(Config.option),
  email: Config.string("NOT_CODEX_BITBUCKET_EMAIL").pipe(Config.option),
  apiToken: Config.string("NOT_CODEX_BITBUCKET_API_TOKEN").pipe(Config.option),
});
...
>()("notcodex/sourceControl/BitbucketApi") {}
...
function checkoutBranchName(input: {
  ...
  return `notcodex/pr-${input.pullRequestId}/${sanitizeBranchFragment(input.headBranch)}`;
}
...
function authFromConfig(
  ...
  return {
    ...
    host: Option.some("bitbucket.org"),
    detail: Option.some(
      "Set NOT_CODEX_BITBUCKET_EMAIL and NOT_CODEX_BITBUCKET_API_TOKEN, or NOT_CODEX_BITBUCKET_ACCESS_TOKEN.",
    ),
  };
}
...
function responseError(
  ...
  return collectUint8StreamText({
    ...
    // Some platform stream adapters surface a ReadableStream failure as a defect. Normalize it
    // back into the HTTP client's typed decode failure so callers retain the immediate cause.
