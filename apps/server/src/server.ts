...
import {
  AutomationSchedulerLive,
  AutomationSchedulerRuntimeLive,
} from "./automation/Layers/AutomationScheduler.ts";
import {
...
import {
  ...
  LoopAnyConnectorRuntimeLive,
} from "./integrations/Layers/LoopAnyConnector.ts";
import {
...
const AutomationServiceLayerLive = AutomationServiceLive.pipe(
  Layer.provideMerge(AutomationRepositoryLive),
  Layer.provideMerge(AutomationEventBusLive),
);

const AutomationSchedulerLayerLive = AutomationSchedulerLive.pipe(
  Layer.provideMerge(AutomationRepositoryLive),
  Layer.provideMerge(AutomationServiceLayerLive),
);

const AutomationLayerLive = Layer.empty.pipe(
  Layer.provideMerge(AutomationRepositoryLive),
  Layer.provideMerge(AutomationEventBusLive),
  Layer.provideMerge(AutomationServiceLayerLive),
  Layer.provideMerge(AutomationSchedulerLayerLive),
);

const RuntimeCorePrimaryDependenciesLive = ReactorLayerLive.pipe(
  // Core Services
  ...
  // logger instances.
  // Legacy-model classification data, refreshed from Not Codex's hosted
  // `model-manifest.json` and applied by the Codex and Claude drivers.
  Layer.provideMerge(
    Layer.mergeAll(ProviderEventLoggers.ProviderEventLoggersLive, ModelManifest.layer),
  ),
...
// Keep the dependency composition split into bounded groups. Effect's typed
// `pipe` overload intentionally caps the number of operations in one call.
const RuntimeCoreBaseDependenciesLive = RuntimeCorePrimaryDependenciesLive.pipe(
...
const RuntimeCoreWithLoopAnyLive = LoopAnyConnectorLive.pipe(
...
const RuntimeCoreDependenciesLive = Layer.mergeAll(
  ...
  AutomationExecutorRuntimeLive,
  LoopAnyConnectorRuntimeLive,
).pipe(Layer.provideMerge(RuntimeCoreWithAutomationExecutorLive));
