...
import { HostProcessHostname, HostProcessPlatform } from "@notcodex/shared/hostProcess";
...
describe("resolveServerEnvironmentLabel", () => {
  ...
  it.effect("prefers the macOS ComputerName", () =>
    Effect.gen(function* () {
      runMock.mockReturnValueOnce(
        Effect.succeed({
          stdout: " Collaborator's MacBook Pro \n",
          stderr: "",
      ...
      expect(result).toBe("Collaborator's MacBook Pro");
  ...
  it.effect("falls back to the hostname when friendly labels are unavailable", () =>
    Effect.gen(function* () {
      const result = yield* ServerEnvironmentLabel.resolveServerEnvironmentLabel({
        cwdBaseName: "notcodex",
      }).pipe(Effect.provide(withHostPlatform(TestLayer, "win32", "COLLABORATOR-LAPTOP")));

      expect(result).toBe("COLLABORATOR-LAPTOP");
    }),
  );
