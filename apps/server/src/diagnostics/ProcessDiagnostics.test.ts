      ...
      const spawnerLayer = Layer.succeed(
        ...
        ChildProcessSpawner.make((command) => {
          ...
          return Effect.succeed(
            mockHandle({
              stdout: [
                ` ${process.pid}     1 ${process.pid} Ss 0.0 1024 01:02.03 notcodex server`,
  ...
  it.effect("does not allow signaling the diagnostics query process", () =>
    Effect.gen(function* () {
      ...
      expect(result).toEqual({
        ...
        signal: "SIGINT",
        signaled: false,
        message: Option.some("Process 4242 is not a live descendant of the Not Codex server."),
      });
    }),
  );
});
