...
// Matches the event store's default page size.
const SHELL_RESUME_MAX_GAP = 1_000;

// Thread replay reads a global event range and filters afterwards. A stale
// cursor can otherwise decode hundreds of thousands of unrelated payloads and
// exhaust memory; beyond this gap, converge from a fresh thread snapshot.
const THREAD_RESUME_MAX_GAP = 1_000;

const RPC_REQUIRED_SCOPE = new Map<string, AuthEnvironmentScope>([
  [AUTOMATION_WS_METHODS.listDefinitions, AuthOrchestrationReadScope],
  [AUTOMATION_WS_METHODS.getDefinition, AuthOrchestrationReadScope],
  [AUTOMATION_WS_METHODS.createDefinition, AuthOrchestrationOperateScope],
  [AUTOMATION_WS_METHODS.updateDefinition, AuthOrchestrationOperateScope],
  [AUTOMATION_WS_METHODS.deleteDefinition, AuthOrchestrationOperateScope],
  [AUTOMATION_WS_METHODS.runNow, AuthOrchestrationOperateScope],
  [AUTOMATION_WS_METHODS.cancelRun, AuthOrchestrationOperateScope],
  [AUTOMATION_WS_METHODS.retryRun, AuthOrchestrationOperateScope],
  [AUTOMATION_WS_METHODS.listRuns, AuthOrchestrationReadScope],
  ...
  [INTEGRATION_WS_METHODS.resumeRun, AuthOrchestrationOperateScope],
  [INTEGRATION_WS_METHODS.retryRun, AuthOrchestrationOperateScope],
...
const makeWsRpcLayer = (
  ...
  WsRpcGroup.toLayer(
    Effect.gen(function* () {
      ...
      const integrationService = Option.getOrElse(
        ...
        () =>
          IntegrationService.IntegrationService.of({
            ...
            resumeRun: () => Effect.fail(integrationUnavailable),
            retryRun: () => Effect.fail(integrationUnavailable),
          }),
      );
      ...
      // removal if the projection has no active row for it. This keeps a
      // coalesced delete/archive burst correct even when its final event would
      // normally be represented by a refetch.
      ...
      // Projection reads return current aggregate state, so only the latest
      // event per aggregate in a short window needs a refetch. Re-sort the
      // survivors by sequence before emitting because clients apply them in
      ...
      const automationRpcEffect = <A, E, R>(method: string, effect: Effect.Effect<A, E, R>) =>
        observeRpcEffect(
          method,
          effect.pipe(
            Effect.mapError((cause) =>
              isAutomationRequestError(cause)
                ? cause
                : new AutomationRequestError({
                    code: "automation-operation-failed",
                    message:
                      cause instanceof Error ? cause.message : "Automation operation failed.",
                  }),
            ),
          ),
      ...
      return WsRpcGroup.of({
        [AUTOMATION_WS_METHODS.listDefinitions]: (input) =>
          automationRpcEffect(
            AUTOMATION_WS_METHODS.listDefinitions,
            automationService.listDefinitions(input),
          ),
        [AUTOMATION_WS_METHODS.getDefinition]: (input) =>
          automationRpcEffect(
            AUTOMATION_WS_METHODS.getDefinition,
            automationService.getDefinition(input),
          ),
        [AUTOMATION_WS_METHODS.createDefinition]: (input) =>
          automationRpcEffect(
            AUTOMATION_WS_METHODS.createDefinition,
            automationService.createDefinition(input),
          ),
        [AUTOMATION_WS_METHODS.updateDefinition]: (input) =>
          automationRpcEffect(
            AUTOMATION_WS_METHODS.updateDefinition,
            automationService.updateDefinition(input),
          ),
        [AUTOMATION_WS_METHODS.deleteDefinition]: (input) =>
          automationRpcEffect(
            AUTOMATION_WS_METHODS.deleteDefinition,
            automationService.deleteDefinition(input),
          ),
        [AUTOMATION_WS_METHODS.runNow]: (input) =>
          automationRpcEffect(AUTOMATION_WS_METHODS.runNow, automationService.runNow(input)),
        [AUTOMATION_WS_METHODS.cancelRun]: (input) =>
          automationRpcEffect(AUTOMATION_WS_METHODS.cancelRun, automationService.cancelRun(input)),
        [AUTOMATION_WS_METHODS.retryRun]: (input) =>
          automationRpcEffect(AUTOMATION_WS_METHODS.retryRun, automationService.retryRun(input)),
        [AUTOMATION_WS_METHODS.listRuns]: (input) =>
          automationRpcEffect(AUTOMATION_WS_METHODS.listRuns, automationService.listRuns(input)),
        [AUTOMATION_WS_METHODS.getRun]: (input) =>
          automationRpcEffect(AUTOMATION_WS_METHODS.getRun, automationService.getRun(input)),
        [AUTOMATION_WS_METHODS.subscribe]: (_input) =>
          observeRpcStream(AUTOMATION_WS_METHODS.subscribe, automationService.changes, {
            "rpc.aggregate": "automation",
          }),
        [AUTOMATION_WS_METHODS.listTemplates]: (_input) =>
          automationRpcEffect(
        ...
        [INTEGRATION_WS_METHODS.listRuns]: (input) =>
          observeRpcEffect(INTEGRATION_WS_METHODS.listRuns, integrationService.listRuns(input), {
            "rpc.aggregate": "integrations",
          }),
        ...
        [INTEGRATION_WS_METHODS.resumeRun]: (input) =>
          observeRpcEffect(INTEGRATION_WS_METHODS.resumeRun, integrationService.resumeRun(input), {
            "rpc.aggregate": "integrations",
          }),
        [INTEGRATION_WS_METHODS.retryRun]: (input) =>
          observeRpcEffect(INTEGRATION_WS_METHODS.retryRun, integrationService.retryRun(input), {
            "rpc.aggregate": "integrations",
          }),
        ...
        [ORCHESTRATION_WS_METHODS.subscribeShell]: (input) =>
          observeRpcStreamEffect(
            ...
            Effect.gen(function* () {
              // Attach live delivery before loading a snapshot or draining a
              // replay so events published during either operation are buffered.
              // Coalescing prevents high-frequency activity from serializing a
              // database refetch for every event.
              ...
              const bufferedLiveStream = coalesceShellStream(Stream.fromQueue(liveBuffer));
              ...
              // replaying shell events after it. Stale or invalid cursors fall
              // back to a fresh bounded snapshot instead of causing an unbounded
              // replay. Overlapping replay/live events are deduped by sequence on
              // the client.
          ...
          observeRpcStreamEffect(
            ...
            Effect.gen(function* () {
              ...
              if (input.afterSequence !== undefined) {
                ...
                // A large gap or cursor ahead of authoritative state falls
                // through to the fresh-snapshot path below.
              }
