...
import { detectSourceControlProviderFromRemoteUrl } from "@notcodex/shared/sourceControl";
...
>()("notcodex/pullRequest/PullRequestService") {}
...
function toPullRequestError(
  operation: string,
): (error: PullRequestProviderError | PullRequestBusyError) => PullRequestError {
  return (error) =>
    error._tag === "PullRequestBusyError"
      ? error
...
export const make = Effect.gen(function* () {
  ...
  const refineUnknownProjectKinds = (
    ...
    for (const project of projects) {
      ...
      // Legacy identities have no canonical host until provider refinement.
  ...
  const viewerPermissionsOf = (project: SupportedProject, ref: PullRequestRef, operation: string) =>
    workBudget
      .runRead(
        project.host,
        project.api.getViewerPermissions({
    ...
    Effect.forEach(
      ...
      (host) =>
        Effect.flatMap(
          ...
          (now): Effect.Effect<ResolvedViewer, PullRequestBusyError> => {
            ...
            return Effect.firstSuccessOf(
              roots.map((cwd) => workBudget.runRead(host, api.getViewer({ cwd }))),
            ).pipe(
              Effect.map((viewer) => ({
                host,
                kind: api.kind,
              ...
              Effect.catch((error) =>
                error._tag === "PullRequestBusyError"
                  ? Effect.fail(error)
                  : Effect.succeed({ host, kind: api.kind, viewer: null, error }),
              ),
            );
          },
        ),
  ...
  const listUncached: PullRequestService["Service"]["list"] = (input) =>
    Effect.gen(function* () {
      ...
      ): Effect.Effect<RepositoryBatch, PullRequestBusyError> => {
        {
          ...
          return workBudget
            .runRead(
              project.host,
              project.api.listChangeRequests({
                cwd: project.project.workspaceRoot,
                repository: project.repository,
                host: project.host,
                state: input.state,
      ...
      ): Effect.Effect<ReadonlyArray<RepositoryBatch>, PullRequestBusyError> => {
        const first = chunk[0]!;
        ...
        return workBudget
          ...
          .pipe(
            Effect.flatMap((page) => {
              ...
              return Effect.forEach(
                chunk,
                (project): Effect.Effect<RepositoryBatch, PullRequestBusyError> => {
            ...
            Effect.catch((error) =>
              error._tag === "PullRequestBusyError" ? Effect.fail(error) : separately(),
            ),
          );
      };
      ...
      const reads: Array<Effect.Effect<ReadonlyArray<RepositoryBatch>, PullRequestBusyError>> =
    ...
    requireProject(input).pipe(
      Effect.flatMap((project) =>
        workBudget
          .runRead(
            project.host,
            project.api.getChangeRequestActivity({
              cwd: project.project.workspaceRoot,
              repository: project.repository,
              host: project.host,
              number: input.number,
          ...
          return workBudget
            ...
            .pipe(
              ...
              Effect.catch((error) =>
                error._tag === "PullRequestBusyError"
                  ? Effect.fail(error)
                  : Effect.succeed<ReadonlyArray<PullRequestDiffStat>>([]),
              ),
            );
        },
        { concurrency: REPOSITORY_CONCURRENCY },
