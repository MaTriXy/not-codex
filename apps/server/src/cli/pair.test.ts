  ...
  Effect.acquireUseRelease(
    Effect.callback<NodeHttp.Server>((resume) => {
      const server = NodeHttp.createServer((request, response) => {
        if (request.url === "/.well-known/notcodex/environment") {
...
describe("notcodex pair", () => {
  it.effect("mints a token and prints a QR pairing URL for a live server", () =>
    withDescriptorServer((origin) =>
      Effect.gen(function* () {
        const baseDir = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "notcodex-pair-test-"));
        ...
        assert.equal(credentials[0]?.label, "notcodex pair");
      }),
    ...
    withDescriptorServer((origin) =>
      Effect.gen(function* () {
        const baseDir = NodeFS.mkdtempSync(
          NodePath.join(NodeOS.tmpdir(), "notcodex-pair-dev-test-"),
        );
        const port = Number(new URL(origin).port);
  ...
  it.effect("directs to serve or connect when no server is running", () =>
    Effect.gen(function* () {
      const baseDir = NodeFS.mkdtempSync(
        NodePath.join(NodeOS.tmpdir(), "notcodex-pair-none-test-"),
      );
      ...
      assert.include(rendered, "No running Not Codex server found.");
      assert.include(rendered, "notcodex serve");
      assert.include(rendered, "notcodex connect");
    }).pipe(Effect.provide(NodeServices.layer)),
  );
  ...
  it.effect("ignores runtime state whose recorded pid is no longer alive", () =>
    withDescriptorServer((origin) =>
      Effect.gen(function* () {
        const baseDir = NodeFS.mkdtempSync(
          NodePath.join(NodeOS.tmpdir(), "notcodex-pair-pid-test-"),
        );
        const statePath = NodePath.join(baseDir, "userdata", "server-runtime.json");
  ...
  it.effect("ignores stale runtime state pointing at a dead server", () =>
    Effect.gen(function* () {
      const baseDir = NodeFS.mkdtempSync(
        NodePath.join(NodeOS.tmpdir(), "notcodex-pair-stale-test-"),
      );
      const statePath = NodePath.join(baseDir, "userdata", "server-runtime.json");
