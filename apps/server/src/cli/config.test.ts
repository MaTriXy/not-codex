...
const makeDesktopBootstrap = (
  ...
  noBrowser: true,
  port: 4888,
  notCodexHome: "/tmp/notcodex-bootstrap-home",
  host: "127.0.0.1",
...
it.layer(NodeServices.layer)("cli config resolution", (it) => {
  ...
  it.effect("falls back to effect/config values when flags are omitted", () =>
    Effect.gen(function* () {
      ...
      const baseDir = join(NodeOS.tmpdir(), "notcodex-cli-config-env-base");
          ...
          Layer.mergeAll(
            ConfigProvider.layer(
              ConfigProvider.fromEnv({
                env: {
                  NOT_CODEX_LOG_LEVEL: "Warn",
                  NOT_CODEX_MODE: "desktop",
                  NOT_CODEX_PORT: "4001",
                  NOT_CODEX_HOST: "0.0.0.0",
                  NOT_CODEX_HOME: baseDir,
                  VITE_DEV_SERVER_URL: "http://127.0.0.1:5173",
                  NOT_CODEX_NO_BROWSER: "true",
                  NOT_CODEX_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "false",
                  NOT_CODEX_LOG_WS_EVENTS: "true",
                },
              }),
            ),
            NetService.layer,
  ...
  it.effect("uses CLI flags when provided", () =>
    Effect.gen(function* () {
      const { join } = yield* Path.Path;
      const baseDir = join(NodeOS.tmpdir(), "notcodex-cli-config-flags-base");
            ...
            ConfigProvider.layer(
              ConfigProvider.fromEnv({
                env: {
                  ...
                  NOT_CODEX_PORT: "4001",
                  NOT_CODEX_HOST: "0.0.0.0",
                  NOT_CODEX_HOME: join(NodeOS.tmpdir(), "ignored-base"),
                  VITE_DEV_SERVER_URL: "http://127.0.0.1:5173",
                  NOT_CODEX_NO_BROWSER: "false",
                  NOT_CODEX_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "false",
                  NOT_CODEX_LOG_WS_EVENTS: "false",
                },
              }),
            ),
            NetService.layer,
  ...
  it.effect("preserves explicit false CLI boolean flags over env and bootstrap values", () =>
    Effect.gen(function* () {
      const { join } = yield* Path.Path;
      const baseDir = join(NodeOS.tmpdir(), "notcodex-cli-config-false-flags");
        ...
        Effect.provide(
          Layer.mergeAll(
            ConfigProvider.layer(
              ConfigProvider.fromEnv({
                env: {
                  NOT_CODEX_BOOTSTRAP_FD: String(fd),
                  NOT_CODEX_NO_BROWSER: "true",
                  NOT_CODEX_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "true",
                  NOT_CODEX_LOG_WS_EVENTS: "true",
                },
              }),
            ),
  ...
  it.effect("uses bootstrap envelope values as fallbacks when flags and env are absent", () =>
    Effect.gen(function* () {
      const { join } = yield* Path.Path;
      const baseDir = "/tmp/notcodex-bootstrap-home";
      const fd = yield* openBootstrapFd(
        makeDesktopBootstrap({
          port: 4888,
          host: "127.0.0.2",
          notCodexHome: baseDir,
          noBrowser: true,
  ...
  it.effect("creates derived runtime directories during config resolution", () =>
    Effect.gen(function* () {
      ...
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "notcodex-cli-config-dirs-" });
  ...
  it.effect("applies flag then env precedence over bootstrap envelope values", () =>
    Effect.gen(function* () {
      const { join } = yield* Path.Path;
      const baseDir = join(NodeOS.tmpdir(), "notcodex-cli-config-env-wins");
      const fd = yield* openBootstrapFd(
        makeDesktopBootstrap({
          port: 4888,
          host: "127.0.0.2",
  ...
  it.effect("falls back to persisted observability settings when env vars are absent", () =>
    Effect.gen(function* () {
      ...
      const baseDir = yield* fs.makeTempDirectoryScoped({
        prefix: "notcodex-cli-config-settings-",
      });
      const derivedPaths = yield* deriveServerPaths(baseDir, undefined);
  ...
  it.effect("forces noBrowser and disables auto-bootstrap for headless startup presentation", () =>
    Effect.gen(function* () {
      const { join } = yield* Path.Path;
      const baseDir = join(NodeOS.tmpdir(), "notcodex-cli-config-headless-base");
      const derivedPaths = yield* deriveServerPaths(baseDir, undefined);
