...
it.effect("restarts a current boot service so a new link intent is reconciled", () => {
  let restartCalls = 0;
  const service = BootService.BootService.of({
    ...
    status: Effect.succeed({
      ...
      installed: true,
      current: true,
      unitPath: "/home/test/.config/systemd/user/notcodex.service",
      logPath: "/home/test/.notcodex/userdata/logs/boot-service.log",
    }),
  });

  return offerBootService.pipe(
