  ...
  Command.withHandler((flags) =>
    runCloudCommand(
      ...
      Effect.gen(function* () {
        const tokens = yield* CliTokenManager.CloudCliTokenManager;
        yield* tokens.get;
        yield* Console.log("Signed in to Not Codex Connect.");
      }),
    ),
  ),
);
      ...
      Effect.gen(function* () {
        ...
        if (!flags.publishOnly) {
          ...
          if (Option.isNone(installed)) {
            yield* Console.log(
              "Not Codex Connect setup cancelled. The relay client was not installed.",
            );
            return;
          }
          yield* Console.log(
        ...
        yield* Console.log(
          flags.publishOnly
            ? "This environment will publish agent activity to your mobile clients the next time Not Codex starts (no managed tunnel)."
            : "This Not Codex environment will be available through Not Codex Connect the next time Not Codex starts.",
        );
      }),
    ),
  ),
...
export const connectCommand = Command.make("connect").pipe(
  Command.withDescription("Manage headless Not Codex Connect access."),
