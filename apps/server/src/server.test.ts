...
import {
  ...
  type DpopPublicJwk,
} from "@notcodex/shared/dpop";
import { RELAY_HEALTH_REQUEST_TYP, RELAY_MINT_REQUEST_TYP } from "@notcodex/shared/relayJwt";
  ...
  Effect.gen(function* () {
    ...
    const runtime = ManagedRuntime.make(
      OtlpTracer.layer({
        ...
        resource: {
          serviceName: "notcodex-web",
          attributes: {
            "service.runtime": "notcodex-web",
...
const buildAppUnderTest = (options?: {
  ...
  Effect.gen(function* () {
    ...
    const config: ServerConfig.ServerConfig["Service"] = {
      ...
      otlpServiceName: "notcodex-server",
      mode: "desktop",
      port: 0,
      host: "127.0.0.1",
      cwd: process.cwd(),
    ...
    const automationServiceLayer = Layer.succeed(
      AutomationService.AutomationService,
      AutomationService.AutomationService.of({
        listDefinitions: () => Effect.succeed([]),
        getDefinition: () => Effect.succeed(null),
        createDefinition: () => Effect.die("AutomationService.createDefinition not stubbed"),
        updateDefinition: () => Effect.die("AutomationService.updateDefinition not stubbed"),
        deleteDefinition: () => Effect.die("AutomationService.deleteDefinition not stubbed"),
        runNow: () => Effect.die("AutomationService.runNow not stubbed"),
        cancelRun: () => Effect.die("AutomationService.cancelRun not stubbed"),
        retryRun: () => Effect.die("AutomationService.retryRun not stubbed"),
        listRuns: () => Effect.succeed([]),
        getRun: () => Effect.succeed(null),
        listTemplates: () => Effect.succeed([]),
        enqueueDefinition: () => Effect.die("AutomationService.enqueueDefinition not stubbed"),
        advanceSchedule: () => Effect.die("AutomationService.advanceSchedule not stubbed"),
        updateRun: () => Effect.die("AutomationService.updateRun not stubbed"),
        appendRunEvent: () => Effect.die("AutomationService.appendRunEvent not stubbed"),
    ...
    const servedRoutesBaseLayer = HttpRouter.serve(
      ...
      Layer.provide(
        Layer.mock(ServerSettings.ServerSettingsService)({
          ...
          streamChanges: Stream.empty,
          subscribeChanges: Effect.flatMap(PubSub.unbounded<never>(), PubSub.subscribe),
    ...
    const appLayer = servedRoutesLayer.pipe(
      ...
      Layer.provide(
        Layer.mock(CloudCliTokenManager.CloudCliTokenManager)({
          get: Effect.die(new Error("Unexpected Not Codex Connect CLI authorization request.")),
...
const makeCloudMintCredentialRequest = (input: {
  ...
  const payload = {
    ...
    aud: input.audience ?? `notcodex-env:${input.environmentId}`,
...
it.layer(NodeServices.layer)("server router seam", (it) => {
  ...
  it.effect("serves the public environment descriptor without requiring auth", () =>
    Effect.gen(function* () {
      ...
      const url = yield* getHttpServerUrl("/.well-known/notcodex/environment");
  ...
  it.effect("reports unauthenticated session state without requiring auth", () =>
    Effect.gen(function* () {
      ...
      assert.isTrue(body.auth.sessionCookieName.startsWith("notcodex_session_"));
    }).pipe(Effect.provide(NodeHttpServer.layerTest)),
  );
      ...
      Effect.gen(function* () {
        ...
        const tokenResponse = yield* fetchEffect(tokenUrl, {
          ...
          body: new URLSearchParams({
            ...
            subject_token_type: "urn:notcodex:params:oauth:token-type:environment-bootstrap",
      ...
      const linkProofResponse = yield* fetchEffect(linkProofUrl, {
        ...
        body: jsonRequestBody({
          ...
          endpoint: {
            ...
            // "manual" and "cloudflare_tunnel" are supported; "notcodex_relay" is not.
            providerKind: "notcodex_relay",
          },
          origin: {
    ...
    () =>
      Effect.gen(function* () {
        const installedRelayClient = {
          ...
          executablePath: "/tmp/notcodex/tools/cloudflared",
    ...
    Effect.gen(function* () {
      ...
      const relayConfigResponse = yield* fetchEffect(relayConfigUrl, {
        ...
        body: jsonRequestBody({
          ...
          environmentCredential: "notcodexenv_test_credential",
  ...
  it.effect("rejects relay config replacement from a different cloud account", () =>
    Effect.gen(function* () {
      ...
      const firstResponse = yield* postRelayConfig("user_123", "notcodexenv_first_credential");
      const replacementResponse = yield* postRelayConfig(
        "user_456",
        "notcodexenv_second_credential",
      );
  ...
  it.effect("serves the documented Not Codex Connect mint credential endpoint", () =>
    Effect.gen(function* () {
      yield* buildAppUnderTest();
      ...
      const mintUrl = yield* getHttpServerUrl("/api/not-codex-connect/mint-credential");
  ...
  it.effect("serves signed Not Codex Connect environment health checks", () =>
    Effect.gen(function* () {
      yield* buildAppUnderTest();
      ...
      const healthUrl = yield* getHttpServerUrl("/api/not-codex-connect/health");
    ...
    Effect.gen(function* () {
      ...
      const wrongAudience = yield* postMint(
        makeCloudMintCredentialRequest({
          ...
          environmentId: testEnvironmentDescriptor.environmentId,
          clientProofKeyThumbprint: "client-proof-key-thumbprint",
          audience: "notcodex-env:other-environment",
  ...
  for (const desktopOrigin of ["notcodex://app", "notcodex-dev://app"]) {
  ...
  it.effect("lists paired clients and revokes other sessions while keeping the administrator", () =>
    Effect.gen(function* () {
      ...
      assert.equal(ownerPairingBody.label, "Collaborator iPhone");
        ...
        resourceSpans: [
          {
            resource: {
              attributes: [
                {
                  key: "service.name",
                  value: { stringValue: "notcodex-web" },
                },
              ],
            },
            scopeSpans: [
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        assert.equal(record.resourceAttributes["service.name"], "notcodex-web");
  ...
  it.effect("rejects websocket rpc handshake when session authentication is missing", () =>
    Effect.gen(function* () {
      ...
      const workspaceDir = yield* fs.makeTempDirectoryScoped({
        prefix: "notcodex-ws-auth-required-",
      });
  ...
  it.effect("routes websocket rpc projects.searchEntries", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const workspaceDir = yield* fs.makeTempDirectoryScoped({
        prefix: "notcodex-ws-project-search-",
      });
      yield* fs.writeFileString(
        path.join(workspaceDir, "needle-file.ts"),
        "export const needle = 1;",
  ...
  it.effect("routes websocket rpc projects.listEntries and projects.readFile", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const workspaceDir = yield* fs.makeTempDirectoryScoped({
        prefix: "notcodex-ws-project-files-",
      });
  ...
  it.effect("routes websocket rpc projects.searchEntries excludes gitignored files", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const workspaceDir = yield* fs.makeTempDirectoryScoped({
        prefix: "notcodex-ws-project-search-gitignored-",
      });
  ...
  it.effect("preserves structured workspace rpc failures", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const workspaceDir = yield* fs.makeTempDirectoryScoped({
        prefix: "notcodex-ws-workspace-errors-",
      });
      const outsideDir = yield* fs.makeTempDirectoryScoped({
        prefix: "notcodex-ws-workspace-errors-outside-",
      });
  ...
  it.effect("reports workspace root stat failures without relabeling them as missing", () =>
    Effect.gen(function* () {
      ...
      const blockedRoot = yield* fs.makeTempDirectoryScoped({
        prefix: "notcodex-ws-workspace-stat-error-",
      });
  ...
  it.effect("routes websocket rpc projects.writeFile", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const workspaceDir = yield* fs.makeTempDirectoryScoped({
        prefix: "notcodex-ws-project-write-",
      });
  ...
  it.effect("creates a missing workspace root during websocket project.create dispatch", () =>
    Effect.gen(function* () {
      ...
      const parentDir = yield* fs.makeTempDirectoryScoped({
        prefix: "notcodex-ws-project-create-",
      });
  ...
  it.effect("buffers shell events published while the initial snapshot loads", () =>
    Effect.gen(function* () {
      ...
      const removedThreadId = ThreadId.make("thread-removed-during-snapshot");
      const deletedEvent = {
        sequence: 2,
        eventId: EventId.make("event-shell-delete"),
      ...
      assert.equal(items[1]?.kind === "thread-removed" ? items[1].threadId : null, removedThreadId);
    }).pipe(Effect.provide(NodeHttpServer.layerTest), TestClock.withLive),
  );
  ...
  it.effect("subscribeShell sends a fresh snapshot instead of replaying a large gap", () =>
    Effect.gen(function* () {
      ...
      const snapshotThreadId = ThreadId.make("thread-from-shell-snapshot");
      ...
      const first = yield* Effect.scoped(
        withWsRpcClient(wsUrl, (client) =>
          client[ORCHESTRATION_WS_METHODS.subscribeShell]({ afterSequence: 5 }).pipe(
            Stream.runHead,
          ),
        ),
      );
      ...
      if (item.kind === "snapshot") {
        assert.equal(item.snapshot.threads[0]?.id, snapshotThreadId);
      }
      assert.equal(readEventsCalls, 0);
    }).pipe(Effect.provide(NodeHttpServer.layerTest)),
  );

  it.effect("subscribeShell coalesces a per-thread replay burst", () =>
    Effect.gen(function* () {
      ...
      const event = (
        threadId: ThreadId,
        sequence: number,
        type: "thread.message-sent" | "thread.created",
      ) =>
        ({
          sequence,
            ...
            readEvents: (_afterSequence, limit) => {
              ...
              return Stream.fromIterable([
                ...Array.from({ length: 20 }, (_unused, index) =>
                  event(busyThreadId, index + 1, "thread.message-sent"),
                ),
                event(newThreadId, 50, "thread.created"),
              ]);
            },
          },
          projectionSnapshotQuery: {
      ...
      const upsertedIds = Array.from(items).flatMap((item) =>
  ...
  it.effect("subscribeThread replaces a large replay gap with a fresh snapshot", () =>
    Effect.gen(function* () {
      let readEventsCalls = 0;
...
it.live(
  ...
  () =>
    Effect.gen(function* () {
            ...
            (harness) =>
              Effect.gen(function* () {
                ...
                const seededSnapshot =
                  yield* harness.snapshotQuery.getThreadDetailSnapshot(TRANSFER_THREAD_ID);
                assert.isTrue(Option.isSome(seededSnapshot));
                yield* buildAppUnderTest({
                  layers: {
                ...
                return yield* Effect.scoped(
                  Effect.gen(function* () {
                    ...
                    const initialThreadItems = yield* collectQueueUntil(
                      ...
                      `${provider} thread subscription snapshot`,
                    );
                    assert.isTrue(initialThreadItems.some((item) => item.kind === "snapshot"));
      ...
      const reportPath = yield* Config.string("NOT_CODEX_TRANSFER_BUDGET_REPORT_PATH").pipe(
        Config.option,
      );
      ...
      const resultPath = yield* Config.string("NOT_CODEX_TRANSFER_BUDGET_RESULT_PATH").pipe(
        Config.option,
      );
