...
export const make = Effect.fn("NodePtyAdapter.make")(function* (
  ...
  return PtyAdapter.PtyAdapter.of({
    spawn: Effect.fn("NodePtyAdapter.spawn")(function* (input) {
      ...
      // node-pty only applies `name` to TERM on Unix. ConPTY leaves the
      // environment untouched, so provide a 256-color default on Windows
      // without overriding an explicit caller preference.
      const env =
