...
export function formatLoopbackAuthorizationPrompt(authorizationUrl: string): string {
  return [
    "Open this URL to authorize Not Codex Connect:",
    `  ${authorizationUrl}`,
    "",
...
const readLoopbackAuthorizationAction = Effect.fn(
  ...
  while (true) {
    // EOF means terminal hotkeys are unavailable (for example redirected
    // stdin), not that browser authorization was cancelled. Park this branch
    // so the loopback callback can still win the race.
    const event = yield* Queue.take(input).pipe(Effect.catch(() => Effect.never));
    const keyName = event.key.name.toLowerCase();
    if (event.key.ctrl && !event.key.meta && keyName === "c") {
...
export const waitForLoopbackAuthorization = Effect.fn(
  ...
  return yield* Effect.scoped(
    Effect.gen(function* () {
      // Terminal input is an optional convenience. Capture acquisition defects
      // and disable only the hotkey branch so non-interactive callers can
      // still finish through the browser callback.
      const terminalInput = yield* Effect.exit(input.terminal.readInput);
      const terminalActions = Exit.isSuccess(terminalInput)
        ? Effect.gen(function* () {
            while (true) {
              const action = yield* readLoopbackAuthorizationAction(terminalInput.value);
              if (action === "headless") {
      ...
      return yield* Effect.raceFirst(
        input.callback.pipe(
          Effect.map((code): LoopbackAuthorizationResult => ({ _tag: "AuthorizationCode", code })),
        ),
        terminalActions,
      );
    }),
...
export class CloudCliCredentialRemovalError extends Schema.TaggedErrorClass<CloudCliCredentialRemovalError>()(
  ...
  override get message(): string {
    return "Could not remove the stored Not Codex Connect CLI credential.";
  }
}
...
export class CloudCliCredentialRefreshError extends Schema.TaggedErrorClass<CloudCliCredentialRefreshError>()(
  ...
  override get message(): string {
    return "Could not refresh the Not Codex Connect CLI credential.";
  }
}
...
export class CloudCliCredentialReadError extends Schema.TaggedErrorClass<CloudCliCredentialReadError>()(
  ...
  override get message(): string {
    return "Could not read the stored Not Codex Connect CLI credential.";
  }
}
...
export class CloudCliAuthorizationError extends Schema.TaggedErrorClass<CloudCliAuthorizationError>()(
  ...
  override get message(): string {
    return "Could not authorize the Not Codex Connect CLI.";
  }
}
...
export class CloudCliAuthorizationTimeoutError extends Schema.TaggedErrorClass<CloudCliAuthorizationTimeoutError>()(
  ...
  override get message(): string {
    return "Timed out waiting for Not Codex Connect authorization.";
  }
}
...
>()("notcodex/cloud/CliTokenManager/CloudCliTokenManager") {}
    ...
    const callbackRoute = HttpRouter.add(
      ...
      Effect.gen(function* () {
        ...
        if (url.searchParams.get("state") !== state || !code) {
          return HttpServerResponse.text("Invalid Not Codex Connect authorization callback.", {
            status: 400,
          });
        }
