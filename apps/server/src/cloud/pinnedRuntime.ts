...
/**
 * An exact `notcodex@<version>` npm install under
 * <baseDir>/runtime/versions/<version>. Background services and remote updates
 * use these durable runtimes instead of an ephemeral npx/pnpm/bunx cache.
 */
...
export function pinnedRuntimePaths(
  ...
  return {
    versionDir,
    entryPath: path.join(versionDir, "node_modules", "notcodex", "dist", "bin.mjs"),
...
export interface PinnedRuntimeInstallInput {
...
const installPinnedRuntime = Effect.fn("cloud.pinned_runtime.ensure_installed")(function* (
  ...
  const stagingPaths: PinnedRuntimePaths = {
    ...
    entryPath: input.path.join(stagingDir, "node_modules", "notcodex", "dist", "bin.mjs"),
    sentinelPath: input.path.join(stagingDir, ".install-complete"),
  };

  return yield* Effect.gen(function* () {
    const installStep = "installing the pinned Not Codex runtime (this can take a few minutes)";
    yield* runner
      .run({
        command: "npm",
        args: [
