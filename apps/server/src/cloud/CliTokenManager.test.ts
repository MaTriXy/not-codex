import { readConnectAuthorizeRequest } from "@notcodex/shared/connectAuth";
...
const TEST_ENV = {
  NOT_CODEX_CLERK_PUBLISHABLE_KEY: "pk_test_Y2xlcmsuZXhhbXBsZS50ZXN0JA==",
  NOT_CODEX_CLERK_CLI_OAUTH_CLIENT_ID: "oauth_client_test",
  NOT_CODEX_HOSTED_APP_URL: "https://hosted.example.test",
};
...
const makeTestTerminal = (queue: Queue.Queue<Terminal.UserInput, Cause.Done>) =>
  Terminal.make({
...
const userInput = (name: string, options?: { readonly ctrl?: boolean }): Terminal.UserInput => ({
...
it.effect("cancels loopback authorization when the user presses Ctrl-C", () =>
  Effect.gen(function* () {
    const queue = yield* Queue.make<Terminal.UserInput, Cause.Done>();
    yield* Queue.offer(queue, userInput("c", { ctrl: true }));

    const error = yield* CliTokenManager.waitForLoopbackAuthorization({
      authorizationUrl: "https://clerk.example.test/authorize",
      callback: Effect.never,
      terminal: makeTestTerminal(queue),
      launchBrowser: () => Effect.die("browser launch should not run"),
    ...
    assert.isTrue(Terminal.isQuitError(error));
  }),
);

it.effect("keeps waiting for the callback after terminal input reaches EOF", () =>
  Effect.gen(function* () {
    const queue = yield* Queue.make<Terminal.UserInput, Cause.Done>();
    yield* Queue.end(queue);
    const callback = yield* Deferred.make<string>();

    const waiting = yield* CliTokenManager.waitForLoopbackAuthorization({
      authorizationUrl: "https://clerk.example.test/authorize",
      callback: Deferred.await(callback),
      terminal: makeTestTerminal(queue),
      launchBrowser: () => Effect.die("browser launch should not run"),
    ...
    yield* Deferred.succeed(callback, "clerk-code-after-eof");
    const result = yield* Fiber.join(waiting);

    assert.deepEqual(result, { _tag: "AuthorizationCode", code: "clerk-code-after-eof" });
  }),
);

it.effect("keeps waiting for the callback when terminal input cannot be acquired", () =>
  Effect.gen(function* () {
    const terminal = Terminal.make({
      columns: Effect.succeed(80),
      rows: Effect.succeed(24),
      readInput: Effect.die("stdin unavailable"),
      readLine: Effect.never,
      display: () => Effect.void,
    });
    ...
    const result = yield* CliTokenManager.waitForLoopbackAuthorization({
      authorizationUrl: "https://clerk.example.test/authorize",
      callback: Effect.succeed("clerk-code-without-stdin"),
      terminal,
      launchBrowser: () => Effect.die("browser launch should not run"),
    });

    assert.deepEqual(result, { _tag: "AuthorizationCode", code: "clerk-code-without-stdin" });
  }),
);
