...
export type ServicePreflightResult =
  ...
  | { readonly status: "blocked"; readonly version: string; readonly reason: string };
...
export function runServicePreflight(input: {
  ...
  if (input.launcherProtocol !== SERVICE_LAUNCHER_PROTOCOL) {
    return {
      ...
      version,
      reason:
        "This release requires a newer Not Codex service launcher. Update it on the server machine.",
    };
  }
