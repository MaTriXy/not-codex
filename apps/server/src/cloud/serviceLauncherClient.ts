  ...
  override get message(): string {
    switch (this.operation) {
      ...
      case "version-mismatch":
        return "The service launcher started a different Not Codex version.";
...
export class ServiceLauncherRejectedError extends Schema.TaggedErrorClass<ServiceLauncherRejectedError>()(
  ...
  { targetVersion: Schema.String, reason: Schema.String },
) {
  override get message(): string {
    return this.reason;
  }
...
export const ServiceLauncherHostProcess = Context.Reference<ServiceLauncherProcess>(
  "notcodex/cloud/serviceLauncherHostProcess",
  {
    defaultValue: () => ({
...
>()("notcodex/cloud/serviceLauncherClient") {}
...
export const make = Effect.fn("cloud.service_launcher_client.make")(function* (options?: {
      ...
      ? exchange(
          ...
          Effect.flatMap((reply) =>
            reply.type !== "committed"
              ? Effect.die("service launcher returned an impossible prepared response")
              : Effect.succeed({
                  id: pending.id,
  ...
  return ServiceLauncherClient.of({ managed, requestUpdate, prepareTrial });
});
