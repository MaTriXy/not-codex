...
it.effect("launches an installed editor with platform-safe arguments", () =>
  Effect.gen(function* () {
    ...
    const binDir = yield* fileSystem.makeTempDirectoryScoped({ prefix: "notcodex-editors-" });
...
// oxlint-disable-next-line notcodex/no-global-process-runtime -- the skip decision needs the real host platform, outside any Effect runtime.
it.skipIf(process.platform !== "win32")(
  "delivers the raw /select switch for spaced paths through real PowerShell",
  { timeout: 60_000 },
  async () => {
    const tempDir = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "notcodex-reveal-smoke-"));
    try {
...
it.effect("reveals a WSL file in Windows File Explorer through its UNC path", () =>
  Effect.gen(function* () {
    ...
    const result = yield* Effect.gen(function* () {
      ...
      yield* launcher.launchEditor({
        editor: "file-manager",
        cwd: "/home/notcodex/workspace/media/clip.mp4",
        reveal: true,
      });
      return { kind, editors };
    }).pipe(
    ...
    assert.equal(
      decodedCommand,
      "$ProgressPreference = 'SilentlyContinue'; Start-Process 'explorer.exe' -ArgumentList ('/select,\"' + '\\\\wsl.localhost\\Ubuntu-24.04\\home\\notcodex\\workspace\\media\\clip.mp4' + '\"')",
    );
  }).pipe(Effect.scoped, Effect.provide(NodeServices.layer)),
);
...
it.effect("reveals through the Linux file manager when WSL lacks interop PowerShell", () =>
  Effect.gen(function* () {
    ...
    assert.ok(launch);
    assert.deepEqual(launch.args, ["/home/notcodex/workspace/media"]);
  ...
  () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        yield* launcher.launchEditor({
          editor: "file-manager",
          cwd: '/home/notcodex/work "quoted"/clip.mp4',
          reveal: true,
        });
      }).pipe(
        Effect.provide(
      ...
      Layer.mergeAll(
        ...
        ConfigProvider.layer(
          ConfigProvider.fromEnv({
            env: {
              PATH: "C:\\notcodex-editor-discovery-cache-test",
              PATHEXT: ".COM;.EXE;.BAT;.CMD",
            },
          }),
        ),
    ...
    Effect.provide(
      Layer.mergeAll(
        ...
        ConfigProvider.layer(
          ConfigProvider.fromEnv({
            env: {
              PATH: "C:\\notcodex-editor-discovery-interrupt-test",
              PATHEXT: ".COM;.EXE;.BAT;.CMD",
            },
          }),
        ),
