...
/** Rows without a finite, non-negative token count cannot replace a valid
 * context-window reading because consumers skip them while walking backward. */
...
// Clients consume only the latest valid context-window value per turn. Keeping
// thousands of superseded rows in snapshots wastes transport and renderer heap.
// Retention remains per turn so reverting newer turns can reveal the prior value.
