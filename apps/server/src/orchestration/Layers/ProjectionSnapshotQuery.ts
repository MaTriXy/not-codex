...
// Match the in-memory projector's retained activity window without decoding an
// unbounded set of activity payloads for a detail read.
...
const makeProjectionSnapshotQuery = Effect.gen(function* () {
  ...
  const listTurnWindowRows = SqlSchema.findAll({
    ...
    execute: ({ threadId, beforeAnchorAt, beforeTurnKey, userTurnLimit, maxRawTurns }) =>
      sql`
        ...
        SELECT anchor_at AS "anchorAt", turn_key AS "turnKey"
        FROM walked
  ...
  // Request cards must remain actionable even when their activity falls
  // outside the retained recent window.
            ...
            : repositoryIdentityResolver.resolve(option.value.workspaceRoot).pipe(
                Effect.map((repositoryIdentity) =>
                  Option.some({
                    ...
                    faviconPath: option.value.faviconPath,
  ...
  const getThreadDetailByIdBounded = (threadId: ThreadId, bounds?: ThreadDetailBounds) =>
    Effect.gen(function* () {
      const [
        threadRow,
        messageRows,
  ...
  const getThreadDetailById: ProjectionSnapshotQueryShape["getThreadDetailById"] = (threadId) =>
    getThreadDetailByIdBounded(threadId);
