 ...
 * ~/.notcodex database, then run this checkout's migrations against it.
 *
 * `vp run migrate-dev-db` from a worktree:
 *   1. Nukes `<worktree>/.notcodex/userdata/state.sqlite`.
 ...
 *      threads that have fully stopped. Working and settled threads are
...
// @effect-diagnostics nodeBuiltinImport:off - node:os resolves the shared Not Codex home guard.
...
import { migrationEntries, runMigrations } from "../src/persistence/Migrations.ts";
...
export class MigrateDevDbNotInWorktreeError extends Schema.TaggedErrorClass<MigrateDevDbNotInWorktreeError>()(
  ...
  override get message(): string {
    return "Not inside a linked git worktree. Pass --base-dir to target an isolated .notcodex directory.";
  }
}
...
export class MigrateDevDbSharedHomeError extends Schema.TaggedErrorClass<MigrateDevDbSharedHomeError>()(
  ...
  override get message(): string {
    return "Refusing to rebuild the shared ~/.notcodex database. Use an isolated --base-dir.";
  }
}
...
export class MigrateDevDbServerRunningError extends Schema.TaggedErrorClass<MigrateDevDbServerRunningError>()(
  ...
  override get message(): string {
    return `Dev database at '${this.databasePath}' is open by a running server (pid ${this.pid} per server-runtime.json). Stop that server first; if that pid is not actually a Not Codex server (stale descriptor, reused pid), delete the server-runtime.json next to the database and retry.`;
  }
}
...
export interface RunMigrateDevDbInput {
  /** Isolated .notcodex directory. Defaults to `<worktree>/.notcodex` of the cwd. */
  readonly baseDir?: string | undefined;
  /** Source database. Defaults to `~/.notcodex/userdata/state.sqlite`. */
...
export interface RunMigrateDevDbOptions {
  ...
  /** Overridable for tests; defaults to the host process working directory. */
...
const pruneSnapshot = Effect.fn("pruneDevDbSnapshot")(function* (input: RunMigrateDevDbInput) {
  ...
  // reached status 'stopped' and nothing marks the thread settled. In-memory
  // working liveness never persists, so filtering the session is sufficient.
  ...
  yield* sql.withTransaction(
    Effect.gen(function* () {
      // Never adopt runnable automation, integration, or connector state from
      // the user's live app. These tables are operational state, not fixtures.
      ...
      yield* sql`DELETE FROM open_kritt_findings`;
      yield* sql`DELETE FROM open_kritt_scan_snapshots`;
      yield* sql`DELETE FROM open_kritt_scan_correlations`;
      yield* sql`DELETE FROM open_kritt_diagnostics`;
...
const verifyMigrationSlots = Effect.fn("verifyMigrationSlots")(function* () {
  ...
  for (const [slot, codeName] of migrationEntries) {
...
export const runMigrateDevDb = Effect.fn("runMigrateDevDb")(function* (
  ...
  const sharedHome = path.resolve(options.sharedHome ?? path.join(NodeOS.homedir(), ".notcodex"));
  const configuredSource = input.source?.trim();
  const sourcePath = path.resolve(
    configuredSource || path.join(sharedHome, "userdata", "state.sqlite"),
  );

  const configuredBaseDir = input.baseDir?.trim();
  const baseDir =
    configuredBaseDir !== undefined && configuredBaseDir.length > 0
      ? path.resolve(configuredBaseDir)
      : yield* resolveWorktreeNotCodexHome(options.workingDirectory ?? process.cwd());
...
export const migrateDevDbCommand = Command.make(
  "migrate-dev-db",
  {
    ...
    baseDir: Flag.string("base-dir").pipe(
      ...
      Flag.withDescription(
        "Isolated .notcodex directory. Defaults to the current worktree's .notcodex.",
      ),
    ),
    source: Flag.string("source").pipe(
      Flag.optional,
      Flag.withDescription("Source database. Defaults to ~/.notcodex/userdata/state.sqlite."),
    ),
  },
  ...
  Command.withDescription(
    "Rebuild the worktree dev database from a pruned snapshot of the real ~/.notcodex data, then run migrations.",
  ),
);
