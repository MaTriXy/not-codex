...
import { fromJsonStringPretty } from "@notcodex/shared/schemaJson";
...
export class SqliteStateDatabaseMissingError extends Schema.TaggedErrorClass<SqliteStateDatabaseMissingError>()(
  ...
  override get message(): string {
    return `Database does not exist at '${this.databasePath}'. Start Not Codex once to run migrations.`;
  }
}
...
export class SqliteStateSharedHomeMutationError extends Schema.TaggedErrorClass<SqliteStateSharedHomeMutationError>()(
  ...
  override get message(): string {
    return "Refusing to mutate the shared ~/.notcodex database. Use an isolated --base-dir.";
  }
}
...
export const runSqliteState = Effect.fn("runSqliteState")(function* (
  ...
  const program = Effect.gen(function* () {
    ...
    if (input.operation === "query") {
      ...
      return { operation: "query", database: databasePath, rows } as const;
    }
    ...
    return { operation: "exec", database: databasePath, backup: backupPath } as const;
  });
...
export const notCodexSqliteStateCommand = Command.make(
  "notcodex-sqlite-state",
  {
    ...
    baseDir: Flag.string("base-dir").pipe(
      Flag.withDescription("Explicit Not Codex base directory containing userdata/state.sqlite."),
    ),
  ...
  Command.withDescription(
    "Inspect or seed an isolated Not Codex SQLite database with automatic backups for writes.",
  ),
);

if (import.meta.main) {
  Command.run(notCodexSqliteStateCommand, { version: "0.0.0" }).pipe(
