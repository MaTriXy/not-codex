...
export const DEFAULT_NOT_CODEX_HOME = Effect.map(Effect.service(Path.Path), (path) =>
  path.join(NodeOS.homedir(), ".notcodex"),
);

const MODE_ARGS = {
  dev: [
    "run",
    "--filter=@notcodex/contracts",
    "--filter=@notcodex/web",
    "--filter=notcodex",
    "--parallel",
    "dev",
  ],
  "dev:server": ["run", "--filter=notcodex", "dev"],
  "dev:web": ["run", "--filter=@notcodex/web", "dev"],
  "dev:desktop": ["run", "--filter=@notcodex/desktop", "--filter=@notcodex/web", "dev"],
...
export class DevRunnerInvalidPortOffsetError extends Schema.TaggedErrorClass<DevRunnerInvalidPortOffsetError>()(
  ...
  {
    configKey: Schema.Literal("NOT_CODEX_PORT_OFFSET"),
...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("NOT_CODEX_PORT_OFFSET"),
  devInstance: optionalStringConfig("NOT_CODEX_DEV_INSTANCE"),
});
...
> {
  if (config.portOffset !== undefined) {
    if (config.portOffset < 0) {
      return Effect.fail(
        new DevRunnerInvalidPortOffsetError({
          configKey: "NOT_CODEX_PORT_OFFSET",
    ...
    return Effect.succeed({
      ...
      source: `NOT_CODEX_PORT_OFFSET=${config.portOffset}`,
    });
  }
  ...
  if (/^\d+$/.test(seed)) {
    return Effect.succeed({
      offset: Number(seed),
      source: `numeric NOT_CODEX_DEV_INSTANCE=${seed}`,
    });
  }

  const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
  return Effect.succeed({ offset, source: `hashed NOT_CODEX_DEV_INSTANCE=${seed}` });
}
...
function resolveBaseDir(baseDir: string | undefined): Effect.Effect<string, never, Path.Path> {
  return Effect.gen(function* () {
    ...
    return yield* DEFAULT_NOT_CODEX_HOME;
  });
}
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    if (!isDesktopMode) {
      ...
      delete output.NOT_CODEX_MODE;
      delete output.NOT_CODEX_NO_BROWSER;
      delete output.NOT_CODEX_HOST;
    }
    ...
    if (autoBootstrapProjectFromCwd !== undefined) {
      output.NOT_CODEX_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.NOT_CODEX_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.NOT_CODEX_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.NOT_CODEX_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.NOT_CODEX_MODE = "web";
      delete output.NOT_CODEX_DESKTOP_WS_URL;
    }
      ...
      Effect.mapError(
        (cause) =>
          new DevRunnerConfigurationError({
            configKeys: ["NOT_CODEX_PORT_OFFSET", "NOT_CODEX_DEV_INSTANCE"],
            cause,
          }),
      ),
    );
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to NOT_CODEX_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("NOT_CODEX_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription("WebSocket event logging toggle (equivalent to NOT_CODEX_LOG_WS_EVENTS)."),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("NOT_CODEX_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to NOT_CODEX_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("NOT_CODEX_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to NOT_CODEX_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("NOT_CODEX_PORT")),
  ),
