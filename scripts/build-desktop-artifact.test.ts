...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
  ...
  it("maps desktop versions to their packaged web asset brand", () => {
        ...
        Effect.provide(
          ConfigProvider.layer(
            ConfigProvider.fromEnv({
              env: {
                NOT_CODEX_DESKTOP_UPDATE_REPOSITORY: "MaTriXy/not-codex",
              },
            }),
          ),
        ),
      ...
      const nightlyConfig = yield* resolveGitHubPublishConfig("nightly").pipe(
        Effect.provide(
          ConfigProvider.layer(
            ConfigProvider.fromEnv({
              env: {
                GITHUB_REPOSITORY: "MaTriXy/not-codex",
              },
            }),
          ),
        ),
      ...
      resolveDesktopRuntimeDependencies(
        {
          ...
          "@notcodex/contracts": "workspace:*",
          "@notcodex/shared": "workspace:*",
          "@notcodex/ssh": "workspace:*",
          "@notcodex/tailscale": "workspace:*",
          effect: "catalog:",
          electron: "41.5.0",
        },
        {
  ...
  it("derives macOS passkey signing configuration from the Clerk publishable key", () => {
    const configuration = resolveMacPasskeySigningConfiguration({
      ...
      NOT_CODEX_MACOS_PROVISIONING_PROFILE: "/tmp/notcodex.provisionprofile",
      NOT_CODEX_CLERK_PUBLISHABLE_KEY: `pk_test_${btoa("example.clerk.accounts.dev$")}`,
    });

    assert.deepStrictEqual(configuration, {
      appId: "com.notcodex.notcodex",
      teamId: "ABC1234567",
      rpDomains: ["example.clerk.accounts.dev"],
      provisioningProfilePath: "/tmp/notcodex.provisionprofile",
    });
  });
  ...
  it("normalizes explicit macOS passkey RP domains and renders required entitlements", () => {
    const configuration = resolveMacPasskeySigningConfiguration({
      NOT_CODEX_APPLE_TEAM_ID: "ABC1234567",
      NOT_CODEX_MACOS_PROVISIONING_PROFILE: "/tmp/notcodex.provisionprofile",
      NOT_CODEX_CLERK_PASSKEY_RP_DOMAINS:
    ...
    assert.include(entitlements, "<string>ABC1234567.com.notcodex.notcodex</string>");
  ...
  it("rejects incomplete macOS passkey signing configuration", () => {
    ...
    const missingProfileError = captureError({
      NOT_CODEX_APPLE_TEAM_ID: "ABC1234567",
      NOT_CODEX_CLERK_PASSKEY_RP_DOMAINS: "example.clerk.accounts.dev",
    });
    assert.instanceOf(missingProfileError, MissingMacPasskeyProvisioningProfileError);
    assert.equal(
      missingProfileError.message,
      "NOT_CODEX_MACOS_PROVISIONING_PROFILE must point to an Associated Domains provisioning profile.",
    );
    ...
    const invalidDomainError = captureError({
      NOT_CODEX_APPLE_TEAM_ID: "ABC1234567",
      NOT_CODEX_MACOS_PROVISIONING_PROFILE: "/tmp/notcodex.provisionprofile",
      NOT_CODEX_CLERK_PASSKEY_RP_DOMAINS: unsafeDomain,
    });
    ...
    assert.throws(
      () =>
        resolveMacPasskeySigningConfiguration({
          NOT_CODEX_APPLE_TEAM_ID: "ABC1234567",
          NOT_CODEX_MACOS_PROVISIONING_PROFILE: "/tmp/notcodex.provisionprofile",
          NOT_CODEX_CLERK_PASSKEY_RP_DOMAINS: "example.clerk.accounts.dev:8443",
        }),
    ...
    const invalidPublishableKeyError = captureError({
      NOT_CODEX_APPLE_TEAM_ID: "ABC1234567",
      NOT_CODEX_MACOS_PROVISIONING_PROFILE: "/tmp/notcodex.provisionprofile",
      NOT_CODEX_CLERK_PUBLISHABLE_KEY: "pk_test_%",
    });
    ...
    assert.equal(invalidPublishableKeyError.message, "NOT_CODEX_CLERK_PUBLISHABLE_KEY is invalid.");
      ...
      const resolved = yield* resolveBuildOptions({
        ...
        Effect.provide(
          ConfigProvider.layer(
            ConfigProvider.fromEnv({
              env: {
                NOT_CODEX_DESKTOP_SKIP_BUILD: "true",
                NOT_CODEX_DESKTOP_KEEP_STAGE: "true",
                NOT_CODEX_DESKTOP_SIGNED: "true",
                NOT_CODEX_DESKTOP_VERBOSE: "true",
                NOT_CODEX_DESKTOP_MOCK_UPDATES: "true",
              },
            }),
          ),
        ),
