...
it("batches codesign calls without changing existing signing options", async () => {
  const options = {
    app: "/tmp/Not Codex.app",
    identity: "Developer ID Application: Not Codex",
    keychain: "/tmp/notcodex.keychain",
    provisioningProfile: "/tmp/notcodex.provisionprofile",
    optionsForFile: () => ({
      entitlements: "/tmp/notcodex.entitlements.plist",
