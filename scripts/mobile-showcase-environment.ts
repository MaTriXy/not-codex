// @effect-diagnostics nodeBuiltinImport:off globalDate:off - This host-side fixture creates an isolated local Not Codex environment.
...
export const SHOWCASE_PROJECT_ID = "notcodex";
...
export const SHOWCASE_TERMINAL_BUFFER = [
  "\u001b[38;5;75m~/Code/notcodex\u001b[0m \u001b[38;5;212mfeat/remote-command-center\u001b[0m",
  "$ vp test run --changed",
  "",
  "  \u001b[38;5;117mnotcodex-mobile\u001b[0m       184 passed",
...
const PROJECT_FAVICONS = {
  notcodex: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 128 128">
  <rect width="128" height="128" rx="10" fill="#000"/>
  <path d="M30 98V30h16l36 43V30h16v68H82L46 55v43H30Z" fill="#fff"/>
</svg>`,
...
export const SHOWCASE_PROJECTS = [
  {
    ...
    title: "Not Codex",
    directory: "notcodex",
    repositoryUrl: "https://github.com/MaTriXy/not-codex.git",
    favicon: PROJECT_FAVICONS.notcodex,
  },
  {
    id: "react",
    title: "React",
...
async function seedNotCodexWorkspace(workspaceRoot: string): Promise<void> {
  ...
  await NodeFSP.writeFile(
    ...
    `${JSON.stringify({ name: "notcodex", private: true, scripts: { test: "vp test" } }, null, 2)}\n`,
  );
  await NodeFSP.writeFile(NodePath.join(workspaceRoot, "favicon.svg"), PROJECT_FAVICONS.notcodex);
  await NodeFSP.writeFile(
...
async function seedCompanionWorkspace(input: {
  ...
  await NodeFSP.writeFile(
    ...
    `# ${input.title}\n\nSeeded by the Not Codex mobile screenshot harness.\n`,
  );
  await initializeRepository({
...
export async function seedShowcaseEnvironment(input: {
  ...
  if (primaryProject.id === SHOWCASE_PROJECT_ID) {
    await seedNotCodexWorkspace(workspaceRoot);
  }
  await Promise.all(
    projects
