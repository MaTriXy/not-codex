...
const APP_SCHEME = "notcodex-dev";
const IOS_READY_FILENAME = "NotCodexShowcaseReadyScene";
const SERVER_HOST = "0.0.0.0";
const IOS_SIMULATOR_ARCH = NodeProcess.arch === "arm64" ? "arm64" : "x86_64";
const IOS_APP_PATH = NodePath.join(
  MOBILE_ROOT,
  ".showcase/ios-derived-data/Build/Products/Debug-iphonesimulator/NotCodexDev.app",
);
...
Usage:
  pnpm --filter @notcodex/mobile screenshots [options]
...
function startMetro(config: ShowcaseConfig): NodeChildProcess.ChildProcess {
  return spawnProcess(
    "pnpm",
    ["exec", "expo", "start", "--dev-client", "--clear", "--port", String(config.metroPort)],
    {
      cwd: MOBILE_ROOT,
      env: {
        ...MOBILE_BUILD_ENV,
        // Screenshot capture is entirely local and must not depend on the
        // developer's Expo login (or Expo's API availability) to serve the
        // development manifest. A stale session otherwise turns the manifest
        // request into a 500 before the app can write its readiness sentinel.
        EXPO_OFFLINE: "1",
...
async function buildIos(): Promise<string> {
  ...
  await runCommand(
    ...
    [
      "-workspace",
      NodePath.join(MOBILE_ROOT, "ios/NotCodexDev.xcworkspace"),
      "-scheme",
      "NotCodexDev",
      "-configuration",
      "Debug",
...
async function writeAndroidShowcaseScene(serial: string, scene: ShowcaseScene): Promise<void> {
  await runAdb(serial, [
    "shell",
    `run-as ${ANDROID_PACKAGE} sh -c 'mkdir -p files && rm -f files/notcodex-showcase-ready && printf %s ${scene} > files/notcodex-showcase-scene'`,
  ]);
}
...
async function main(): Promise<void> {
  ...
  const showcaseRootDir = await NodeFSP.mkdtemp(
    NodePath.join(NodeOS.tmpdir(), "notcodex-mobile-showcase-"),
  );
