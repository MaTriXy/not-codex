...
it.layer(NodeServices.layer)("dev-runner", (it) => {
  ...
  describe("resolveOffset", () => {
    it.effect("uses explicit NOT_CODEX_PORT_OFFSET when provided", () =>
      Effect.gen(function* () {
        const result = yield* resolveOffset({ portOffset: 12, devInstance: undefined });
        assert.deepStrictEqual(result, {
          offset: 12,
          source: "NOT_CODEX_PORT_OFFSET=12",
        });
      }),
    );
    ...
    it.effect("returns structured context for a negative port offset", () =>
      Effect.gen(function* () {
        ...
        assert.equal(error.configKey, "NOT_CODEX_PORT_OFFSET");
  ...
  describe("createDevRunnerEnv", () => {
    ...
    it.effect("supports explicit typed overrides", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.NOT_CODEX_HOME, path.resolve("/tmp/custom-notcodex"));
        assert.equal(env.NOT_CODEX_PORT, "4222");
        assert.equal(env.VITE_HTTP_URL, "http://localhost:4222");
        assert.equal(env.VITE_WS_URL, "ws://localhost:4222");
        assert.equal(env.NOT_CODEX_NO_BROWSER, "1");
        assert.equal(env.NOT_CODEX_AUTO_BOOTSTRAP_PROJECT_FROM_CWD, "0");
        assert.equal(env.NOT_CODEX_LOG_WS_EVENTS, "1");
        assert.equal(env.NOT_CODEX_HOST, "0.0.0.0");
    ...
    it.effect("does not force websocket logging on in dev mode when unset", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {
            NOT_CODEX_LOG_WS_EVENTS: "keep-me-out",
          },
          serverOffset: 0,
          webOffset: 0,
          notCodexHome: undefined,
        ...
        assert.equal(env.NOT_CODEX_MODE, "web");
        assert.equal(env.NOT_CODEX_LOG_WS_EVENTS, undefined);
      }),
    );
    ...
    it.effect("forwards explicit websocket logging false without coercing it away", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.NOT_CODEX_LOG_WS_EVENTS, "0");
      }),
    );

    it.effect("uses custom notCodexHome when provided", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const env = yield* createDevRunnerEnv({
          mode: "dev",
        ...
        assert.equal(env.NOT_CODEX_HOME, path.resolve("/tmp/my-notcodex"));
      }),
    );
      ...
      Effect.gen(function* () {
        ...
        const env = yield* createDevRunnerEnv({
          ...
          baseEnv: {
            ...
            NOT_CODEX_MODE: "web",
            NOT_CODEX_NO_BROWSER: "0",
            NOT_CODEX_HOST: "0.0.0.0",
        ...
        assert.equal(env.NOT_CODEX_MODE, undefined);
        assert.equal(env.NOT_CODEX_NO_BROWSER, undefined);
        assert.equal(env.NOT_CODEX_HOST, undefined);
    ...
    it.effect("defaults dev server mode to the higher backend port range", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.NOT_CODEX_PORT, "13773");
    ...
    it.effect("preserves invalid configuration as the exact cause", () =>
      Effect.gen(function* () {
          ...
          Effect.provide(
            Layer.merge(
              ...
              ConfigProvider.layer(
                ConfigProvider.fromEnv({ env: { NOT_CODEX_PORT_OFFSET: "not-an-integer" } }),
              ),
            ),
          ),
          Effect.flip,
        ...
        assert.deepStrictEqual(error.configKeys, [
          "NOT_CODEX_PORT_OFFSET",
          "NOT_CODEX_DEV_INSTANCE",
        ]);
