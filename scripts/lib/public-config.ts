...
export interface NotCodexPublicConfig {
...
export function loadRepoEnv({
  ...
  return {
    ...
    ...(config.clerkPublishableKey
      ? {
          NOT_CODEX_CLERK_PUBLISHABLE_KEY: config.clerkPublishableKey,
    ...
    ...(config.clerkJwtTemplate
      ? {
          NOT_CODEX_CLERK_JWT_TEMPLATE: config.clerkJwtTemplate,
    ...
    ...(config.relayUrl
      ? {
          NOT_CODEX_RELAY_URL: config.relayUrl,
          VITE_NOT_CODEX_RELAY_URL: config.relayUrl,
        }
      : {}),
    ...(config.mobileOtlpTracesUrl
      ? {
          NOT_CODEX_MOBILE_OTLP_TRACES_URL: config.mobileOtlpTracesUrl,
          EXPO_PUBLIC_OTLP_TRACES_URL: config.mobileOtlpTracesUrl,
        }
      : {}),
    ...(config.mobileOtlpTracesDataset
      ? {
          NOT_CODEX_MOBILE_OTLP_TRACES_DATASET: config.mobileOtlpTracesDataset,
          EXPO_PUBLIC_OTLP_TRACES_DATASET: config.mobileOtlpTracesDataset,
        }
      : {}),
    ...(config.mobileOtlpTracesToken
      ? {
          NOT_CODEX_MOBILE_OTLP_TRACES_TOKEN: config.mobileOtlpTracesToken,
          EXPO_PUBLIC_OTLP_TRACES_TOKEN: config.mobileOtlpTracesToken,
        }
      : {}),
    ...(config.relayClientOtlpTracesUrl
      ? {
          NOT_CODEX_RELAY_CLIENT_OTLP_TRACES_URL: config.relayClientOtlpTracesUrl,
          VITE_RELAY_OTLP_TRACES_URL: config.relayClientOtlpTracesUrl,
        }
      : {}),
    ...(config.relayClientOtlpTracesDataset
      ? {
          NOT_CODEX_RELAY_CLIENT_OTLP_TRACES_DATASET: config.relayClientOtlpTracesDataset,
          VITE_RELAY_OTLP_TRACES_DATASET: config.relayClientOtlpTracesDataset,
        }
      : {}),
    ...(config.relayClientOtlpTracesToken
      ? {
          NOT_CODEX_RELAY_CLIENT_OTLP_TRACES_TOKEN: config.relayClientOtlpTracesToken,
...
export function resolvePublicConfig(...sources: readonly Environment[]): NotCodexPublicConfig {
  return {
