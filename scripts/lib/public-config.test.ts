...
describe("loadRepoEnv", () => {
  it("does not project cloud configuration for an unconfigured clone", () => {
    ...
    expect(env.NOT_CODEX_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.NOT_CODEX_CLERK_CLI_OAUTH_CLIENT_ID).toBeUndefined();
    expect(env.VITE_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.EXPO_PUBLIC_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.NOT_CODEX_CLERK_JWT_TEMPLATE).toBeUndefined();
    expect(env.VITE_CLERK_JWT_TEMPLATE).toBeUndefined();
    expect(env.EXPO_PUBLIC_CLERK_JWT_TEMPLATE).toBeUndefined();
    expect(env.NOT_CODEX_RELAY_URL).toBeUndefined();
    expect(env.VITE_NOT_CODEX_RELAY_URL).toBeUndefined();
    expect(env.NOT_CODEX_MOBILE_OTLP_TRACES_URL).toBeUndefined();
    expect(env.NOT_CODEX_MOBILE_OTLP_TRACES_DATASET).toBeUndefined();
    expect(env.NOT_CODEX_MOBILE_OTLP_TRACES_TOKEN).toBeUndefined();
    ...
    expect(env.NOT_CODEX_RELAY_CLIENT_OTLP_TRACES_URL).toBeUndefined();
    expect(env.NOT_CODEX_RELAY_CLIENT_OTLP_TRACES_DATASET).toBeUndefined();
    expect(env.NOT_CODEX_RELAY_CLIENT_OTLP_TRACES_TOKEN).toBeUndefined();
  ...
  it("applies process, root local, and root precedence in that order", () => {
    ...
    NodeFS.writeFileSync(
      ...
      "NOT_CODEX_CLERK_PUBLISHABLE_KEY=pk_root\nNOT_CODEX_CLERK_JWT_TEMPLATE=template_root\nNOT_CODEX_CLERK_CLI_OAUTH_CLIENT_ID=oauth_root\nNOT_CODEX_RELAY_URL=https://root.example.test\n",
    );
    NodeFS.writeFileSync(
      NodePath.join(repoRoot, ".env.local"),
      "NOT_CODEX_CLERK_PUBLISHABLE_KEY=pk_local\nNOT_CODEX_CLERK_JWT_TEMPLATE=template_local\nNOT_CODEX_CLERK_CLI_OAUTH_CLIENT_ID=oauth_local\nNOT_CODEX_RELAY_URL=https://local.example.test\n",
    );

    expect(loadRepoEnv({ baseEnv: {}, repoRoot }).NOT_CODEX_RELAY_URL).toBe(
      "https://local.example.test",
    );
    expect(
      loadRepoEnv({
        baseEnv: {
          NOT_CODEX_CLERK_PUBLISHABLE_KEY: "pk_ci",
          NOT_CODEX_CLERK_JWT_TEMPLATE: "template_ci",
          NOT_CODEX_CLERK_CLI_OAUTH_CLIENT_ID: "oauth_ci",
          NOT_CODEX_RELAY_URL: "https://ci.example.test",
        },
        repoRoot,
      }),
    ).toMatchObject({
      ...
      NOT_CODEX_RELAY_URL: "https://ci.example.test",
      VITE_NOT_CODEX_RELAY_URL: "https://ci.example.test",
    });
  });
  ...
  it("accepts legacy framework aliases as root overrides", () => {
    expect(
      resolvePublicConfig({
        ...
        NOT_CODEX_CLERK_CLI_OAUTH_CLIENT_ID: "oauth_canonical",
        VITE_NOT_CODEX_RELAY_URL: "https://legacy.example.test",
  ...
  it("projects canonical relay client tracing values to web build aliases", () => {
    expect(
      loadRepoEnv({
        baseEnv: {
          NOT_CODEX_RELAY_CLIENT_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
          NOT_CODEX_RELAY_CLIENT_OTLP_TRACES_DATASET: "relay-client-traces",
          NOT_CODEX_RELAY_CLIENT_OTLP_TRACES_TOKEN: "relay-client-token",
        },
    ...
    expect(
      loadRepoEnv({
        baseEnv: {
          ...
          NOT_CODEX_MOBILE_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
          NOT_CODEX_MOBILE_OTLP_TRACES_DATASET: "mobile-traces",
          NOT_CODEX_MOBILE_OTLP_TRACES_TOKEN: "mobile-token",
        },
        repoRoot: makeTemporaryDirectory(),
      }),
    ).toEqual({
      NOT_CODEX_RELAY_URL: "https://relay.example.test",
      VITE_NOT_CODEX_RELAY_URL: "https://relay.example.test",
      NOT_CODEX_MOBILE_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
      NOT_CODEX_MOBILE_OTLP_TRACES_DATASET: "mobile-traces",
      NOT_CODEX_MOBILE_OTLP_TRACES_TOKEN: "mobile-token",
      EXPO_PUBLIC_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
...
function makeTemporaryDirectory() {
  const directory = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "notcodex-public-config-"));
