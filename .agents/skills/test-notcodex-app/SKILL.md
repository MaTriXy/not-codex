---
name: test-notcodex-app
description: Launch and test the Not Codex web app in isolated development environments, including first-try browser authentication with one-time pairing URLs, pairing-token recovery, worktree-safe state directories, dev server lifecycle, and direct SQLite inspection or fixture seeding. Use when an agent needs to run Not Codex locally, test UI behavior in a browser, recover from an expired or consumed pairing token, isolate dev state, or prepare test data in state.sqlite.
---
...
```bash
NOT_CODEX_PORT=<server-port> node apps/server/src/bin.ts auth pairing create \
  --base-dir <base-dir> \
  --dev-url <web-url> \
  --base-url <web-url> \
  --ttl 15m \
...
Use the `Pair URL` from this command once. Derive `<server-port>` and `<web-url>` from the current dev-runner output, including any automatically selected port offset. Setting `NOT_CODEX_PORT` keeps the administrative CLI from probing for an unrelated free port.
...
- Use `node apps/server/scripts/notcodex-sqlite-state.ts query` for schema discovery and read-only checks.
- Stop the dev server before using `node apps/server/scripts/notcodex-sqlite-state.ts exec`, then restart it with the same base directory.
...
The helper refuses to write to the shared `~/.notcodex` directory by default and creates a database backup before each mutation.
