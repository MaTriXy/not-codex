...
Load this reference only when inspecting or seeding local Not Codex state directly.
...
```bash
node apps/server/scripts/notcodex-sqlite-state.ts query \
  --base-dir <base-dir> \
...
```bash
node apps/server/scripts/notcodex-sqlite-state.ts exec \
  --base-dir <base-dir> \
  --file /tmp/notcodex-seed.sql
```
