# Not Codex Connect Relay

> [!WARNING]
> Not Codex Connect is currently in private beta. Join the waitlist in the app under Settings > Not Codex Connect.

The relay is the hosted control plane for Not Codex Connect. It helps clients discover and connect to
...
The relay is intentionally not in the hot path for normal Not Codex traffic. After a client connects,
regular API and WebSocket traffic goes directly between that client and the selected environment.
See the [Not Codex Connect architecture overview](../../docs/cloud/not-codex-connect-auth-flow.html) for the larger system
design.
...
- Linking Not Codex environments to a cloud account.
...
```sh
vp run --filter notcodex-relay deploy
```
...
```sh
vp run --filter notcodex-relay deploy -- --stage prod
vp run --filter notcodex-relay deploy -- --env-file .env.local
```
...
`relay.<RELAY_API_ZONE_NAME>` and `dev_collaborator` uses
`relay-dev-collaborator.<RELAY_API_ZONE_NAME>`. Managed environment endpoints are provisioned below
...
- [Not Codex Connect Clerk Setup](../../docs/cloud/notcodex-connect-clerk.md) for Clerk keys, JWT templates, and waitlist
  setup.
- [Relay Observability](../../docs/operations/relay-observability.md) for deployment tracing and diagnostics.
- [Not Codex Connect Architecture Overview](../../docs/cloud/not-codex-connect-auth-flow.html) for the full link,
