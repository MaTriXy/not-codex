...
const publicConfigEnvEntries = (config: RelayPublicConfig) =>
  ({
    ...
    NOT_CODEX_MOBILE_OTLP_TRACES_URL: config.mobileTracingUrl,
    NOT_CODEX_MOBILE_OTLP_TRACES_DATASET: config.mobileTracingDataset,
    NOT_CODEX_MOBILE_OTLP_TRACES_TOKEN: config.mobileTracingToken,
    NOT_CODEX_RELAY_CLIENT_OTLP_TRACES_URL: config.clientTracingUrl,
    NOT_CODEX_RELAY_CLIENT_OTLP_TRACES_DATASET: config.clientTracingDataset,
    NOT_CODEX_RELAY_CLIENT_OTLP_TRACES_TOKEN: config.clientTracingToken,
  }) as const;
...
export function reconcileRootEnvRelayUrl(contents: string, relayUrl: string): string {
  return reconcileRootEnvPublicConfig(contents, {
    ...
    .split("\n")
    .filter((line) => !line.startsWith("NOT_CODEX_MOBILE_OTLP_TRACES_"))
    .filter((line) => !line.startsWith("NOT_CODEX_RELAY_CLIENT_OTLP_TRACES_"))
    .join("\n");
}
...
const readRelayPublicConfig = Effect.fn("relay.deploy.readState")(function* (stage: string) {
  ...
  const output = yield* service.getOutput({ stack: "NotCodexRelay", stage });
...
export const relayDeployCommand = Command.make(
  ...
  deploy,
).pipe(Command.withDescription("Deploy the Not Codex relay through Alchemy."));
