...
function makeEnvironmentLinks(
  ...
  return {
    upsert: () => Effect.void,
    listUsersForEnvironment: () => Effect.succeed(["dev:collaborator"]),
    listDeliveryUsersForEnvironment: () =>
      Effect.succeed([
        {
          userId: "dev:collaborator",
          notificationsEnabled: true,
          liveActivitiesEnabled: true,
        },
      ]),
...
const config = RelayConfiguration.RelayConfiguration.of({
  ...
  apns: {
    ...
    teamId: "team-id",
    keyId: "key-id",
    bundleId: "codes.notcodex.mobile",
  ...
  it.effect("registers devices through the device persistence service", () => {
    ...
    return Effect.gen(function* () {
      const result = yield* Effect.gen(function* () {
        ...
        return yield* registrations.registerDevice({ userId: "dev:collaborator", payload: device });
      }).pipe(
        Effect.provide(
      ...
      expect(registered).toEqual({ userId: "dev:collaborator", registration: device });
