...
import { RELAY_HEALTH_RESPONSE_TYP, RELAY_MINT_RESPONSE_TYP } from "@notcodex/shared/relayJwt";
...
function signMintResponse(
  ...
  const payload = {
    iss: `notcodex-env:${requestProof.environmentId}`,
...
describe("EnvironmentConnector", () => {
  ...
  it.effect("checks linked environment health through the managed endpoint", () => {
    ...
    return Effect.gen(function* () {
      ...
      expect(seenUrls).toEqual(["https://env.example.test/api/not-codex-connect/health"]);
      expect(seenProofs[0]).toMatchObject({
        iss: "https://relay.example.test",
        aud: "notcodex-env:env-connector-test",
        sub: "user_123",
        environmentId: "env-connector-test",
  ...
  it.effect("mints a one-time environment credential through the linked endpoint", () => {
    ...
    return Effect.gen(function* () {
      ...
      expect(seenUrls).toEqual(["https://env.example.test/api/not-codex-connect/mint-credential"]);
      expect(seenProofs[0]).toMatchObject({
        iss: "https://relay.example.test",
        aud: "notcodex-env:env-connector-test",
        sub: "user_123",
